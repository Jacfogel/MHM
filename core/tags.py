...
def _load_default_tags_from_resources() -> List[str]:
    ...
    """
    try:
        default_tags_file = Path(__file__).parent.parent / 'resources' / 'default_tags.json'
...
def load_user_tags(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not tags_file.exists():
            logger.debug(f"Tags file not found for user {user_id}, initializing with defaults")
            ...
            logger.info(f"Initialized tags file for user {user_id} with {len(default_tags)} default tags")
            return tags_data
        ...
        if data is None:
            logger.warning(f"Tags file corrupted for user {user_id}, reinitializing with defaults")
            default_tags = _load_default_tags_from_resources()
            tags_data = {
                'tags': default_tags,
                'metadata': {
