...
@handle_errors("checking dev tools run", default_return=False)
def _is_dev_tools_run():
    """True when entry point or env indicates development tools (audit, scripts, etc.)."""
    if os.getenv("MHM_DEV_TOOLS_RUN") == "1":
        return True
    argv_str = " ".join(getattr(sys, "argv", []))
    return "development_tools" in argv_str or "run_dev_tools" in argv_str
...
def apply_test_context_formatter_to_all_loggers():
    ...
    if count > 0:
        print(
            f"DEBUG: Applied PytestContextLogFormatter to {count} handlers",
            file=sys.stderr,
        )
...
class ComponentLogger:
    ...
    def __init__(
        self, component_name: str, log_file_path: str, level: int = logging.INFO
    ):
        """
        ...
        if os.getenv("TEST_CONSOLIDATED_LOGGING") == "1" and _is_testing_environment():
            ...
            try:
                ...
                consolidated_handler = logging.FileHandler(
                    consolidated_log_file, encoding="utf-8"
                )
...
class BackupDirectoryRotatingFileHandler(TimedRotatingFileHandler):
    ...
    def doRollover(self):
        ...
        # Use actual rotation time for backup naming. rolloverAt can be stale when rotation was
        # skipped on previous midnights (e.g. file too small or too new), which would produce
        # wrong backup dates and overwrite older backups with newer content (data loss).
        current_time = int(time.time())
...
class HeartbeatWarningFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if (
            ...
            and "heartbeat blocked" in record.getMessage()
        ):
...
def get_component_logger(component_name: str) -> ComponentLogger:
    ...
    if os.getenv("MHM_TESTING") == "1":
        ...
        if verbose_logs in ("1", "2"):
            try:
                # Use configurable test logs directory
                tests_logs_dir = os.getenv("LOGS_DIR") or os.getenv(
                    "TEST_LOGS_DIR", str(Path("tests") / "logs")
                )
                os.makedirs(tests_logs_dir, exist_ok=True)
            except Exception:
            ...
            try:
                ...
                for handler in comp_logger.logger.handlers:
                    if isinstance(
                        ...
                        log_paths = _get_log_paths_for_environment()
                        if base_filename and os.path.basename(
                            base_filename
                        ) == os.path.basename(log_paths["discord_file"]):
...
def setup_logging():
    ...
    # When running as development tools or short-lived subprocess, keep init at DEBUG to reduce app.log noise
    _is_tool_or_short_lived = "development_tools" in " ".join(
        getattr(sys, "argv", [])
    ) or "run_dev_tools" in " ".join(getattr(sys, "argv", []))
    _init_level = logging.DEBUG if _is_tool_or_short_lived else logging.INFO
    if not hasattr(setup_logging, "_logged_initialization"):
        logger.log(
            _init_level,
            f"Logging initialized - Console level: {logging.getLevelName(log_level)}, File level: DEBUG",
        )
        logger.log(
            _init_level,
...
def get_log_file_info():
    ...
    try:
        ...
        if os.path.exists(log_paths["backup_dir"]):
            backup_pattern = str(
                Path(log_paths["backup_dir"]) / f"{Path(log_paths['main_file']).name}*"
            )
...
def cleanup_old_logs(max_total_size_mb=50):
    ...
    for log_file, _ in log_files_with_time:
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Failed to remove log file {log_file}: {e}"
            )

    if removed_count > 0:
        logging.getLogger(__name__).info(
            f"Log cleanup completed: removed {removed_count} files"
        )
        return True
                    ...
                    if os.path.exists(log_file):
                        ...
                        try:
                            with open(log_file, "rb") as f_in:
                                ...
                                archive_file = str(
                                    Path(log_paths["archive_dir"]) / f"{filename}.gz"
                                )
                            ...
                            compressed_count += 1
                            logging.getLogger(__name__).info(
                                f"Compressed and archived: {log_file}"
                            )

                        except Exception as e:
                            logging.getLogger(__name__).warning(
                                f"Failed to compress {log_file}: {e}"
                            )

        if compressed_count > 0:
            logging.getLogger(__name__).info(
                f"Log archival completed: {compressed_count} files compressed and archived"
            )
...
def cleanup_old_archives(max_days=30):
    ...
    try:
        ...
        if removed_count > 0:
            logging.getLogger(__name__).info(
                f"Archive cleanup completed: {removed_count} files removed"
            )
