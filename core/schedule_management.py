    ...
    if periods:
        ...
        for period_name, period_data in periods.items():
            if not isinstance(period_data, dict):
                logger.warning(
                    f"Period {period_name} in category {category} is not a dictionary: {period_data}"
                )
                continue
        ...
        logger.debug(
            f"Retrieved and sorted schedule time periods for user {user_id}, category {category}"
        )
        ...
        if category not in ("tasks", "checkin"):
            ...
            logger.debug(
                f"No schedule periods found for user {user_id}, category {category}, returning ALL period"
            )
            return all_period
        else:
            _schedule_periods_cache[cache_key] = ({}, current_time)
            logger.debug(
                f"No schedule periods found for user {user_id}, category {category}, returning empty period"
            )
            return {}
...
def set_schedule_period_active(user_id, category, period_name, active=True):
    ...
    if period_name in current_periods:
        ...
        logger.info(
            f"Period {period_name} in category {category} for user {user_id} has been {status}."
        )
        return True
    else:
        logger.error(
            f"Period {period_name} not found in category {category} for user {user_id}."
        )
        return False
...
def get_current_time_periods_with_validation(user_id, category):
    ...
    if user_id is None:
        logger.error(
            "get_current_time_periods_with_validation called with None user_id"
        )
        return [], []
    ...
    if not matching_periods:
        ...
        if valid_periods:
            logger.info(
                f"Current time is not within any defined period, defaulting to first period: {valid_periods[0]}. Available periods: {valid_periods}"
            )
...
def add_schedule_period(
    ...
    if period_name in current_periods:
        logger.warning(
            f"Period '{period_name}' already exists in category '{category}' for user {user_id}"
        )
        raise ValidationError(
            f"A period named '{period_name}' already exists in this category. Please choose a different name.",
            details={
    ...
    set_schedule_periods(user_id, category, current_periods)
    logger.info(
        f"Added period {period_name} in category {category} for user {user_id}."
    )
    ...
    if scheduler_manager:
        ...
        if created is False:
            logger.debug(
                "Skipped creating reschedule request because service is not running"
            )


@handle_errors("editing schedule period")
def edit_schedule_period(
    ...
    if period_name in current_periods:
        ...
        if success:
            logger.info(
                f"Edited period {period_name} in category {category} for user {user_id}."
            )
        else:
            logger.error(
                f"Failed to save edited period {period_name} in category {category} for user {user_id}."
            )
    else:
        logger.warning(
            f"Period {period_name} not found in category {category} for user {user_id}."
...
def delete_schedule_period(category, period_name, scheduler_manager=None):
    ...
    if period_name in current_periods:
        ...
        set_schedule_periods(user_id, category, current_periods)
        logger.info(
            f"Deleted period {period_name} from category {category} for user {user_id}."
        )
...
def clear_schedule_periods_cache(user_id=None, category=None):
    ...
    if user_id is None and category is None:
        ...
        if cache_key in _schedule_periods_cache:
            ...
            logger.debug(
                f"Cleared schedule periods cache for user {user_id}, category {category}"
            )
...
def get_period_data__validate_and_format_time(time_str):
    ...
    raise ValidationError(
        f"Invalid time format: {time_str}", details={"time_str": time_str}
    )
...
def get_period_data__time_12h_display_to_24h(hour_12, minute, is_pm):
    ...
    try:
        ...
        logger.error(
            f"Error converting 12-hour time to 24-hour: hour={hour_12}, minute={minute}, is_pm={is_pm}, error={e}"
        )
        raise ValidationError(
...
def set_schedule_periods(user_id, category, periods_dict):
    ...
    for period_name, period_data in periods_dict.items():
        if not isinstance(period_name, str):
            logger.warning(
                f"set_schedule_periods: period_name is not a string: {period_name} (type: {type(period_name)}) - converting to string."
            )
