...
class MHMService:
                ...
                if categories:  # Only process if list is not empty
                    for category in categories:
                        try:
                            ...
                            logger.error(
                                f"Error determining file path for category '{category}' and user '{user_id}': {e}"
                            )
    ...
    def _check_and_fix_logging__verify_test_message_present(
        self, recent_content, test_message, test_timestamp
    ):
        """Check if our test message or recent timestamp patterns are present in log content."""
        if test_message in recent_content or any(
            str(test_timestamp - i) in recent_content for i in range(60)
        ):
    ...
    def check_and_fix_logging(self):
        ...
        try:
            ...
            try:
                ...
                if self._check_and_fix_logging__verify_test_message_present(
                    recent_content, test_message, test_timestamp
                ):
                    return

                # Check if there's any recent activity (within last 5 minutes)
                if self._check_and_fix_logging__check_recent_activity_timestamps(
                    recent_content
                ):
                    return
            ...
            logger.error(
                f"Logging system is not working - cannot write log messages: {log_error}"
            )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Tests data directory cleanup failed (non-critical): {e}"
                    )
            except Exception as e:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.error(
                        f"Failed to initialize CommunicationManager on attempt {attempt + 1}/{max_retries}: {e}"
                    )
                    time.sleep(1)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.error(
                        f"Failed to initialize SchedulerManager on attempt {attempt + 1}/{max_retries}: {e}"
                    )
                    time.sleep(1)
    ...
    def _cleanup_test_message_requests__remove_request_file(
        self, request_file, filename
    ):
