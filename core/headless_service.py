...
class HeadlessServiceManager:
    ...
    def can_start_headless_service(self):
        ...
        if is_headless_running:
            logger.info(
                "Headless service is already running - will restart existing services"
            )
        ...
        if is_ui_service_running():
            logger.info(
                "UI service is running - will stop UI services before starting headless"
            )
    ...
    def start_headless_service(self):
        ...
        if processes:
            logger.warning(
                f"Found {len(processes)} existing service processes, cleaning up..."
            )
        ...
        try:
            ...
            self.running = True
            logger.info(
                f"Headless service started with PID: {self.service_process.pid}"
            )
    ...
    def stop_headless_service(self):
        ...
        try:
            ...
            logger.warning(
                "Graceful shutdown timed out, attempting to terminate process"
            )
    ...
    def get_service_info(self):
        ...
        for proc in processes:
            logger.debug(
                f"Process {proc['pid']}: {proc['process_type']} - {' '.join(proc['cmdline'])}"
            )
        ...
        info = {
            ...
            "unknown_services": [
                p for p in processes if p["process_type"] == "unknown"
            ],
