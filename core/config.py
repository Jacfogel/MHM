...
from core.error_handling import handle_configuration_error, handle_errors
...
class ConfigValidationError(Exception):
    ...
    def __init__(
        self,
        message: str,
        missing_configs: list[str] | None = None,
...
USER_INFO_DIR_PATH = _normalize_path(
    os.getenv("USER_INFO_DIR_PATH", str(Path(BASE_DATA_DIR) / "users"))
)
# Fix: Use forward slashes for cross-platform compatibility and avoid path normalization issues
DEFAULT_MESSAGES_DIR_PATH = os.getenv(
...
LM_STUDIO_AUTO_START = (
    os.getenv("LM_STUDIO_AUTO_START", "true").lower() == "true"
)  # Auto-start LM Studio if not running
LM_STUDIO_AUTO_LOAD_MODEL = (
    os.getenv("LM_STUDIO_AUTO_LOAD_MODEL", "true").lower() == "true"
)  # Auto-load model if not loaded
LM_STUDIO_STARTUP_TIMEOUT = int(
    os.getenv("LM_STUDIO_STARTUP_TIMEOUT", "60")
)  # Timeout for LM Studio startup (seconds)
LM_STUDIO_MODEL_LOAD_TIMEOUT = int(
    os.getenv("LM_STUDIO_MODEL_LOAD_TIMEOUT", "30")
...
)  # Increased timeout for resource-constrained systems
AI_BATCH_SIZE = int(os.getenv("AI_BATCH_SIZE", "4"))  # For batch processing
AI_CUDA_WARMUP = (
    os.getenv("AI_CUDA_WARMUP", "false").lower() == "true"
)  # Disabled for LM Studio
...
AI_CONNECTION_TEST_TIMEOUT = int(
    os.getenv("AI_CONNECTION_TEST_TIMEOUT", "15")
)  # Timeout for testing LM Studio connection
AI_API_CALL_TIMEOUT = int(
    os.getenv("AI_API_CALL_TIMEOUT", "15")
)  # Default timeout for LM Studio API calls
AI_COMMAND_PARSING_TIMEOUT = int(
    os.getenv("AI_COMMAND_PARSING_TIMEOUT", "15")
)  # Timeout for AI-enhanced command parsing
AI_PERSONALIZED_MESSAGE_TIMEOUT = int(
    os.getenv("AI_PERSONALIZED_MESSAGE_TIMEOUT", "40")
)  # Longer timeout for personalized messages
AI_CONTEXTUAL_RESPONSE_TIMEOUT = int(
    os.getenv("AI_CONTEXTUAL_RESPONSE_TIMEOUT", "35")
)  # Timeout for contextual responses
AI_QUICK_RESPONSE_TIMEOUT = int(
    os.getenv("AI_QUICK_RESPONSE_TIMEOUT", "8")
)  # Shorter timeout for real-time interactions

# Command Parsing Confidence Thresholds
AI_RULE_BASED_HIGH_CONFIDENCE_THRESHOLD = float(
    os.getenv("AI_RULE_BASED_HIGH_CONFIDENCE_THRESHOLD", "0.8")
)  # Threshold for high-confidence rule-based parsing
AI_AI_ENHANCED_CONFIDENCE_THRESHOLD = float(
    os.getenv("AI_AI_ENHANCED_CONFIDENCE_THRESHOLD", "0.6")
)  # Threshold for using AI-enhanced parsing results
AI_RULE_BASED_FALLBACK_THRESHOLD = float(
    os.getenv("AI_RULE_BASED_FALLBACK_THRESHOLD", "0.3")
)  # Threshold for fallback to rule-based parsing
AI_AI_PARSING_BASE_CONFIDENCE = float(
    os.getenv("AI_AI_PARSING_BASE_CONFIDENCE", "0.7")
...
AI_MAX_RESPONSE_LENGTH = int(
    os.getenv("AI_MAX_RESPONSE_LENGTH", "1200")
)  # Maximum character length for AI-generated responses
AI_MAX_RESPONSE_WORDS = (
    int(os.getenv("AI_MAX_RESPONSE_WORDS", "0")) or None
)  # Optional word limit (0 = disabled)
AI_MAX_RESPONSE_TOKENS = int(
    os.getenv("AI_MAX_RESPONSE_TOKENS", "300")
)  # Maximum tokens for AI responses
AI_MIN_RESPONSE_LENGTH = int(
    os.getenv("AI_MIN_RESPONSE_LENGTH", "50")
)  # Minimum character length to avoid too-short responses

# AI Temperature Configuration - Controls response randomness for different modes
AI_CHAT_TEMPERATURE = float(
    os.getenv("AI_CHAT_TEMPERATURE", "0.7")
)  # Chat mode: conversational and varied
AI_COMMAND_TEMPERATURE = float(
    os.getenv("AI_COMMAND_TEMPERATURE", "0.0")
)  # Command parsing: deterministic classification
AI_CLARIFICATION_TEMPERATURE = float(
    os.getenv("AI_CLARIFICATION_TEMPERATURE", "0.1")
)  # Clarification: consistent clarification requests
...
MHM_FLAGS_DIR = os.getenv(
    "MHM_FLAGS_DIR"
)  # Directory for service flag files (defaults to project root)

# Test Environment Configuration
TEST_LOGS_DIR = os.getenv(
    "TEST_LOGS_DIR"
)  # Test logs directory (defaults to tests/logs)
TEST_DATA_DIR = os.getenv(
    "TEST_DATA_DIR"
)  # Test data directory (defaults to tests/data)

# Logging Configuration
LOG_LEVEL = os.getenv(
    "LOG_LEVEL", "WARNING"
).upper()  # Default to WARNING for quiet operation
LOG_MAX_BYTES = int(os.getenv("LOG_MAX_BYTES", "5242880"))  # 5MB default
LOG_BACKUP_COUNT = int(
    os.getenv("LOG_BACKUP_COUNT", "7")
)  # Keep 7 backup files (standardized for consistency)
LOG_COMPRESS_BACKUPS = (
    os.getenv("LOG_COMPRESS_BACKUPS", "false").lower() == "true"
)  # Compress old logs
...
LOGS_DIR = _normalize_path(
    os.getenv("LOGS_DIR", _default_logs_dir)
)  # Main logs directory
LOG_BACKUP_DIR = _normalize_path(
    os.getenv("LOG_BACKUP_DIR", str(Path(LOGS_DIR) / "backups"))
)  # Backup directory for rotated logs
LOG_ARCHIVE_DIR = _normalize_path(
    os.getenv("LOG_ARCHIVE_DIR", str(Path(LOGS_DIR) / "archive"))
)  # Archive directory for old logs

# Component-specific log files
LOG_MAIN_FILE = _normalize_path(
    os.getenv("LOG_MAIN_FILE", str(Path(LOGS_DIR) / "app.log"))
)  # Main application log
LOG_DISCORD_FILE = _normalize_path(
    os.getenv("LOG_DISCORD_FILE", str(Path(LOGS_DIR) / "discord.log"))
)  # Discord bot log
LOG_AI_FILE = _normalize_path(
    os.getenv("LOG_AI_FILE", str(Path(LOGS_DIR) / "ai.log"))
)  # AI interactions log
LOG_USER_ACTIVITY_FILE = _normalize_path(
    os.getenv("LOG_USER_ACTIVITY_FILE", str(Path(LOGS_DIR) / "user_activity.log"))
)  # User activity log
LOG_ERRORS_FILE = _normalize_path(
    os.getenv("LOG_ERRORS_FILE", str(Path(LOGS_DIR) / "errors.log"))
)  # Errors only log
LOG_COMMUNICATION_MANAGER_FILE = _normalize_path(
    os.getenv(
        "LOG_COMMUNICATION_MANAGER_FILE",
        str(Path(LOGS_DIR) / "communication_manager.log"),
    )
)  # Communication manager log
LOG_EMAIL_FILE = _normalize_path(
    os.getenv("LOG_EMAIL_FILE", str(Path(LOGS_DIR) / "email.log"))
)  # Email channel log
LOG_UI_FILE = _normalize_path(
    os.getenv("LOG_UI_FILE", str(Path(LOGS_DIR) / "ui.log"))
)  # UI interactions log
LOG_FILE_OPS_FILE = _normalize_path(
    os.getenv("LOG_FILE_OPS_FILE", str(Path(LOGS_DIR) / "file_ops.log"))
)  # File operations log
...
if not all(
    [EMAIL_SMTP_SERVER, EMAIL_IMAP_SERVER, EMAIL_SMTP_USERNAME, EMAIL_SMTP_PASSWORD]
):
...
DISCORD_WEBHOOK_PORT = int(
    os.getenv("DISCORD_WEBHOOK_PORT", "8080")
)  # Port for webhook server
# Auto-launch ngrok for webhook tunneling (development only)
DISCORD_AUTO_NGROK = os.getenv("DISCORD_AUTO_NGROK", "false").lower() in (
    "true",
    "1",
    "yes",
)
...
@handle_errors(
    "validating AI configuration", default_return=(False, ["Validation failed"], [])
)
def validate_ai_configuration() -> tuple[bool, list[str], list[str]]:
    ...
    try:
        ...
        if not LM_STUDIO_BASE_URL:
            ...
            errors.append(
                "LM_STUDIO_BASE_URL must be a valid URL starting with http:// or https://"
            )

        # LM_STUDIO_API_KEY has a default value, so we only warn if it's explicitly set to empty
        if LM_STUDIO_API_KEY == "":
            warnings.append(
                "LM_STUDIO_API_KEY is explicitly set to empty (using default)"
            )

        # LM_STUDIO_MODEL has a default value, so we only warn if it's explicitly set to empty
        if LM_STUDIO_MODEL == "":
            warnings.append(
                "LM_STUDIO_MODEL is explicitly set to empty (using default)"
            )

        # Check AI performance settings
        if AI_TIMEOUT_SECONDS < 5:
            warnings.append(
                "AI_TIMEOUT_SECONDS is very low (< 5 seconds), may cause timeouts"
            )
        elif AI_TIMEOUT_SECONDS > 300:
            warnings.append(
                "AI_TIMEOUT_SECONDS is very high (> 5 minutes), may cause long waits"
            )
        ...
        if AI_BATCH_SIZE < 1:
            ...
            warnings.append(
                "AI_BATCH_SIZE is very high (> 20), may cause memory issues"
            )

        # Check cache settings
        if CONTEXT_CACHE_TTL < 60:
            warnings.append(
                "CONTEXT_CACHE_TTL is very low (< 1 minute), may cause frequent cache misses"
            )
        elif CONTEXT_CACHE_TTL > 3600:
            warnings.append(
                "CONTEXT_CACHE_TTL is very high (> 1 hour), may use excessive memory"
            )

        if CONTEXT_CACHE_MAX_SIZE < 10:
            warnings.append(
                "CONTEXT_CACHE_MAX_SIZE is very low (< 10), may cause frequent cache evictions"
            )
        elif CONTEXT_CACHE_MAX_SIZE > 1000:
            warnings.append(
                "CONTEXT_CACHE_MAX_SIZE is very high (> 1000), may use excessive memory"
            )
...
def validate_communication_channels() -> tuple[bool, list[str], list[str]]:
    ...
    try:
        ...
        if email_missing:
            warnings.append(
                f"Email channel disabled - missing: {', '.join(email_missing)}"
            )
        else:
            available_channels.append("email")
            ...
            if email_user and "@" not in email_user:
                warnings.append(
                    "EMAIL_SMTP_USERNAME doesn't appear to be a valid email address"
                )
        ...
        if not discord_token:
            warnings.append(
                "Discord channel disabled - DISCORD_BOT_TOKEN not configured"
            )
        else:
            available_channels.append("discord")
            ...
            if not discord_token.startswith(("MT", "OT", "NT")):
                warnings.append(
                    "DISCORD_BOT_TOKEN doesn't match expected Discord bot token format"
                )

        # If no channels are configured, treat as warning for flexibility
        if not available_channels:
            warnings.append(
                "No communication channels are properly configured. At least one channel (Email or Discord) should be configured."
            )
...
def validate_logging_configuration() -> tuple[bool, list[str], list[str]]:
    ...
    try:
        ...
        if LOG_MAX_BYTES < 1024 * 1024:  # Less than 1MB
            warnings.append(
                f"LOG_MAX_BYTES ({LOG_MAX_BYTES}) is very small, consider increasing"
            )
        ...
        if LOG_BACKUP_COUNT < 1:
            ...
            warnings.append(
                f"LOG_BACKUP_COUNT ({LOG_BACKUP_COUNT}) is very high, consider reducing"
            )
        ...
        if os.path.exists(LOG_MAIN_FILE):
            ...
            if current_size_mb > 10:  # More than 10MB
                warnings.append(
                    f"Current log file is {current_size_mb:.1f}MB, consider reducing LOG_LEVEL or increasing LOG_MAX_BYTES"
                )
...
def validate_scheduler_configuration() -> tuple[bool, list[str], list[str]]:
    ...
    try:
        ...
        if SCHEDULER_INTERVAL < 10:
            ...
            warnings.append(
                "SCHEDULER_INTERVAL is very low (< 30 seconds), may cause high CPU usage"
            )
        elif SCHEDULER_INTERVAL > 3600:
            warnings.append(
                "SCHEDULER_INTERVAL is very high (> 1 hour), may cause delayed responses"
            )
...
@handle_errors(
    "validating file organization settings",
    default_return=(False, ["Validation failed"], []),
)
def validate_file_organization_settings() -> tuple[bool, list[str], list[str]]:
    ...
    try:
        ...
        if AUTO_CREATE_USER_DIRS:
            warnings.append(
                "AUTO_CREATE_USER_DIRS is enabled - user directories will be created automatically"
            )
...
def validate_environment_variables() -> tuple[bool, list[str], list[str]]:
    ...
    try:
        ...
        if problematic_vars:
            warnings.append(
                f"Empty environment variables found: {', '.join(problematic_vars)}"
            )
...
def validate_all_configuration() -> dict[str, Any]:
    ...
    try:
        ...
        if all_errors:
            ...
            summary = (
                f"Configuration validation passed with {len(all_warnings)} warning(s)"
            )
        else:
