...
class MessageAnalytics:
    ...
    def get_message_frequency(
        ...
        messages = get_recent_messages(
            user_id, category=category, limit=1000, days_back=days
        )
        ...
        avg_per_category = (
            {cat: count / days for cat, count in category_counts.items()}
            if days > 0
            else {}
        )
        ...
        return {
            ...
            "category_averages": {
                cat: round(avg, 2) for cat, avg in avg_per_category.items()
            },
            "time_period_counts": dict(time_period_counts),
            "category_time_period_breakdown": {
                cat: dict(periods)
                for cat, periods in category_time_period_counts.items()
            },
            "most_active_day": (
                {"date": most_active_day[0], "count": most_active_day[1]}
                if most_active_day
                else None
            ),
    ...
    def get_message_summary(self, user_id: str, days: int = 30) -> dict:
        ...
        return {
            ...
            "top_category": (
                ...
                if frequency_data.get("category_counts")
                else None
            ),
