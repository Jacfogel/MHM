...
from PySide6.QtWidgets import (
    QWidget, QMessageBox, QCheckBox, QHBoxLayout, QPushButton, 
    QVBoxLayout, QGroupBox, QLabel, QSpinBox, QComboBox, QFormLayout
)
...
class CheckinSettingsWidget(QWidget):
    ...
    def __init__(self, parent=None, user_id=None):
        ...
        self.dynamic_question_checkboxes = {}  # question_key -> (always_checkbox, sometimes_checkbox, edit_button, delete_button, container_widget)
    ...
    def _setup_question_count_controls(self):
        ...
        self.min_questions_spinbox.setToolTip("Minimum number of questions to ask per check-in. Must be at least the number of 'always include' questions (+1 if any 'sometimes' questions are enabled).")
        ...
        self.max_questions_spinbox.setToolTip("Maximum number of questions to ask per check-in. Must be >= minimum and <= total enabled questions.")
    ...
    def load_existing_data(self):
        ...
        if self.min_questions_spinbox:
            self.min_questions_spinbox.setValue(checkin_settings.get('min_questions', 1))
        if self.max_questions_spinbox:
            self.max_questions_spinbox.setValue(checkin_settings.get('max_questions', 8))
        ...
        for question_key, widgets in self.dynamic_question_checkboxes.items():
            ...
            if always_cb or sometimes_cb:
                current_questions[question_key] = {
                    ...
                    'enabled': (always_cb.isChecked() if always_cb else False) or (sometimes_cb.isChecked() if sometimes_cb else False)
                }
    ...
    def set_question_checkboxes(self, questions):
        ...
        try:
            ...
            for category_key in category_order:
                ...
                category_name = category_name.title() if category_name else category_key.replace('_', ' ').title()
                ...
                for question_key, question_info in questions_by_category[category_key]:
                    ...
                    if not question_data:
                        ...
                        if question_key.startswith('custom_'):
                            ...
                            question_data = {
                                'enabled': question_info.get('enabled_by_default', False),
                                'always_include': False,
                                'sometimes_include': question_info.get('enabled_by_default', False)
                            }
                    ...
                    always_checkbox.setToolTip("Check this to always ask this question in every check-in")
                    always_checkbox.toggled.connect(lambda checked: self._validate_question_counts())
                    always_checkbox.toggled.connect(lambda checked, key=question_key: self._on_always_toggled(key, checked))
                    row_layout.addWidget(always_checkbox)
                    
                    # Sometimes checkbox
                    sometimes_checkbox = QCheckBox("Sometimes")
                    sometimes_checkbox.setToolTip("Check this to sometimes include this question for variety (not asked every check-in)")
                    sometimes_checkbox.toggled.connect(lambda checked: self._validate_question_counts())
                    sometimes_checkbox.toggled.connect(lambda checked, key=question_key: self._on_sometimes_toggled(key, checked))
                    row_layout.addWidget(sometimes_checkbox)
                    
                    # Set initial states
                    always_include = question_data.get('always_include', False)
                    sometimes_include = question_data.get('sometimes_include', False) or (not always_include and question_data.get('enabled', False))
                    
                    always_checkbox.setChecked(always_include)
                    sometimes_checkbox.setChecked(sometimes_include and not always_include)
                    ...
                    if question_key.startswith('custom_'):
                        ...
                        edit_button.clicked.connect(lambda checked=False, key=question_key: self._edit_custom_question(key))
                        row_layout.addWidget(edit_button)
                        
                        delete_button = QPushButton("Delete")
                        delete_button.setMaximumWidth(60)
                        delete_button.clicked.connect(lambda checked=False, key=question_key: self._delete_custom_question(key))
    ...
    def _show_question_dialog(self, question_key=None, question_def=None):
        ...
        from PySide6.QtWidgets import (
            ...
            QPushButton, QDialogButtonBox, QFormLayout, QGroupBox, QTextEdit
        )
        from core.checkin_dynamic_manager import dynamic_checkin_manager
        
        # Create dialog
        dialog = QDialog(self)
        dialog.setWindowTitle("Edit Custom Check-in Question" if question_key else "Add Custom Check-in Question")
        ...
        question_text_hint = QLabel("Use 'you' to match included questions (e.g., 'How are you feeling today?')")
        ...
        categories = dynamic_checkin_manager.get_categories()
        category_keys = list(categories.keys()) if categories else ["mood", "energy", "health", "activities"]
        for cat_key in category_keys:
            cat_info = categories.get(cat_key, {})
            cat_name = cat_info.get('name', cat_key.replace('_', ' ').title())
            cat_name = cat_name.title() if cat_name else cat_key.replace('_', ' ').title()
        ...
        display_name_hint = QLabel("Short name shown in the question list. Type hint will be added automatically.")
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            new_question_def = {
                ...
                'always_include': True if is_new_question else question_def.get('always_include', False),  # Default to always for new questions
                'sometimes_include': False  # Never sometimes for new questions
            }
            
            # Save the custom question
            if dynamic_checkin_manager.save_custom_question(self.user_id, final_key, new_question_def):
                ...
                if self.min_questions_spinbox:
                    min_valid = max(self.min_questions_spinbox.minimum(), min(current_min, self.min_questions_spinbox.maximum()))
                    self.min_questions_spinbox.setValue(min_valid)
                if self.max_questions_spinbox:
                    max_valid = max(self.max_questions_spinbox.minimum(), min(current_max, self.max_questions_spinbox.maximum()))
    ...
    def _edit_custom_question(self, question_key):
        ...
        if question_key not in custom_questions:
            QMessageBox.warning(self, "Question Not Found", f"Custom question '{question_key}' not found.")
            return
    ...
    def _delete_custom_question(self, question_key):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if dynamic_checkin_manager.delete_custom_question(self.user_id, question_key):
                QMessageBox.information(
                    self,
                    "Question Deleted",
    ...
    def get_checkin_settings(self):
        ...
        min_questions = self.min_questions_spinbox.value() if self.min_questions_spinbox else 1
        max_questions = self.max_questions_spinbox.value() if self.max_questions_spinbox else 8
