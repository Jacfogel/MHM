...
from core.tags import get_default_tags, get_user_tags
...
class TagWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        if self.mode == "selection":
            ...
            self.ui.listWidget_tags.setSelectionMode(
                self.ui.listWidget_tags.SelectionMode.SingleSelection
            )
        else:
    ...
    def setup_connections(self):
        ...
        if self.mode == "management":
            ...
            self.ui.listWidget_tags.itemSelectionChanged.connect(
    ...
    def load_tags(self):
        ...
        if not self.user_id:
            self.available_tags = get_default_tags()
        ...
        try:
            self.available_tags = get_user_tags(self.user_id)
            self.refresh_tag_list()
            if self.mode == "management":
                self.update_button_states()
    ...
    def add_tag(self):
        ...
        if tag_text in self.available_tags:
            QMessageBox.warning(
                self, "Duplicate Tag", f"Tag '{tag_text}' already exists."
            )
            return
        ...
        try:
            ...
            if not self.user_id:
                ...
                self.tags_changed.emit()
                logger.info(
                    f"Added tag '{tag_text}' to local list (account creation mode)"
                )
            else:
                ...
                if add_user_task_tag(self.user_id, tag_text):
                    ...
                    QMessageBox.critical(
                        self, "Error", f"Failed to add tag '{tag_text}'."
                    )
        except Exception as e:
    ...
    def edit_tag(self):
        ...
        new_tag, ok = QInputDialog.getText(
            self, "Edit Tag", f"Enter new name for tag '{old_tag}':", text=old_tag
        )
        ...
        if new_tag in self.available_tags:
            QMessageBox.warning(
                self, "Duplicate Tag", f"Tag '{new_tag}' already exists."
            )
            return
        ...
        try:
            ...
            if not self.user_id:
                ...
                self.tags_changed.emit()
                logger.info(
                    f"Edited tag '{old_tag}' to '{new_tag}' in local list (account creation mode)"
                )
            else:
                # Normal mode - save to database
                if remove_user_task_tag(self.user_id, old_tag) and add_user_task_tag(
                    self.user_id, new_tag
                ):
                    # Update local list
                    index = self.available_tags.index(old_tag)
                    self.available_tags[index] = new_tag
                    ...
                    self.tags_changed.emit()
                    logger.info(
                        f"Edited tag '{old_tag}' to '{new_tag}' for user {self.user_id}"
                    )
                else:
                    QMessageBox.critical(
                        self, "Error", f"Failed to edit tag '{old_tag}'."
                    )
        except Exception as e:
            logger.error(
                f"Error editing tag '{old_tag}' to '{new_tag}' for user {self.user_id}: {e}"
            )
    ...
    def delete_tag(self):
        ...
        try:
            ...
            if not self.user_id:
                ...
                self.tags_changed.emit()
                logger.info(
                    f"Deleted tag '{tag_to_delete}' from local list (account creation mode)"
                )
            else:
                # Normal mode - save to database
                if remove_user_task_tag(self.user_id, tag_to_delete):
                    ...
                    self.tags_changed.emit()
                    logger.info(
                        f"Deleted tag '{tag_to_delete}' for user {self.user_id}"
                    )
                else:
                    QMessageBox.critical(
                        self, "Error", f"Failed to delete tag '{tag_to_delete}'."
                    )
        except Exception as e:
            logger.error(
                f"Error deleting tag '{tag_to_delete}' for user {self.user_id}: {e}"
            )
