...
class CategoryManagementDialog(QDialog):
    ...
    def __init__(self, parent=None, user_id=None):
        ...
        try:
            ...
            self.ui.groupBox_enable_automated_messages.toggled.connect(
                self.on_enable_messages_toggled
            )
    ...
    def save_category_settings(self):
        ...
        try:
            ...
            logger.debug(
                f"Category dialog: Got categories from widget: {selected_categories}"
            )
            ...
            if not messages_enabled:
                ...
                other_features = [
                    ...
                    if k != "automated_messages" and v == "enabled"
                ]
            ...
            account = user_data_result.get("account") or {}
            features = account.get("features", {})
            features["automated_messages"] = (
                "enabled" if messages_enabled else "disabled"
            )
            account["features"] = features
            update_user_account(self.user_id, account)
            logger.debug(
                f"Category dialog: Updated account features - automated_messages: {features['automated_messages']}"
            )
            ...
            if not messages_enabled:
                ...
                logger.info(
                    f"Cleared schedule cache for user {self.user_id} after disabling automated messages"
                )

            status_text = "enabled" if messages_enabled else "disabled"
            category_text = (
                f" with {len(selected_categories)} categories"
