...
class TaskManagementDialog(QDialog):
    ...
    def __init__(self, parent=None, user_id=None):
        ...
        self.ui.groupBox_checkBox_enable_task_management.toggled.connect(
            self.on_enable_task_management_toggled
        )
        self.on_enable_task_management_toggled(
            self.ui.groupBox_checkBox_enable_task_management.isChecked()
        )
    ...
    def on_enable_task_management_toggled(self, checked):
        ...
        for child in self.ui.groupBox_checkBox_enable_task_management.findChildren(
            QWidget
        ):
        ...
        if checked and len(self.task_widget.period_widgets) == 0:
            logger.info(
                f"Task management enabled for user {self.user_id} - adding default period"
            )
    ...
    def save_task_settings(self):
        ...
        try:
            ...
            period_names = [
                w.get_period_data().get("name") for w in self.task_widget.period_widgets
            ]
            ...
            logger.info(
                f"Saving task time periods for user {self.user_id}: {time_periods}"
            )
            # When saving periods
            ...
            was_disabled = (
                account.get("features", {}).get("task_management") != "enabled"
            )
            will_be_enabled = tasks_enabled

            account["features"]["task_management"] = (
            ...
            if was_disabled and will_be_enabled:
                logger.info(
                    f"Task management enabled for user {self.user_id} - setting up default tags"
                )
            ...
            QMessageBox.information(
                self, "Task Settings Saved", "Task settings saved successfully."
            )
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to save task settings: {str(e)}"
            )
