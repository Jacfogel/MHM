...
class UserAnalyticsDialog(QDialog):
    ...
    def setup_connections(self):
        ...
        self.ui.comboBox_time_period.currentIndexChanged.connect(self.on_time_period_changed)
    ...
    def setup_initial_state(self):
        ...
        if self.user_id:
            ...
            username = user_data.get('account', {}).get('internal_username', self.user_id)
    ...
    def load_analytics_data(self):
        ...
        try:
            ...
            self.current_days = time_periods[self.ui.comboBox_time_period.currentIndex()]
    ...
    def detect_available_data_types(self):
        ...
        try:
            ...
            data_types_result = self.analytics.get_available_data_types(self.user_id, self.current_days)
            
            if 'error' in data_types_result:
                logger.warning(f"Could not detect data types: {data_types_result['error']}")
                self.available_data_types = {
    ...
    def configure_tab_visibility(self):
        ...
        try:
            ...
            tab_configs = [
                ...
                ("Quantitative Data", self.available_data_types.get("quantitative", False))
            ]
            ...
            if not any(self.available_data_types.values()):
                logger.warning("No specific data types available - showing overview only")
    ...
    def load_overview_data(self):
        ...
        try:
            # Get wellness score
            wellness_data = self.analytics.get_wellness_score(self.user_id, self.current_days)
            ...
            if 'error' in wellness_data:
                ...
                self.ui.label_wellness_score.setText("Wellness Score: No data available")
                self.ui.label_wellness_score.setStyleSheet("color: gray; font-weight: bold;")
...
To see wellness insights, please complete more check-ins."""
                ...
                self.ui.textEdit_recommendations.setPlainText("Complete more check-ins to see wellness insights.")
                return
            ...
            self.ui.label_wellness_score.setText(f"Wellness Score: {score}/100 ({level})")
            self.ui.label_wellness_score.setStyleSheet(f"color: {color}; font-weight: bold;")
            ...
            data_completeness = (total_checkins / self.current_days) * 100 if self.current_days > 0 else 0
    ...
    def load_mood_data(self):
        ...
        try:
            ...
            if 'error' in mood_data:
                self.ui.textEdit_mood_data.setPlainText("No mood data available for analysis.")
                return
...
Recent Mood Data:"""
            ...
            if recent_data:
                ...
                mood_values = [entry.get('mood', 0) for entry in recent_data if isinstance(entry.get('mood'), (int, float))]
                if mood_values:
                    avg_mood = sum(mood_values) / len(mood_values)
                    trend_indicator = "📈" if len(mood_values) > 1 and mood_values[-1] > mood_values[0] else "📉" if len(mood_values) > 1 and mood_values[-1] < mood_values[0] else "➡️"
    ...
    def load_habits_data(self):
        ...
        try:
            habit_data = self.analytics.get_habit_analysis(self.user_id, self.current_days)
            
            if 'error' in habit_data:
                self.ui.textEdit_habits_data.setPlainText("No habit data available for analysis.")
                return
            ...
            completed_habits = sum(stats.get('completed_days', 0) for stats in habits_dict.values())
...
Habit Breakdown:"""
    ...
    def load_sleep_data(self):
        ...
        try:
            sleep_data = self.analytics.get_sleep_analysis(self.user_id, self.current_days)
            
            if 'error' in sleep_data:
                self.ui.textEdit_sleep_data.setPlainText("No sleep data available for analysis.")
                return
            ...
            if avg_hours is not None:
                hours_indicator = "🟢" if avg_hours >= 7 else "🟡" if avg_hours >= 6 else "🔴"
            else:
                hours_indicator = "❓"  # No data available
                
            if avg_quality is not None:
                quality_indicator = "🟢" if avg_quality >= 4 else "🟡" if avg_quality >= 3 else "🔴"
            else:
                quality_indicator = "❓"  # No data available
                
            if consistency is not None:
                consistency_indicator = "🟢" if consistency >= 80 else "🟡" if consistency >= 60 else "🔴"
            else:
                consistency_indicator = "❓"  # No data available
            
            # Format display values
            hours_display = f"{avg_hours:.1f} hours" if avg_hours is not None else "No data"
            quality_display = f"{avg_quality:.1f}/5" if avg_quality is not None else "No data"
            consistency_display = f"{consistency:.1f}%" if consistency is not None else "No data"
...
Recommendations:"""
    ...
    def load_quantitative_data(self):
        ...
        try:
            ...
            summaries = self.analytics.get_quantitative_summaries(self.user_id, self.current_days)
            
            if 'error' in summaries:
                self.ui.textEdit_quantitative_data.setPlainText("No quantitative data available for analysis.")
                return
