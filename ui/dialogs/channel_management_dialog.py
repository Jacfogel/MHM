...
class ChannelManagementDialog(QDialog):
        ...
        try:
            ...
            if self.user_id:
                ...
                def load_user_channel_data():
                    ...
                    self.channel_widget.set_contact_info(
                        email=email, discord_id=discord_id, timezone=timezone
                    )
            ...
            logger.error(
                f"Failed to initialize ChannelManagementDialog: {e}", exc_info=True
            )
            raise

    @handle_errors(
        "handling save button click", default_return=None, user_friendly=True
    )
    def _on_save_clicked(self):
        ...
        if result is False:
            ...
            QMessageBox.critical(
                self, "Error", "Failed to save channel settings. Please try again."
            )
            self.reject()
    ...
    def save_channel_settings(self):
        ...
        account["discord_user_id"] = (
            contact_info["discord_id"] if contact_info["discord_id"] else ""
        )
