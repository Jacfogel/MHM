...
class AccountCreatorDialog(QDialog):
    ...
    def _validate_and_accept__create_account(self, account_data: dict) -> bool:
        """Create the account via the shared provisioning service."""
        from core.admin_account_provisioning import provision_admin_account

        try:
            logger.info("Provisioning new account")
            user_id = provision_admin_account(account_data)
            logger.info(f"provision_admin_account returned: {user_id}")
