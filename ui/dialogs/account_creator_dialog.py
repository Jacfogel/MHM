...
class AccountCreatorDialog(QDialog):
    ...
    def load_task_management_widget(self):
        ...
        self.task_widget.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        self.ui.widget_placeholder_task_settings.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
    ...
    def load_checkin_settings_widget(self):
        ...
        # Make the widget and placeholder expand to fill available space
        self.checkin_widget.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        self.ui.widget_placeholder_checkin_settings.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
    ...
    def setup_profile_button(self):
        ...
        if profile_button:
            profile_button.setToolTip(
                "Click to setup detailed personalization settings"
            )
    ...
    def setup_connections(self):
        ...
        if hasattr(self.ui, "checkBox_enable_task_management"):
            self.ui.checkBox_enable_task_management.toggled.connect(
    ...
    def update_tab_visibility(self):
        ...
        logger.info(
            f"Updating tab visibility - Messages: {messages_enabled}, Tasks: {tasks_enabled}, Check-ins: {checkins_enabled}"
        )
    ...
    def open_personalization_dialog(self):
        ...
        tz = (
            self.ui.comboBox_time_zone.currentText()
            if hasattr(self.ui, "comboBox_time_zone")
            else ""
        )
        if tz:
        ...
        open_personalization_dialog(
            self, temp_user_id, on_personalization_save, self.personalization_data
        )
    ...
    def validate_input(self) -> tuple[bool, str]:
        ...
        logger.info(
            f"Feature enablement - messages: {messages_enabled}, tasks: {tasks_enabled}, checkins: {checkins_enabled}"
        )
        ...
        if messages_enabled:
            ...
            if hasattr(self, "category_widget"):
                ...
                if not selected_categories:
                    logger.warning(
                        "Validation failed: At least one message category must be selected"
                    )
            ...
            if hasattr(self, "channel_widget"):
                selected_service, contact_value = (
                    self.channel_widget.get_selected_channel()
                )
                if not selected_service:
                    logger.warning(
                        "Validation failed: Please select a communication service"
                    )
                    return False, "Please select a communication service."
                if not contact_value:
                    service_name = selected_service.title()
                    logger.warning(
                        f"Validation failed: Please provide contact information for {service_name}"
                    )
                    return (
                        False,
                        f"Please provide contact information for {service_name}.",
                    )
                ...
                if phone and not is_valid_phone(phone):
                    logger.warning(
                        "Validation failed: Please enter a valid phone number (digits only, minimum 10 digits)"
                    )
                    return (
                        False,
                        "Please enter a valid phone number (digits only, minimum 10 digits).",
                    )
                ...
                if discord_id and not is_valid_discord_id(discord_id):
                    logger.warning(
                        "Validation failed: Please enter a valid Discord user ID (17-19 digit number)"
                    )
                    return (
                        False,
                        "Please enter a valid Discord user ID (17-19 digit number).",
                    )
            else:
                logger.warning("Validation failed: Channel widget not loaded")
                return False, "Channel widget not loaded."
    ...
    def _validate_and_accept__collect_channel_data(self) -> tuple[str, dict, dict]:
        ...
        channel_data = {
            "type": selected_service.lower() if selected_service else "discord"
        }
        logger.info(
            f"Channel widget data - service: {selected_service}, contact: {contact_value}"
        )
        ...
        if hasattr(self, "channel_widget"):
            ...
            discord_id = self.channel_widget.ui.lineEdit_discordID.text().strip()
            logger.info(
                f"Contact fields - email: '{email}', phone: '{phone}', discord: '{discord_id}'"
            )
        else:
            email = phone = discord_id = ""

        contact_info = {"email": email, "phone": phone, "discord": discord_id}
    ...
    def _validate_and_accept__collect_widget_data(self) -> tuple[list, dict, dict]:
        ...
        task_settings = (
            self.task_widget.get_task_settings() if hasattr(self, "task_widget") else {}
        )

        logger.info("About to collect checkin widget data")
        checkin_settings = (
            self.checkin_widget.get_checkin_settings()
            if hasattr(self, "checkin_widget")
            else {}
        )
    ...
    def _validate_and_accept__input_errors(self) -> bool:
        ...
        logger.info(
            f"Validation result: valid={is_valid}, error_message='{error_message}'"
        )
    ...
    def _validate_and_accept__collect_data(self) -> dict:
        ...
        timezone, channel_data, contact_info = (
            self._validate_and_accept__collect_channel_data()
        )
    ...
    def create_account(self, account_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            user_preferences = self._validate_and_accept__build_user_preferences(
                account_data
            )
    ...
    def _validate_and_accept__build_user_preferences(
        ...
        chat_id = self._determine_chat_id(
        ...
        self._validate_and_accept__add_feature_settings(
            user_preferences, account_data, features_enabled
        )
    ...
    def _determine_chat_id(
        self, channel_type: str, email: str, phone: str, discord_user_id: str
    ) -> str:
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    internal_username = test_data.get("account", {}).get(
                        "internal_username"
                    )
                    if internal_username:
                        ...
                        time.sleep(
                            0.2
                        )  # Increased from 0.1s to 0.2s for better reliability
                        ...
                        if found_user_id == user_id:
                            ...
                            logger.warning(
                                f"User index not updated correctly for user {user_id} (internal_username: {internal_username}) after {max_retries} attempts"
                            )
                    else:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Error updating user index for user {user_id} after {max_retries} attempts: {retry_error}"
                        )
        except Exception as e:
