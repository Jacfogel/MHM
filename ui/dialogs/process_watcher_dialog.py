...
class ProcessWatcherDialog(QDialog):
    ...
    def setup_all_processes_tab(self):
        ...
        try:
            ...
            self.all_processes_table.itemSelectionChanged.connect(
    ...
    def update_all_processes(self):
        ...
        try:
            ...
            for proc in psutil.process_iter(
                ...
                try:
                    if proc.info["name"] and "python" in proc.info["name"].lower():
                        ...
                        logger.debug(
                            f"Found Python process: PID {proc.info['pid']}, Name: {proc.info['name']}"
                        )
                except (
            ...
            for row, proc_info in enumerate(python_processes):
                ...
                # PID
                self.all_processes_table.setItem(
                    row, 0, QTableWidgetItem(str(proc_info["pid"]))
                )

                # Name
                self.all_processes_table.setItem(
                    row, 1, QTableWidgetItem(proc_info["name"] or "Unknown")
                )
                ...
                # CPU %
                self.all_processes_table.setItem(
                    row, 3, QTableWidgetItem(f"{proc_info['cpu_percent']:.1f}%")
                )

                # Memory %
                self.all_processes_table.setItem(
                    row, 4, QTableWidgetItem(f"{proc_info['memory_percent']:.1f}%")
                )
            ...
            logger.debug(
                f"Updated all processes table with {len(python_processes)} processes"
            )
        except Exception as e:
    ...
    def update_mhm_processes(self):
        ...
        try:
            ...
            for row, proc_info in enumerate(service_processes):
                # PID
                self.mhm_processes_table.setItem(
                    row, 0, QTableWidgetItem(str(proc_info["pid"]))
                )
                ...
                # Memory %
                self.mhm_processes_table.setItem(
                    row, 4, QTableWidgetItem(f"{memory_percent:.1f}%")
                )
            ...
            logger.debug(
                f"Updated MHM processes table with {len(service_processes)} processes"
            )
        except Exception as e:
    ...
    def show_process_details(self, pid):
        ...
        try:
            try:
                ...
                details += f"Create Time: {datetime.fromtimestamp(proc.create_time()).strftime(READABLE_TIMESTAMP_FORMAT)}\n"
    ...
    @handle_errors(
        "closing process watcher dialog", user_friendly=False, default_return=None
    )
