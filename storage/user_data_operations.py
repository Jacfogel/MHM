#!/usr/bin/env python3
"""
User data operations â€” backup, export, indexing, analytics summaries, and admin utilities.
...
class UserDataManager:
    ...
    def _get_user_data_summary__process_message_files(
        ...
        try:
            ...
            self._get_user_data_summary__process_enabled_message_files(
                user_id, categories, summary
            )

            # Process orphaned message files
            self._get_user_data_summary__process_orphaned_message_files(
    ...
    def _get_user_data_summary__process_enabled_message_files(
        ...
        try:
            for category in categories:
                ...
                if os.path.exists(file_path):
                    self._get_user_data_summary__add_message_file_info(
                        file_path, category, summary, orphaned=False
                    )
                else:
                    self._get_user_data_summary__add_missing_message_file_info(
    ...
    def _get_user_data_summary__add_message_file_info(
        ...
        try:
            ...
            if orphaned:
                message_info["orphaned"] = (
                    True  # Mark as orphaned for potential cleanup
                )
    ...
    def _get_last_interaction(self, user_id: str) -> str:
        ...
        try:
            ...
            try:
                ...
                recent_chats = get_recent_responses(
