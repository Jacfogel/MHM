"""
Save pipeline for user data: validate, merge, invariants, write, save_user_data,
save_user_data_transaction, and high-level section updates (formerly ``user_data_updates``).
"""
...
# --- High-level section updates (centralised save_user_data wrappers) ---
...
def _update_user_section(
    ...
    *,
    ...
    result = save_user_data(user_id, {section_key: data}, auto_create=auto_create)
    return result.get(section_key, False)


# not_duplicate: user_data_write_updates_section
...
def update_user_account(
    ...
    if not _validate_user_id_and_dict(user_id, updates, "updates"):
        return False
    return _update_user_section(user_id, "account", updates, auto_create=auto_create)
...
def update_user_context(
    ...
    if not _validate_user_id_and_dict(user_id, updates, "updates"):
        return False
    return _update_user_section(user_id, "context", updates, auto_create=auto_create)
...
def update_channel_preferences(
    ...
    if not _validate_user_id_and_dict(user_id, updates, "updates"):
        return False
    return _update_user_section(user_id, "preferences", updates, auto_create=auto_create)
