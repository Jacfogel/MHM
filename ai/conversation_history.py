...
class ConversationHistory:
    ...
    def end_session(self, user_id: str) -> bool:
        ...
        try:
            if user_id in self._active_sessions:
                ...
                logger.debug(f"Ended conversation session {session.session_id} for user {user_id}")
                return True
    ...
    def get_session_messages(self, user_id: str, session_id: str) -> List[ConversationMessage]:
        ...
        try:
            ...
            logger.error(f"Error getting session messages for user {user_id}, session {session_id}: {e}")
            return []
        ...
        try:
            ...
            for i, session in enumerate(sessions):
                if session.session_id == session_id:
                    ...
                    if user_id in self._active_sessions and self._active_sessions[user_id].session_id == session_id:
                        del self._active_sessions[user_id]
