...
class TestTaskCleanupBugVerification:
    ...
    def test_cleanup_task_reminders_fails_in_real_system(self, test_data_dir):
        ...
        with patch('core.scheduler.SchedulerManager.set_wake_timer'):  # Prevent Windows task creation
    ...
    @pytest.mark.critical
    @pytest.mark.file_io
    def test_schedule_reminders_creates_jobs_but_cleanup_cant_remove_them(self, test_data_dir):
        """
        ...
        with patch('core.scheduler.SchedulerManager.set_wake_timer'):  # Prevent Windows task creation
            schedule_result = schedule_task_reminders(user_id, task_id, reminder_periods)
