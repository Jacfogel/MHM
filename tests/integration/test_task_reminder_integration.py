...
class TestTaskReminderIntegration:
    ...
    def test_task_creation_with_reminders_schedules_reminders(self, test_data_dir):
        ...
        reminder_periods = [
            ...
            {
                "date": (datetime.now() + timedelta(days=2)).strftime(DATE_ONLY_FORMAT),
                "start_time": "14:00",
                "end_time": "15:00",
            },
        ]
        ...
        with patch("core.service.get_scheduler_manager") as mock_get_scheduler:
            ...
            assert (
                mock_scheduler.schedule_task_reminder_at_datetime.call_count == 2
            ...
            assert (
                calls[0][0][2] == reminder_periods[0]["date"]
            ), "First reminder should use first date"
            assert (
                calls[0][0][3] == reminder_periods[0]["start_time"]
            ), "First reminder should use first start_time"
            assert (
                calls[1][0][2] == reminder_periods[1]["date"]
            ), "Second reminder should use second date"
            assert (
                calls[1][0][3] == reminder_periods[1]["start_time"]
            ), "Second reminder should use second start_time"
        ...
        with patch("core.service.get_scheduler_manager") as mock_get_scheduler:
            ...
            with patch(
                ...
                mock_get_scheduler_cleanup.return_value = (
                    mock_scheduler_cleanup_instance
                )
                ...
                mock_scheduler_cleanup_instance.cleanup_task_reminders.assert_called_once_with(
                    user_id, task_id
                )
    ...
    def test_task_update_with_new_reminders_reschedules(self, test_data_dir):
        ...
        with patch("core.service.get_scheduler_manager") as mock_get_scheduler:
            ...
            with patch(
                ...
                mock_scheduler_update_instance.cleanup_task_reminders.assert_called_once_with(
                    user_id, task_id
                )

                # Verify new reminders were scheduled
                assert (
                    mock_scheduler_update_instance.schedule_task_reminder_at_datetime.call_count
                    == 2
                ), "Should schedule 2 new reminders"

    @pytest.mark.integration
    @pytest.mark.tasks
    @pytest.mark.tasks
    @pytest.mark.file_io
    def test_recurring_task_completion_creates_next_instance_with_reminders(
        self, test_data_dir
    ):
        ...
        with patch("core.service.get_scheduler_manager") as mock_get_scheduler:
            ...
            with patch(
                ...
                mock_get_scheduler_complete.return_value = (
                    mock_scheduler_complete_instance
                )
                ...
                assert (
                    len(active_tasks) == 1
                ), "Should have one active task (the next instance)"
                assert (
