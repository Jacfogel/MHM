...
from tasks.task_management import create_task, complete_task, delete_task, get_task_by_id
...
class TestOrphanedReminderCleanup:
    ...
    def test_cleanup_removes_reminders_for_deleted_tasks(self, mock_wake_timer, mock_get_scheduler, test_data_dir):
        ...
        real_scheduler.handle_task_reminder = MagicMock()  # Mock only the reminder handler
        ...
        reminder_jobs = [
            ...
            if hasattr(job.job_func, 'func') and job.job_func.func == scheduler.handle_task_reminder
        ]
        ...
        assert not found_deleted_task_job, "Reminder job for deleted task should be removed"
    ...
    @patch('core.service.get_scheduler_manager')
    @patch('core.scheduler.SchedulerManager.set_wake_timer')
    def test_cleanup_removes_reminders_for_completed_tasks(self, mock_wake_timer, mock_get_scheduler, test_data_dir):
        ...
        assert not found_completed_task_job, "Reminder job for completed task should be removed"
    ...
    @patch('core.service.get_scheduler_manager')
    @patch('core.scheduler.SchedulerManager.set_wake_timer')
    def test_cleanup_preserves_reminders_for_active_tasks(self, mock_wake_timer, mock_get_scheduler, test_data_dir):
        ...
        assert len(reminder_jobs_after) > 0, "Reminder job for active task should be preserved"
        assert len(reminder_jobs_after) == len(reminder_jobs_before), "Should have same number of reminder jobs"
    ...
    def test_cleanup_handles_multiple_users(self, mock_wake_timer, mock_get_scheduler, test_data_dir):
        ...
        TestUserFactory.create_basic_user(user1_id, enable_tasks=True, test_data_dir=test_data_dir)
        TestUserFactory.create_basic_user(user2_id, enable_tasks=True, test_data_dir=test_data_dir)
