...
def test_debug_file_paths(mock_user_data):
    ...
    print(f"User directory: {mock_user_data['user_dir']}")
    print(f"Core config USER_INFO_DIR_PATH: {core.config.USER_INFO_DIR_PATH}")
    
    # Check if user directory exists
    print(f"User directory exists: {os.path.exists(mock_user_data['user_dir'])}")
    ...
    print(f"Account file exists: {os.path.exists(account_file)}")
    
    # Check if the user directory is in the right place
    expected_user_dir = os.path.join(core.config.USER_INFO_DIR_PATH, user_id)
    print(f"Expected user directory: {expected_user_dir}")
    print(f"Expected user directory exists: {os.path.exists(expected_user_dir)}")
    
    # List contents of USER_INFO_DIR_PATH
    if os.path.exists(core.config.USER_INFO_DIR_PATH):
        print(f"Contents of USER_INFO_DIR_PATH: {os.listdir(core.config.USER_INFO_DIR_PATH)}")
    
    # Check if the paths match
    print(f"Paths match: {mock_user_data['user_dir'] == expected_user_dir}")
    ...
    if os.path.exists(account_file):
        try:
            with open(account_file, 'r') as f:
                content = f.read()
                print(f"Account file content length: {len(content)}")
                print(f"Account file content preview: {content[:200]}...")
