...
class AIResponseValidator:
    ...
    def validate_response(
        ...
        assumption_issues = cls._check_inappropriate_assumptions(
            response, prompt, test_type
        )
        ...
        appropriateness_issues = cls._check_response_appropriateness(
            response, prompt, test_type
        )
        ...
        fabricated_issues = cls._check_fabricated_checkin_data(
            response, prompt, context_info
        )
    ...
    def _check_meta_text(cls, response: str) -> list[str]:
        ...
        for pattern in cls.META_TEXT_PATTERNS:
            if re.search(pattern, response, re.IGNORECASE):
                issues.append(
                    f"Meta-text detected: Response contains debugging/meta information (pattern: {pattern})"
                )
    ...
    def _check_code_fragments(cls, response: str) -> list[str]:
        ...
        for pattern in cls.CODE_FRAGMENT_PATTERNS:
            if re.search(pattern, response, re.MULTILINE):
                issues.append(
                    f"Code fragment detected: Response contains code-like content (pattern: {pattern})"
                )
    ...
    def _check_code_fragments_in_command(cls, response: str) -> list[str]:
        ...
        for pattern in python_patterns:
            if re.search(pattern, response, re.MULTILINE):
                issues.append(
                    f"Code fragment in command response: Response contains Python code instead of clean JSON (pattern: {pattern})"
                )
    ...
    def _check_truncation(cls, response: str, prompt: str) -> list[str]:
        ...
        if response and len(response) > 50:  # Only check longer responses
            ...
            if not response_clean.endswith(
                (".", "!", "?", "…", "...")
            ) and not response_clean.endswith(("...", "…")):
                ...
                if last_char.isalnum() or last_char in [",", ";", ":", "-"]:
                    ...
                    if len(response) in truncation_points:
                        issues.append(
                            f"Response appears truncated: ends mid-sentence at {len(response)} chars (common truncation point)"
                        )
                    # Also check if ends mid-word (more obvious truncation)
                        ...
                        last_word = (
                            response_clean.split()[-1] if response_clean.split() else ""
                        )
                        # If last word is suspiciously short or cut off, might be truncated
                        if len(last_word) < 3 and len(response) > 180:
                            issues.append(
                                "Response appears truncated: ends mid-word without proper punctuation"
                            )

        return issues

    @classmethod
    def _check_inappropriate_assumptions(
        cls, response: str, prompt: str, test_type: str
    ) -> list[str]:
        ...
        if "daily task" in response.lower() and "check-in" in response.lower():
            issues.append(
                "Response makes unclear reference to 'daily task in check-in data' - this is ambiguous"
            )

        # Check for assumptions about check-ins when user is new or context is missing
        if test_type == "contextual" and any(
            word in prompt.lower() for word in ["how am i", "how are you", "hello"]
        ):
    ...
    def _check_past_dates(cls, response: str) -> list[str]:
        ...
        from core.time_utilities import now_datetime_full  # canonical helper

        current_year = now_datetime_full().year
            ...
            for match in matches:
                try:
                    ...
                    if year < current_year - 1:
                        issues.append(
                            f"Response contains past date: {match.group(0)} (year {year} is more than 1 year in the past)"
                        )
                    elif year == current_year - 1:
                        # Check if date is definitely in the past (more than 30 days ago)
                        try:
                            date_obj = datetime(year, month, day)
                            days_ago = (now_datetime_full() - date_obj).days
                            if days_ago > 30:
                                issues.append(
                                    f"Response contains past date: {match.group(0)} ({days_ago} days ago)"
                                )
                        except ValueError:
                            pass
    ...
    def _check_response_appropriateness(
        ...
        if len(response) > 250:  # Longer responses might be going off-topic
            ...
            if len(prompt) < 50 and len(response) > 300:
                issues.append(
                    f"Response may be too long for simple prompt ({len(response)} chars for {len(prompt)} char prompt)"
                )
        ...
        if "capabilities" in prompt.lower() or "what can you" in prompt.lower():
            ...
            has_capabilities = any(
                keyword in response.lower() for keyword in capability_keywords
            )
            if not has_capabilities and len(response) > 100:
                issues.append(
                    "Response about capabilities doesn't mention specific capabilities (tasks, messages, check-ins, etc.)"
                )
        ...
        if any(re.search(pattern, prompt.lower()) for pattern in greeting_prompts):
            ...
            has_greeting_response = any(
                indicator in response.lower() for indicator in greeting_indicators
            )
            ...
            redirects = any(
                indicator in response.lower() for indicator in redirect_indicators
            )
            ...
            greeting_positions = [
                ...
                for ind in greeting_indicators
                if ind in response_lower
            ]
            redirect_positions = [
                response_lower.find(ind)
                for ind in redirect_indicators
                if ind in response_lower
            ]
            ...
            if redirects:
                if not has_greeting_response:
                    issues.append(
                        "Response to greeting ('Hello, how are you?') should acknowledge the greeting before redirecting or asking questions"
                    )
                    ...
                    greeting_segment = response_lower[
                        max(0, first_greeting - 5) : first_redirect + 50
                    ]
                    sentence_endings = []
                    for punct in [". ", "! ", "? "]:
                    ...
                    greeting_end = (
                        first_greeting + 35
                    )  # Default: assume greeting phrase is ~35 chars
                    if sentence_endings:
                    ...
                    if first_redirect < greeting_end + 60:
                        ...
                        if greeting_length < 25:  # Greeting acknowledgment is too brief
                            issues.append(
                                "Response to greeting acknowledges it but immediately redirects - should acknowledge greeting more fully before redirecting"
                            )
                        elif (
                            first_redirect < greeting_end + 40
                        ):  # Redirect is very immediate
                            issues.append(
                                "Response to greeting acknowledges it but immediately redirects - should acknowledge greeting more fully before redirecting"
                            )
        ...
        if (
            "tell me something helpful" in prompt.lower()
            or "something helpful" in prompt.lower()
        ):
        ...
        if (
            re.search(r"\bhow are you feeling\??", prompt.lower())
            and "with special characters" not in prompt.lower()
        ):
            ...
            has_feeling_response = any(
                indicator in response.lower() for indicator in feeling_indicators
            )
            # Should not redirect to "How can I help"
            if "how can i help" in response.lower() and not has_feeling_response:
                issues.append(
                    "Response to 'How are you feeling?' should answer the question, not redirect to 'How can I help'"
                )
        ...
        if "tell me about yourself" in prompt.lower():
            ...
            if (
                "what's your name" in response.lower()
                or "your name" in response.lower()
            ):
                issues.append(
                    "Response to 'Tell me about yourself' should describe the AI, not ask for user information (role reversal)"
                )
        ...
        if prompt.lower().strip() in ["hello", "hi", "hey"]:
            ...
            assumes_negative = any(
                word in response.lower() for word in negative_assumptions
            )
            if assumes_negative:
                issues.append(
                    "Response to simple greeting should not assume user is overwhelmed/anxious without context"
                )
        ...
        if re.search(r"\bhow am i (doing|feeling)?\??", prompt.lower()):
            ...
            if has_vague_refs and not has_explicit:
                issues.append(
                    "Response to 'How am I doing?' should ask for information explicitly, not use vague references like 'that' or 'it'"
                )
    ...
    def _check_missing_context_handling(cls, response: str, prompt: str) -> list[str]:
        ...
        has_supportive = any(
            phrase in response.lower() for phrase in supportive_phrases
        )
        ...
        if not has_supportive:
            ...
            if is_too_generic and len(response) < 100:
                issues.append(
                    "Response to missing context should be more supportive and ask for information, not just generic 'how are you feeling'"
                )

        # Vague references are problematic when context is missing
        if has_vague_refs and not has_supportive:
            issues.append(
                "Response uses vague references ('that', 'it') when context is missing - should ask for information explicitly"
            )
    ...
    def _check_role_reversal(cls, response: str, prompt: str) -> list[str]:
        ...
        if any(
            ...
            for pattern in role_reversal_patterns:
                if re.search(pattern, response, re.IGNORECASE):
                    issues.append(
                        "Role reversal detected: Response treats the user as the AI assistant instead of responding as the AI"
                    )
                    break  # Only report once

        # "Tell me about yourself" - should describe AI, not ask for user name/info
        if "tell me about yourself" in prompt.lower():
            if "what's your name" in response.lower() or (
                "your name" in response.lower() and "?" in response
            ):
                issues.append(
                    "Role reversal detected: User asked AI to tell about itself, but AI asks for user's name instead"
                )

        return issues

    @classmethod
    def _check_fabricated_checkin_data(
        cls, response: str, prompt: str, context_info: dict | None = None
    ) -> list[str]:
        ...
        has_checkin_stats = any(
            re.search(pattern, response.lower()) for pattern in checkin_stat_patterns
        )
        ...
        if has_checkin_stats:
            ...
            if context_info:
                ...
                has_checkin_data = context_info.get(
            ...
            if pos_matches:
                for pos_match in pos_matches:
                    ...
                    for neg_pattern in negative_evidence:
                        neg_matches = list(
                            re.finditer(neg_pattern, response_lower[pos_end:])
                        )
                        if neg_matches:
                            # Found contradiction: positive claim followed by negative evidence
                            issues.append(
                                "Self-contradiction detected: Response claims positive outcome but then provides contradictory negative evidence"
                            )
            ...
            if cap_matches:
                for cap_match in cap_matches:
                    ...
                    for inab_pattern in inability_statements:
                        inab_matches = list(
                            re.finditer(inab_pattern, response_lower[cap_end:])
                        )
                        if inab_matches:
                            issues.append(
                                "Self-contradiction detected: Response claims capability but then states inability"
                            )
                            return issues  # Only report once
