...
class AITestRunner:
    ...
    def log_test(
        ...
        if response_time is not None:
            self.performance_metrics.append(
                {"test_id": test_id, "response_time": response_time}
            )
    ...
    def test_basic_response_generation(self):
        ...
        try:
            ...
            if response and isinstance(response, str) and len(response) > 0:
                self.log_test(
                    "T-1.1",
                    "generate_response() with simple prompt",
                    "PASS",
        ...
        try:
            ...
            response = self.chatbot.generate_response(
                prompt, mode="chat", user_id=test_user_id
            )
            if response and isinstance(response, str):
                self.log_test(
                    "T-1.2",
                    "generate_response() in chat mode",
                    "PASS",
                    f"Chat mode response generated: {response[:80]}...",
                    prompt=prompt,
                    response=response[:200],
                )
            else:
        ...
        try:
            ...
            response = self.chatbot.generate_response(
                prompt, mode="command", user_id=test_user_id
            )
            if response and isinstance(response, str):
                self.log_test(
                    "T-1.3",
                    "generate_response() in command mode",
                    "PASS",
                    f"Command mode response generated: {response[:80]}...",
                    prompt=prompt,
                    response=response[:200],
                )
            else:
        ...
        try:
            ...
            self.log_test(
                "T-1.4", "AI availability check", "FAIL", "", f"Exception: {str(e)}"
            )
        ...
        try:
            ...
            response1 = self.chatbot.generate_response(
                prompt, user_id=test_user_id, mode="command"
            )
            time1 = time.time() - start_time

            # Second call - should potentially be cached
            start_time = time.time()
            response2 = self.chatbot.generate_response(
                prompt, user_id=test_user_id, mode="command"
            )
            ...
            if response1 and response2:
                ...
                cache_status = (
                    "working"
                    if cached
                    else "not used (expected for chat mode or AI variation)"
                )
                ...
                self.log_test(
                    ...
                    "Response caching",
                    "FAIL",
                    "Failed to generate responses for cache test",
                    prompt=prompt,
                )
        except Exception as e:
            self.log_test(
    ...
    def test_contextual_response_generation(self):
        ...
        try:
            ...
            if not success:
                self.log_test(
                    "T-2.0",
                    "Create test user for contextual tests",
                    "FAIL",
                    "",
            ...
            if not actual_user_id:
                self.log_test(
                    "T-2.0", "Get user UUID", "FAIL", "", "Could not get user UUID"
                )
                return
            ...
            try:
                ...
                if response and isinstance(response, str) and len(response) > 0:
                    self.log_test(
                        "T-2.1",
                        "generate_contextual_response() with new user",
                        "PASS",
            ...
            try:
                ...
                self.log_test(
                    "T-2.2", "Context building", "FAIL", "", f"Exception: {str(e)}"
                )
            ...
            try:
                ...
                if response and ("TestUser" in response or "test" in response.lower()):
                    self.log_test(
                        "T-2.3",
                        "User name in contextual response",
                        "PASS",
                    ...
                    self.log_test(
                        ...
                        "User name in contextual response",
                        "PARTIAL",
                        f"Name may not be included (response: {response[:80]}...)",
                        prompt=prompt,
                        response=response[:300],
                    )
            except Exception as e:
        ...
        try:
            ...
            if mode == "command":
                ...
                self.log_test(
                    ...
                    "Detect clear command",
                    "PARTIAL",
                    f"Mode detected: {mode} (expected 'command')",
                    prompt=test_input,
                    response=f"Mode: {mode}",
                )
        except Exception as e:
            self.log_test(
                ...
                "",
                f"Exception: {str(e)}",
                prompt=test_input if "test_input" in locals() else "",
            )
        ...
        try:
            ...
            if mode == "command_with_clarification":
                ...
                self.log_test(
                    ...
                    "Detect ambiguous request",
                    "PARTIAL",
                    f"Mode detected: {mode} (expected 'command_with_clarification')",
                    prompt=test_input,
                    response=f"Mode: {mode}",
                )
        except Exception as e:
        ...
        try:
            ...
            if mode == "chat":
                ...
                self.log_test(
                    ...
                    "Detect chat message",
                    "PARTIAL",
                    f"Mode detected: {mode} (expected 'chat')",
                    prompt=test_input,
                    response=f"Mode: {mode}",
                )
        except Exception as e:
            ...
            try:
                ...
                if has_checkins:
                    self.log_test(
                        ...
                        "PASS",
                        "Check-in data found in context",
                        prompt=prompt,
    ...
    def test_command_variations(self):
            ...
            try:
                ...
                if is_command:
                    ...
                    self.log_test(
                        ...
                        f"Command phrasing: '{command[:30]}...'",
                        "PARTIAL",
                        f"Mode: {mode} (expected command variant)",
                        prompt=command,
                        response=f"Mode: {mode}",
                    )
            except Exception as e:
        ...
        try:
            ...
            if has_structure:
                self.log_test(
                    "T-5.6",
                    "Command parsing creates structured output",
                    "PASS",
                    f"Response appears structured: {response[:100]}...",
                    prompt=prompt,
                ...
                self.log_test(
                    ...
                    "Command parsing creates structured output",
                    "PARTIAL",
                    "Response may not be structured JSON",
                    prompt=prompt,
                    response=str(response)[:300],
                )
        except Exception as e:
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            if response:
                self.log_test(
                    "T-6.1",
                    "Invalid user_id (None) handling",
                    "PASS",
                    "Response generated with None user_id",
                    prompt="Hello",
                    response=response[:200],
                )
            else:
        ...
        try:
            ...
            if response and len(response) > 0:
                self.log_test(
                    ...
                    "PASS",
                    "Response generated despite missing context data",
                    prompt=prompt,
                    response=response[:200],
                )
            else:
        ...
        try:
            ...
            if response:
                self.log_test(
                    ...
                    "PASS",
                    "Response generated for empty prompt",
                    prompt="(empty)",
                    response=response[:200],
                )
            else:
    ...
    def test_conversation_history_in_context(self):
        ...
        try:
            ...
            if not success:
                self.log_test(
                    "T-7.0",
                    "Create user for history context tests",
                    "FAIL",
                    "",
                    "Failed to create test user",
                )
            ...
            if not actual_user_id:
                self.log_test(
                    "T-7.0",
                    "Get user UUID for history context tests",
                    "FAIL",
                    "",
                    "Could not get user UUID",
                )
            ...
            try:
                ...
                response1 = self.chatbot.generate_contextual_response(
                    actual_user_id, prompt1
                )

                prompt2 = "What's my favorite color?"
                response2 = self.chatbot.generate_contextual_response(
            ...
            try:
                ...
                has_history = (
                    ...
                    or "history" in str(context).lower()
                )
    ...
    def test_conversation_history(self):
        ...
        try:
            ...
            if not actual_user_id:
                self.log_test(
                    "T-4.0",
                    "Get user UUID for history tests",
                    "FAIL",
                    "",
                    "Could not get user UUID",
                )
            ...
            try:
                ...
                if len(history) >= 2:
                    self.log_test(
                        ...
                        "PASS",
                        f"Stored {len(history)} exchanges",
                        prompt=f"{prompt1} | {prompt2}",
                        response=exchange_details,
                    )
                else:
                    self.log_test(
                        "T-8.1",
                        "Store conversation exchanges",
                        "PARTIAL",
                        f"Stored {len(history)} exchanges (expected 2+)",
                        prompt=f"{prompt1} | {prompt2}",
                        response=exchange_details,
                    )
            except Exception as e:
                self.log_test(
                    ...
                    "",
                    f"Exception: {str(e)}",
                    prompt=prompt1 if "prompt1" in locals() else "",
                )
            ...
            self.log_test(
                "T-8.0",
                "Conversation history storage setup",
                "FAIL",
                "",
                f"Exception during setup: {str(e)}",
            )
        ...
        try:
            ...
            if response and response_time < 10.0:  # Should be fast
                ...
                self.log_test(
                    "T-9.1",
                    "Simple query response time",
                    "FAIL" if response_time >= 10.0 else "PARTIAL",
                    (
    ...
    def test_api_error_scenarios(self):
            ...
            with patch("ai.chatbot.requests.get") as mock_get:
                ...
                if response and len(response) > 0:
                    ...
                    self.log_test(
                        ...
                        "Connection error handling",
                        "FAIL",
                        "No response generated on connection error",
                        prompt=prompt,
                    )
        except Exception as e:
            self.log_test(
        ...
        try:
            with patch("ai.chatbot.requests.post") as mock_post:
                ...
                prompt = "Test timeout"
                response = self.chatbot.generate_response(
                    prompt, user_id=test_user_id, timeout=1
                )
                ...
                if response and len(response) > 0:
                    ...
                    self.log_test(
                        ...
                        "Timeout handling",
                        "FAIL",
                        "No response generated after timeout",
                        prompt=prompt,
                    )
        except Exception as e:
            ...
            if "timeout" in error_msg.lower() or "Time" in error_msg:
                self.log_test(
                    ...
                    "Timeout handling",
                    "PASS",
                    f"Timeout handled: {error_msg[:100]}...",
                    prompt=prompt,
                )
            else:
                self.log_test(
        ...
        try:
            with patch("ai.chatbot.requests.post") as mock_post:
                ...
                if response and len(response) > 0:
                    ...
                    self.log_test(
                        ...
                        "Invalid API response handling",
                        "FAIL",
                        "No response generated for invalid API response",
                        prompt=prompt,
                    )
        except Exception as e:
            ...
            if (
                ...
                self.log_test(
                    ...
                    "Invalid API response handling",
                    "PARTIAL",
                    f"Exception occurred but may be handled: {error_msg[:100]}...",
                    prompt=prompt,
                )
            else:
                self.log_test(
            ...
            with patch("ai.chatbot.requests.post") as mock_post:
                ...
                if response and len(response) > 0:
                    ...
                    self.log_test(
                        ...
                        "Server error (5xx) handling",
                        "FAIL",
                        "No response generated for server error",
                        prompt=prompt,
                    )
        except Exception as e:
            self.log_test(
    ...
    def test_cache_comprehensive(self):
        ...
        try:
            ...
            response1_user1 = self.chatbot.generate_response(
                prompt, user_id=user1_id, mode="command"
            )

            # Generate response for user 2 (might be different if personalized)
            response1_user2 = self.chatbot.generate_response(
                prompt, user_id=user2_id, mode="command"
            )

            # Second call for user 1 should use cache
            start_time = time.time()
            response2_user1 = self.chatbot.generate_response(
                prompt, user_id=user1_id, mode="command"
            )
            ...
            if response1_user1 and response2_user1:
                ...
                self.log_test(
                    ...
                    notes,
                    prompt=prompt,
                    response=f"User1 R1: {response1_user1[:100]}... | User1 R2: {response2_user1[:100]}...",
                    response_time=time_user1,
        ...
        try:
            ...
            start_time = time.time()
            response_chat2 = self.chatbot.generate_response(
                prompt, user_id=test_user_id, mode="chat"
            )
            ...
            chat_varied = (
                response_chat != response_chat2
            )  # Chat should vary (cache skipped)
            ...
            self.log_test(
                ...
                notes,
                prompt=prompt,
                response=f"Command: {response_command[:80]}... | Chat: {response_chat[:80]}...",
                response_time=time_command,
                metrics={"cache_stats": cache_stats},
            )
        except Exception as e:
            self.log_test(
        ...
        try:
            ...
            if cached_immediate == response1:
                ...
                self.log_test(
                    ...
                    "Cache TTL and retrieval",
                    "PARTIAL",
                    "Cache retrieval may not be working as expected",
                    prompt=prompt,
                    metrics={"cache_stats": cache_stats},
                )
        except Exception as e:
    ...
    def test_response_quality(self):
        ...
        try:
            ...
            if response:
                ...
                is_only_whitespace = (
                    response != response_trimmed and len(response_trimmed) == 0
                )

                if (
                    not is_empty
                    and not is_only_whitespace
                    and len(response_trimmed) >= 5
                ):
                    self.log_test(
                        "T-12.1",
        ...
        try:
            ...
            if response:
                ...
                error_found = any(
                    pattern in response.lower() for pattern in error_patterns
                )
                ...
                is_helpful_error = any(
                    phrase in response.lower() for phrase in helpful_errors
                )

                if not error_found or is_helpful_error:
                    self.log_test(
                        "T-12.2",
                        "Response error message validation",
                        "PASS",
        ...
        try:
            ...
            if response:
                ...
                has_multiple_newlines = (
                    response.count("\n\n\n") > 0
                )  # More than double newline
                has_trailing_newlines = response.endswith("\n\n") or response.endswith(
                    "\n\n\n"
                )
                has_leading_whitespace = response.startswith(
                    " "
                ...
                ) or (response.strip().startswith("[") and "]" in response)
    ...
    def test_edge_cases(self):
        ...
        try:
            ...
            if response and len(response) > 0:
                ...
                self.log_test(
                    ...
                    prompt=f"{long_prompt[:50]}... ({len(long_prompt)} chars)",
                    response=response[:200],
                )
            else:
                self.log_test(
                    ...
                    "Very long prompt handling",
                    "FAIL",
                    "No response to long prompt",
                    prompt=f"{long_prompt[:50]}...",
                )
        except Exception as e:
            self.log_test(
                "T-13.1",
        ...
        try:
            ...
            response = self.chatbot.generate_response(
                special_prompt, user_id=test_user_id
            )
            ...
            if response and len(response) > 0:
                ...
                self.log_test(
                    ...
                    "Special characters handling",
                    "FAIL",
                    "No response to special character prompt",
                    prompt=special_prompt,
                )
        except Exception as e:
            self.log_test(
        ...
        try:
            ...
            response = self.chatbot.generate_response(
                unicode_prompt, user_id=test_user_id
            )
            ...
            if "unicode" in error_msg.lower() or "encode" in error_msg.lower():
                self.log_test(
                    ...
                    "Unicode/emoji handling",
                    "PARTIAL",
                    f"Unicode handling may have issues: {error_msg[:100]}...",
                    prompt=unicode_prompt,
                )
            else:
                self.log_test(
        ...
        try:
            ...
            response = self.chatbot.generate_response(
                messy_prompt, user_id=test_user_id
            )
        ...
        try:
            ...
            response = self.chatbot.generate_response(
                numeric_prompt, user_id=test_user_id
            )
            ...
            if response and len(response) > 0:
                ...
                self.log_test(
                    ...
                    "Numeric-only prompt",
                    "PARTIAL",
                    "No response to numeric prompt (may be expected)",
                    prompt=numeric_prompt,
                )
        except Exception as e:
            self.log_test(
            ...
            for r in self.results:
                ...
                if r.get("response"):
                    ...
                    if len(response) > 500:
                        response = (
                            response[:500]
                            + "... (truncated - full response available in test data)"
                        )
