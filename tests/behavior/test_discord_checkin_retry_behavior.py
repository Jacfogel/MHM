...
class TestDiscordCheckinRetryBehavior:
    ...
    def user_id(self, test_data_dir):
        ...
        if actual_user_id:
            prefs = get_user_data(actual_user_id, 'preferences', normalize_on_read=True).get('preferences', {})
    ...
    def test_checkin_message_queued_on_discord_disconnect(self, comm_manager, user_id, test_data_dir):
        ...
        assert queue_size > 0, f"Failed check-in message should be queued for retry, but queue size is {queue_size}"
        
        # Verify queued message details
        queued_messages = []
        while not comm_manager.retry_manager._failed_message_queue.empty():
            queued_messages.append(comm_manager.retry_manager._failed_message_queue.get())
        
        assert len(queued_messages) > 0, "Should have at least one queued message"
        checkin_message = next((m for m in queued_messages if m.category == "checkin"), None)
        assert checkin_message is not None, "Check-in message should be in retry queue"
        assert checkin_message.user_id == actual_user_id, f"Queued message should have correct user_id (expected {actual_user_id}, got {checkin_message.user_id})"
    
    @patch('communication.core.channel_orchestrator.get_component_logger')
    def test_checkin_started_logged_once_after_successful_send(self, mock_get_logger, comm_manager, user_id, test_data_dir):
        ...
        log_calls = [call for call in mock_user_logger.info.call_args_list
                    if len(call[0]) > 0 and "check-in started" in str(call[0][0]).lower()]
        
        assert len(log_calls) == 1, f"Expected exactly 1 'check-in started' log, got {len(log_calls)}: {log_calls}"
        
        # Verify log call has correct parameters
        log_call = log_calls[0]
        assert "check-in started" in str(log_call[0][0]).lower()
        assert log_call[1].get('user_id') == actual_user_id, f"Expected user_id {actual_user_id}, got {log_call[1].get('user_id')}"
        assert log_call[1].get('checkin_type') == "daily"
    
    @patch('communication.core.channel_orchestrator.get_component_logger')
    def test_checkin_started_not_logged_on_failed_send(self, mock_get_logger, comm_manager, user_id, test_data_dir):
        ...
        assert len(log_calls) == 0, f"Expected 0 'check-in started' logs on failure, got {len(log_calls)}"
    
    def test_checkin_retry_after_discord_reconnect(self, comm_manager, user_id, test_data_dir):
        ...
        assert initial_queue_size > 0, f"Message should be queued after failure, but queue size is {initial_queue_size}"
        ...
        if not comm_manager.retry_manager._failed_message_queue.empty():
            ...
            queued.timestamp = datetime.now() - timedelta(seconds=1)  # Make it eligible for retry
        ...
        assert send_call_count[0] > 0 or initial_queue_size > 0, \
            "Retry mechanism should attempt to resend queued messages"
    
    def test_multiple_checkin_attempts_only_log_once(self, comm_manager, user_id, test_data_dir):
        ...
        # Mock logger
        with patch('communication.core.channel_orchestrator.get_component_logger') as mock_get_logger:
            mock_user_logger = Mock()
            mock_get_logger.return_value = mock_user_logger
            ...
            assert len(log_calls) <= 1, f"Expected at most 1 'check-in started' log, got {len(log_calls)}"
