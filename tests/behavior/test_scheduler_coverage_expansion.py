...
class TestSchedulerManagerLifecycle:
    ...
    def test_scheduler_manager_initialization_real_behavior(
        self, mock_communication_manager
    ):
...
class TestMessageScheduling:
    ...
    @pytest.mark.scheduler
    @pytest.mark.critical
    def test_schedule_all_users_immediately_real_behavior(
        self, scheduler_manager, test_data_dir
    ):
        ...
        prefs_data = {"preferences": {"categories": ["motivational", "health"]}}
    ...
    def test_schedule_message_for_period_real_behavior(self, scheduler_manager):
        ...
        with patch.object(
            scheduler_manager, "get_random_time_within_period"
        ) as mock_get_time:
...
class TestTaskReminderScheduling:
        ...
        with patch("tasks.task_management.are_tasks_enabled") as mock_tasks_enabled:
            ...
            with patch("core.scheduler.get_schedule_time_periods") as mock_get_periods:
                ...
                with patch(
                    ...
                    with patch.object(
                        scheduler_manager, "get_random_time_within_task_period"
                    ) as mock_get_time:
    ...
    def test_schedule_task_reminder_at_time_real_behavior(self, scheduler_manager):
        ...
        with patch("tasks.task_management.get_task_by_id") as mock_get_task:
            ...
            with patch.object(scheduler_manager, "set_wake_timer") as mock_wake_timer:
                ...
                result = scheduler_manager.schedule_task_reminder_at_time(
    ...
    def test_get_random_time_within_task_period_real_behavior(self, scheduler_manager):
        ...
        result = scheduler_manager.get_random_time_within_task_period(
            start_time, end_time
        )
...
class TestTimeManagement:
    ...
    def test_get_random_time_within_period_future_scheduling(self, scheduler_manager):
        ...
        with patch("core.scheduler.get_schedule_time_periods") as mock_get_periods:
            ...
            result = scheduler_manager.get_random_time_within_period(
                user_id, category, period_name
            )
        ...
        with patch("core.scheduler.schedule") as mock_schedule:
            ...
            with patch("subprocess.run") as mock_run:
                ...
                mock_run.return_value.stdout = (
                    "TaskName: Wake_test-user_motivational_1000"
                )
...
class TestErrorHandling:
    ...
    @pytest.mark.behavior
    @pytest.mark.scheduler
    def test_handle_sending_scheduled_message_no_communication_manager(
...
class TestSchedulerLoopCoverage:
    ...
    @pytest.mark.scheduler
    @pytest.mark.slow
    def test_scheduler_loop_daily_job_scheduling_real_behavior(
        self, scheduler_manager, test_data_dir
    ):
        ...
        with patch.object(scheduler_manager, "set_wake_timer") as mock_wake_timer:
            ...
            try:
                ...
                assert (
                    scheduler_manager.scheduler_thread is not None
                ), "Scheduler thread should be created"
                assert (
                    scheduler_manager.scheduler_thread.is_alive()
                ), "Scheduler thread should be running"
                ...
                assert (
                    scheduler_manager.scheduler_thread.is_alive()
                ), "Scheduler thread should still be running"
            ...
            if scheduler_manager.scheduler_thread is not None:
                assert (
                    not scheduler_manager.scheduler_thread.is_alive()
    ...
    def test_scheduler_loop_error_handling_real_behavior(self, scheduler_manager):
        ...
        with (
            ...
            assert (
                scheduler_manager.scheduler_thread is None
                or not scheduler_manager.scheduler_thread.is_alive()
            )
...
class TestTaskReminderSchedulingCoverage:
    ...
    @pytest.mark.behavior
    @pytest.mark.scheduler
    def test_schedule_all_task_reminders_with_tasks_real_behavior(
        self, scheduler_manager
    ):
        ...
        with (
            ...
            # Mock task periods
            mock_get_periods.return_value = {
                "morning": {"start_time": "08:00", "end_time": "10:00", "active": True}
            }
    ...
    @pytest.mark.behavior
    @pytest.mark.scheduler
    def test_schedule_all_task_reminders_no_tasks_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    @pytest.mark.scheduler
    def test_schedule_all_task_reminders_missing_times_real_behavior(
        self, scheduler_manager
    ):
...
class TestSelectTaskForReminderBehavior:
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_single_task_real_behavior(
        self, scheduler_manager
    ):
    ...
    def test_select_task_for_reminder_priority_weighting_real_behavior(
        ...
        ), f"Critical ({critical_count}) should be more frequent than medium ({medium_count})"
        assert (
            critical_count > low_count
        ), f"Critical ({critical_count}) should be more frequent than low ({low_count})"
        assert (
            critical_count > none_count
        ), f"Critical ({critical_count}) should be more frequent than none ({none_count})"
        ...
        assert (
            high_count >= low_count - 30
        ), f"High ({high_count}) should be reasonably close to or greater than low ({low_count})"
        assert (
            high_count >= none_count - 30
        ), f"High ({high_count}) should be reasonably close to or greater than none ({none_count})"

    @pytest.mark.behavior
    def test_select_task_for_reminder_due_today_weighting_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_overdue_weighting_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_no_due_date_weighting_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_invalid_date_format_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_large_task_list_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_zero_weights_fallback_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_exception_handling_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_week_proximity_weighting_real_behavior(
        self, scheduler_manager
    ):
    ...
    @pytest.mark.behavior
    def test_select_task_for_reminder_month_proximity_weighting_real_behavior(
        self, scheduler_manager
    ):
        ...
        assert (
            eight_days_count >= thirty_days_count * 0.5
        ), f"8 days ({eight_days_count}) should be reasonably frequent compared to 30 days ({thirty_days_count})"
        assert (
            fifteen_days_count >= thirty_days_count * 0.5
        ), f"15 days ({fifteen_days_count}) should be reasonably frequent compared to 30 days ({thirty_days_count})"
