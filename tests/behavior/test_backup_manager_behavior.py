...
class TestBackupManagerBehavior:
    ...
    def test_backup_rotation_by_age_real_behavior(self):
        ...
        old_time = now_datetime_full() - timedelta(
            days=self.backup_manager.backup_retention_days + 1
        )
    ...
    def test_list_backups_real_behavior(self):
        ...
        original_cleanup_method = (
            self.backup_manager._create_backup__cleanup_old_backups
        )
        try:
            ...
            assert (
                len(backups) == 3
            ), f"Expected 3 backups, found {len(backups)}. All backups: {[b.get('backup_name') for b in self.backup_manager.list_backups()]}"
        finally:
    ...
    def test_validate_backup_with_corrupted_file_real_behavior(self):
        ...
        assert any(
            any(
                ...
                for keyword in ["corrupted", "zip", "failed", "invalid"]
            )
            for error in errors
        )
    ...
    def test_restore_backup_with_config_files_real_behavior(self):
        ...
        # Create backup with config files
        backup_path = self.backup_manager.create_backup(
            "test_config_restore_backup", include_users=True, include_config=True
        )
    ...
    def test_backup_manager_error_handling_real_behavior(self):
        ...
        with patch.object(
            core.config,
            "get_backups_dir",
            return_value="/invalid/path/that/cannot/be/created",
        ):
            backup_manager = BackupManager()
