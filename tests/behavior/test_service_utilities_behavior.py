...
class TestServiceUtilitiesBehavior:
    ...
    def test_throttler_should_run_returns_true_on_first_call(self, test_data_dir):
        ...
        assert (
            throttler.last_run is not None
        ), "last_run should be set on first call to enable throttling"
    ...
    def test_throttler_should_run_respects_interval(self, test_data_dir):
        ...
        assert (
            throttler.last_run is not None
        ), "last_run should be updated after interval"
    ...
    def test_throttler_handles_invalid_timestamp_format(self, test_data_dir):
        ...
        # Arrange
        throttler = Throttler(60)
        setattr(throttler, "last_run", "invalid_timestamp_format")
        ...
        assert (
            throttler.last_run is not None
        ), "Should update last_run with valid timestamp"
    ...
    def test_is_service_running_checks_actual_processes(self, test_data_dir):
        ...
        with patch(
            ...
            return_value=[mock_process_running],
        ):
            # Act
        ...
        with patch(
            "core.service_utilities.psutil.process_iter",
            return_value=[mock_process_not_running],
        ):
            # Act
            is_running = is_service_running()
    ...
    def test_is_service_running_handles_process_errors_gracefully(self, test_data_dir):
        ...
        with patch(
            "core.service_utilities.psutil.process_iter",
            return_value=[mock_process_error],
        ):
    ...
    def test_wait_for_network_returns_false_when_network_unavailable(
        ...
        with patch("core.service_utilities.socket.create_connection") as mock_socket:
            ...
            ), "Should return False when network is unavailable"

    def test_load_and_localize_datetime_creates_timezone_aware_datetime(
        self, test_data_dir
    ):
        ...
        assert (
            format_timestamp(localized_datetime, TIMESTAMP_MINUTE) == "2025-01-15 14:30"
    ...
    def test_load_and_localize_datetime_raises_error_for_invalid_timezone(
        self, test_data_dir
    ):
    ...
    def test_title_case_preserves_mixed_case_words(self, test_data_dir):
        ...
        mixed_case_tests = [
            ...
            (
                "JSON Data",
                "JSON DATA",
            ),  # 'Data' gets converted to 'DATA' because 'data' is in special words
        ]
    ...
    @pytest.mark.slow
    def test_service_utilities_error_handling_preserves_system_stability(
        self, test_data_dir
    ):
...
class TestServiceUtilitiesIntegration:
    ...
    @pytest.mark.slow
    def test_service_utilities_integration_with_reschedule_workflow(
        self, test_data_dir
    ):
    ...
    def test_service_utilities_error_recovery_with_real_operations(self, test_data_dir):
        ...
        with patch("core.service_utilities.socket.create_connection") as mock_socket:
            ...
            assert (
                network_available is True
            ), "Should recover when network becomes available"
    ...
    def test_service_utilities_concurrent_access_safety(self, test_data_dir):
        ...
        assert all(
            result == "AI Chatbot" for result in results
        ), "Title case should be consistent across calls"
