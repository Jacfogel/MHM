...
from communication.message_processing.conversation_flow_manager import conversation_manager, FLOW_NOTE_BODY, FLOW_LIST_ITEMS
...
class TestNotebookHandlerBehavior:
    ...
    def test_notebook_handler_can_handle_intents(self):
        ...
        expected_intents = [
            ...
            'list_recent_entries', 'list_recent_notes', 'show_entry', 
            'append_to_entry', 'set_entry_body',
        ...
        assert not handler.can_handle('create_task'), "NotebookHandler should not handle create_task"
        assert not handler.can_handle('start_checkin'), "NotebookHandler should not handle start_checkin"
    ...
    def test_notebook_handler_get_help(self):
        ...
        assert "note" in help_text.lower() or "list" in help_text.lower(), "Help text should mention notes/lists"
    ...
    def test_create_note_with_tags(self, test_data_dir):
        ...
        if response.completed:
            assert "#work" in response.message or "work" in response.message.lower(), "Should include tags"
        else:
            # Prompting for body is valid when only title provided
            assert "body" in response.message.lower(), "Should prompt for body when only title provided"
    ...
    def test_create_list_with_items(self, test_data_dir):
        ...
        response = handler.handle(user_id, parsed_command)
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, f"Should complete list creation, got: {response.message}"
        assert "created" in response.message.lower() or "shopping" in response.message.lower(), \
        ...
        assert "3" in response.message or "item" in response.message.lower(), "Should mention items"
    ...
    def test_list_recent_entries(self, test_data_dir):
        ...
        assert "recent" in response.message.lower() or "entries" in response.message.lower(), "Should mention recent entries"
        assert "Note 1" in response.message or "Note 2" in response.message, "Should show created notes"
    ...
    def test_show_entry_not_found(self, test_data_dir):
        ...
        assert "not found" in response.message.lower(), "Should indicate entry not found"
    ...
    def test_append_to_entry(self, test_data_dir):
        ...
        assert "appended" in response.message.lower() or "updated" in response.message.lower(), "Should indicate append"
        ...
        assert "Additional text" in updated_entry.body, "Entry body should contain appended text"
    ...
    def test_add_tags_to_entry(self, test_data_dir):
        ...
        from notebook.notebook_data_manager import get_entry
        updated_entry = get_entry(user_id, short_id)
        assert '#work' in updated_entry.tags or 'work' in updated_entry.tags, "Entry should have #work tag"
    ...
    def test_add_list_item(self, test_data_dir):
        ...
        assert any(item.text == 'Bread' for item in updated_entry.items), "List should contain Bread"
    ...
    def test_search_entries(self, test_data_dir):
        ...
        assert "project" in response.message.lower() or "alpha" in response.message.lower(), "Should find matching entry"
    ...
    def test_note_body_flow_completion(self, test_data_dir):
        ...
        reply_text, completed = conversation_manager.handle_inbound_message(user_id, "This is the body text")
        
        assert completed, f"Flow should complete, got: {reply_text}"
        assert "created" in reply_text.lower(), f"Should indicate note was created, got: {reply_text}"
        ...
        assert flow_value == 0 or flow_value is None, f"Flow should be cleared, got flow={flow_value}, user_state={user_state}"
    ...
    def test_list_items_flow_completion(self, test_data_dir):
        ...
        # Add items
        reply_text, completed = conversation_manager.handle_inbound_message(user_id, "Item 1, Item 2")
        assert not completed, "Flow should continue"
        
        # End flow with !end
        reply_text, completed = conversation_manager.handle_inbound_message(user_id, "!end")
        
        assert completed, f"Flow should complete, got: {reply_text}"
        assert "created" in reply_text.lower(), f"Should indicate list was created, got: {reply_text}"
...
class TestNotebookCommandParsing:
    ...
    def test_command_parser_recognizes_note_commands(self, test_data_dir):
        ...
        for pattern in note_patterns:
            ...
            assert "title" in result.parsed_command.entities or len(result.parsed_command.entities) > 0, \
    ...
    def test_command_parser_recognizes_recent_commands(self, test_data_dir):
        ...
        for pattern, expected_intents in recent_patterns:
            result = parser.parse(pattern)
            assert result.parsed_command.intent in expected_intents, \
    ...
    def test_command_parser_recognizes_list_commands(self, test_data_dir):
        ...
        for pattern in list_patterns:
            result = parser.parse(pattern)
            assert result.parsed_command.intent == "create_list", \
                f"Pattern '{pattern}' should match create_list intent, got {result.parsed_command.intent}"
            assert result.confidence > 0.0, f"Pattern '{pattern}' should have confidence > 0"
            assert "title" in result.parsed_command.entities, \
    ...
    def test_command_parser_recognizes_show_entry_commands(self, test_data_dir):
        ...
        for pattern in show_patterns:
            result = parser.parse(pattern)
            assert result.parsed_command.intent == "show_entry", \
                f"Pattern '{pattern}' should match show_entry intent, got {result.parsed_command.intent}"
            assert result.confidence > 0.0, f"Pattern '{pattern}' should have confidence > 0"
            assert "entry_ref" in result.parsed_command.entities, \
    ...
    def test_note_creation_command_variations_end_to_end(self, test_data_dir):
        ...
        for command in command_patterns:
            ...
            assert isinstance(response, InteractionResponse), f"Should return InteractionResponse for '{command}'"
            # Note creation might prompt for body, so completed could be False
            assert "note" in response.message.lower() or "created" in response.message.lower() or "body text" in response.message.lower(), \
    ...
    def test_recent_command_variations(self, test_data_dir):
        ...
        for command, should_complete in command_patterns:
            ...
            if response.completed:
                ...
                is_recent_response = (
                    "recent" in message_lower or "entries" in message_lower or "notes" in message_lower or 
                    "note 1" in message_lower or "note 2" in message_lower or 
                    "no recent" in message_lower or "no entries" in message_lower
                )
                is_generic_greeting = (
                    "hello" in message_lower or "how are you" in message_lower or 
                    "support" in message_lower or "encouragement" in message_lower
                )
                # Accept either recent response OR generic greeting (command not recognized)
                assert is_recent_response or is_generic_greeting, \
    ...
    def test_list_command_variations(self, test_data_dir):
        ...
        for command in command_patterns:
            ...
            assert "list" in response.message.lower() or "items" in response.message.lower(), \
    ...
    def test_end_command_in_list_flow_end_to_end(self, test_data_dir):
        ...
        assert not response.completed, f"Should prompt for items, got: {response.message}"
        assert "items" in response.message.lower() or "list" in response.message.lower(), \
        ...
        assert "item" in response.message.lower() or "total" in response.message.lower(), \
        ...
        user_state = conversation_manager.user_states.get(user_id, {})
        flow_value = user_state.get('flow') if user_state else 0
        assert flow_value == 0 or flow_value is None, f"Flow should be cleared after !end, got flow={flow_value}, user_state={user_state}"
        ...
        assert any(e.kind == 'list' and 'test list' in (e.title or '').lower() for e in entries), \
...
class TestNotebookEntityExtraction:
    ...
    def test_extract_limit_from_recent_command(self, test_data_dir):
        ...
        assert result.parsed_command.intent in ["list_recent_entries", "list_recent_notes"], "Should match recent intent"
        # Limit extraction may vary - check if present
        entities = result.parsed_command.entities
        if "limit" in entities:
            assert isinstance(entities["limit"], int) or entities["limit"] == "10", "Should extract limit as int or string"
    ...
    def test_handle_missing_entities_gracefully(self, test_data_dir):
        ...
        assert not response.completed or "title" in response.message.lower(), "Should prompt for title or handle gracefully"
...
class TestNotebookFlowStateEdgeCases:
    ...
    def test_skip_note_body_flow(self, test_data_dir):
        ...
        # Skip the flow
        reply_text, completed = conversation_manager.handle_inbound_message(user_id, "skip")
    ...
    def test_cancel_note_body_flow(self, test_data_dir):
        ...
        assert completed, f"Flow should complete, got: {reply_text}"
        assert "cancel" in reply_text.lower(), f"Should indicate cancellation, got: {reply_text}"
        assert "not saved" in reply_text.lower() or "cancelled" in reply_text.lower(), \
        ...
        from notebook.notebook_data_manager import list_recent
        entries = list_recent(user_id, n=5)
        assert not any(e.title == 'Cancel Test' for e in entries), "Note should not be created"
    ...
    def test_interrupt_note_flow_with_command(self, test_data_dir):
        ...
        assert flow_value == 0 or flow_value == FLOW_LIST_ITEMS, "Flow should be cleared or replaced"
    ...
    def test_multiple_items_in_list_flow(self, test_data_dir):
        ...
        # Add second batch
        reply_text, completed = conversation_manager.handle_inbound_message(user_id, "Item 3, Item 4")
        assert not completed, "Flow should continue"
        ...
        entries = list_recent(user_id, n=5)
        list_entry = next((e for e in entries if e.kind == 'list' and 'multi items' in (e.title or '').lower()), None)
        assert list_entry is not None, "List should be created"
        assert len(list_entry.items) >= 4, f"Should have at least 4 items, got {len(list_entry.items)}"
    ...
    def test_list_flow_with_empty_items(self, test_data_dir):
        ...
        from notebook.notebook_data_manager import list_recent
        entries = list_recent(user_id, n=5)
        assert any(e.kind == 'list' and 'empty list' in (e.title or '').lower() for e in entries), \
    ...
    def test_flow_timeout_simulation(self, test_data_dir):
        ...
        user_state = conversation_manager.user_states.get(user_id, {})
        user_state['started_at'] = (datetime.now() - timedelta(minutes=11)).isoformat()
        ...
        reply_text, completed = conversation_manager.handle_inbound_message(user_id, "Body text")
        
        assert completed, f"Flow should complete (expired), got: {reply_text}"
        assert "expired" in reply_text.lower() or "timeout" in reply_text.lower() or "start over" in reply_text.lower(), \
...
class TestNotebookErrorHandling:
    ...
    def test_append_to_nonexistent_entry(self, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Should complete (with error)"
        assert "not found" in response.message.lower() or "error" in response.message.lower(), \
    ...
    def test_toggle_invalid_item_index(self, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Should complete (with error)"
        assert "invalid" in response.message.lower() or "error" in response.message.lower() or \
