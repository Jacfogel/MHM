...
class TestDiscordBotBehavior:
    ...
    def test_discord_task_create_update_complete(self, test_data_dir):
        ...
        # Retry loading tasks in case of race conditions or caching issues
        import time
        task = None
        ...
        assert task is not None, f"Task should exist after update. Found {len(tasks)} tasks: {[t.get('title') for t in tasks]}"
    ...
    def test_discord_complete_task_by_name_variation(self, test_data_dir):
        ...
        for _attempt in range(15):
            clear_user_caches()
            with (
                patch.object(core.config, "BASE_DATA_DIR", str(test_data_dir)),
                patch.object(
                    core.config,
                    "USER_INFO_DIR_PATH",
                    os.path.join(str(test_data_dir), "users"),
                ),
            ):
...
class TestDiscordBotIntegration:
    ...
    def test_discord_bot_integration_with_user_management(self, test_data_dir, test_user_setup):
        ...
        from core import get_all_user_ids, get_user_id_by_identifier
