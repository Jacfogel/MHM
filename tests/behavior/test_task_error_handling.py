...
from tasks.task_management import (
    ...
    load_active_tasks, get_task_by_id, schedule_task_reminders
)
...
class TestTaskErrorHandling:
    ...
    def test_create_task_invalid_priority(self, test_data_dir):
        ...
        task_id = create_task(user_id=user_id, title="Test Task", priority="invalid_priority")
    ...
    def test_create_task_invalid_date_format(self, test_data_dir):
        ...
        task_id = create_task(user_id=user_id, title="Test Task", due_date="invalid-date")
    ...
    def test_schedule_reminders_incomplete_period_data(self, test_data_dir):
        ...
        incomplete_periods = [
            {'date': (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d')},  # Missing start_time, end_time
            {'start_time': '09:00', 'end_time': '10:00'},  # Missing date
            {'date': (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d'), 'start_time': '09:00'}  # Missing end_time
        ]
    ...
    def test_handler_invalid_intent(self):
        ...
        assert isinstance(response, type(handler.handle("test_user", parsed_command))), "Should return InteractionResponse"
...
class TestTaskEdgeCases:
    ...
    def test_task_with_no_due_date_but_reminders(self, test_data_dir):
        ...
        assert task_id is not None, "Task should be created"
        task = get_task_by_id(user_id, task_id)
        assert task['due_date'] is None or task['due_date'] == "", "Task should have no due_date"
    ...
    def test_task_with_unicode_characters(self, test_data_dir):
        ...
        task = get_task_by_id(user_id, task_id)
        assert task['title'] == unicode_title, "Unicode title should be stored correctly"
    ...
    def test_task_with_special_characters_in_description(self, test_data_dir):
        ...
        assert task_id is not None, "Task should be created"
        task = get_task_by_id(user_id, task_id)
        assert task['description'] == special_desc, "Special characters should be stored"
    ...
    def test_multiple_tasks_same_title(self, test_data_dir):
        ...
        assert all(t['title'] == title for t in tasks), "Both tasks should have same title"
