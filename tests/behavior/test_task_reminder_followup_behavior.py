...
class TestTaskReminderFollowupBehavior:
    ...
    def test_task_creation_starts_reminder_followup_flow(self, test_data_dir):
        ...
        assert user_id in conversation_manager.user_states, "User should be in a flow state"
        assert conversation_manager.user_states[user_id]['flow'] == FLOW_TASK_REMINDER, "Should be in TASK_REMINDER flow"
        assert 'task_id' in conversation_manager.user_states[user_id]['data'], "Should store task_id in flow data"
    ...
    def test_reminder_followup_handles_no_reminders_response(self, test_data_dir):
        ...
        assert user_id not in conversation_manager.user_states, "Flow state should be cleared"
        assert "no reminders" in reply.lower() or "got it" in reply.lower(), "Should acknowledge no reminders"
    ...
    def test_reminder_followup_parses_minutes_before(self, mock_get_scheduler, test_data_dir):
        ...
        task = get_task_by_id(user_id, task_id)
        assert task is not None, "Task should exist"
        assert 'reminder_periods' in task, f"Task should have reminder_periods. Task: {task}"
        assert len(task['reminder_periods']) > 0, "Should have at least one reminder period"
        ...
        assert 12 <= start_hour <= 13, f"Start time should be around 1 PM, got {reminder['start_time']}"
    
    @pytest.mark.behavior
    @pytest.mark.communication
    @pytest.mark.tasks
    @patch('core.service.get_scheduler_manager')
    def test_reminder_followup_parses_hours_before(self, mock_get_scheduler, test_data_dir):
        ...
        start_hour = int(reminder['start_time'].split(':')[0])
        assert 9 <= start_hour <= 12, f"Start time should be around 10 AM-12 PM, got {reminder['start_time']}"
    
    @pytest.mark.behavior
    @pytest.mark.communication
    @pytest.mark.tasks
    @patch('core.service.get_scheduler_manager')
    def test_reminder_followup_parses_days_before(self, mock_get_scheduler, test_data_dir):
        ...
        days_before = (due_datetime - reminder_date).days
        assert 1 <= days_before <= 2, f"Reminder should be 1-2 days before, got {days_before} days"
    
    @pytest.mark.behavior
    @pytest.mark.communication
    @pytest.mark.tasks
    @patch('core.service.get_scheduler_manager')
    def test_reminder_followup_handles_task_without_due_date(self, mock_get_scheduler, test_data_dir):
    ...
    @pytest.mark.tasks
    @patch('core.service.get_scheduler_manager')
    def test_reminder_followup_handles_unparseable_response(self, mock_get_scheduler, test_data_dir):
        ...
        assert user_id in conversation_manager.user_states, "Flow state should still be active"
        assert "not sure" in reply.lower() or "specify" in reply.lower(), "Should ask for clarification"
    ...
    def test_reminder_followup_schedules_reminders(self, mock_wake_timer, mock_get_scheduler, test_data_dir):
