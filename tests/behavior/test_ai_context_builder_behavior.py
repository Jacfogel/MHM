...
class TestContextBuilderBehavior:
    ...
    def test_build_user_context_creates_structured_context_data(self, test_data_dir):
        ...
        ), "Context should include recent_checkins"
        assert hasattr(
            context, "conversation_history"
        ), "Context should include conversation_history"
    ...
    def test_build_user_context_handles_missing_user_data_gracefully(
        self, test_data_dir
    ):
    ...
    def test_build_user_context_includes_recent_checkins(self, test_data_dir):
        ...
        assert isinstance(
            context.recent_checkins, list
        ), "Recent checkins should be included"
    ...
    def test_build_user_context_without_conversation_history(self, test_data_dir):
        ...
        assert isinstance(
            context.conversation_history, list
        ), "Conversation history should still be a list"
    ...
    def test_build_user_context_creates_fresh_timestamp(self, test_data_dir):
        ...
        # Act
        # build_user_context now uses canonical time_utilities helpers which are second-precision.
        # To make this test deterministic (and avoid flaky same-second comparisons), patch the
        # helper used by ContextBuilder to produce two distinct timestamps.
        from unittest.mock import patch

        dt1 = datetime(2026, 1, 20, 2, 23, 47)
        ...
        assert (
            context1.current_time != context2.current_time
        ), "Should create fresh timestamps"
    ...
    def test_build_user_context_handles_large_user_data(self, test_data_dir):
        ...
        ), "Context should have user_profile attribute"
        assert hasattr(
            context, "user_context"
        ), "Context should have user_context attribute"
        assert hasattr(
            context, "recent_checkins"
        ), "Context should have recent_checkins attribute"
