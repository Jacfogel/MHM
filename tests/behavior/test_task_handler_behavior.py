...
class TestTaskHandlerBehavior:
    ...
    def test_task_handler_create_task_success(self, mock_create_task, test_data_dir):
        ...
        assert call_kwargs.get('user_id') == user_id, "Should create task for correct user"
        assert call_kwargs.get('title') == 'Test Task', "Should create task with correct title"
        assert call_kwargs.get('priority') == 'high', "Should create task with correct priority"
        assert call_kwargs.get('due_date') is not None, "Should parse and include due_date"
        # Verify due_date was parsed from 'tomorrow' to actual date
        from datetime import datetime, timedelta
        expected_date = (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d')
        assert call_kwargs.get('due_date') == expected_date, f"Should parse 'tomorrow' to {expected_date}"
    ...
    @pytest.mark.file_io
    @patch('communication.command_handlers.task_handler.create_task')
    def test_task_handler_create_task_with_recurrence(self, mock_create_task, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert "created" in response.message.lower(), "Should indicate task was created"
        assert "repeats" in response.message.lower() or "repeat" in response.message.lower(), "Should mention recurrence"
        assert mock_create_task.called, "Should call create_task"
        call_kwargs = mock_create_task.call_args[1] if mock_create_task.call_args else {}
        assert call_kwargs.get('recurrence_pattern') == 'daily', "Should set recurrence pattern"
    ...
    def test_task_handler_parse_relative_date_tomorrow(self):
        ...
        assert result == expected, f"Should parse 'tomorrow' as {expected}, got {result}"
    ...
    def test_task_handler_parse_relative_date_next_week(self):
        ...
        assert result == expected, f"Should parse 'next week' as {expected}, got {result}"
    ...
    def test_task_handler_parse_relative_date_absolute_date(self):
        ...
        assert result == absolute_date, f"Should pass through absolute date {absolute_date}, got {result}"
    ...
    def test_task_handler_list_tasks_success(self, mock_load_tasks, test_data_dir):
        ...
        mock_load_tasks.return_value = [
            {'title': 'Task 1', 'priority': 'high', 'due_date': '2024-12-20', 'task_id': 'task_1'},
            {'title': 'Task 2', 'priority': 'medium', 'due_date': None, 'task_id': 'task_2'}
        ]
        ...
        assert "Task 1" in response.message or "Task 2" in response.message, "Should include task titles"
    ...
    def test_task_handler_list_tasks_no_tasks(self, mock_load_tasks, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "no" in response.message.lower() or "empty" in response.message.lower() or "great job" in response.message.lower(), "Should indicate no tasks"
    ...
    @patch('communication.command_handlers.task_handler.load_active_tasks')
    def test_task_handler_list_tasks_filter_due_soon(self, mock_load_tasks, mock_due_soon, test_data_dir):
        ...
        mock_due_soon.return_value = [
            {'title': 'Due Task', 'priority': 'medium', 'due_date': '2024-12-20', 'task_id': 'task_2'}
        ]
    ...
    @patch('communication.command_handlers.task_handler.load_active_tasks')
    def test_task_handler_complete_task_success(self, mock_load_tasks, mock_complete_task, test_data_dir):
    ...
    def test_task_handler_complete_task_missing_identifier(self):
        ...
        assert "which" in response.message.lower() or "specify" in response.message.lower(), "Should ask for task identifier"
    ...
    @patch('communication.command_handlers.task_handler.load_active_tasks')
    def test_task_handler_delete_task_success(self, mock_load_tasks, mock_delete_task, test_data_dir):
    ...
    @patch('communication.command_handlers.task_handler.load_active_tasks')
    def test_task_handler_update_task_success(self, mock_load_tasks, mock_update_task, test_data_dir):
        ...
        call_task_id = call_args[0][1] if len(call_args[0]) > 1 else call_args[1].get('task_id')
        updates = call_args[0][2] if len(call_args[0]) > 2 else call_args[1].get('updates', {})
    ...
    @pytest.mark.file_io
    @patch('communication.command_handlers.task_handler.load_active_tasks')
    def test_task_handler_update_task_missing_updates(self, mock_load_tasks, test_data_dir):
    ...
    def test_task_handler_task_stats_success(self, mock_get_stats, mock_analytics_class, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "statistics" in response.message.lower() or "stats" in response.message.lower(), "Should mention statistics"
        # Check for task stats content (task names, completion rates, or overall stats)
        assert "task" in response.message.lower() or "habit" in response.message.lower() or "progress" in response.message.lower(), "Should include task stats"
        mock_get_stats.assert_called_once_with(user_id)
        mock_analytics_instance.get_task_weekly_stats.assert_called_once_with(user_id, 7)
    ...
    def test_task_handler_find_task_by_name(self):
        ...
        task = handler._handle_complete_task__find_task_by_identifier(tasks, 'Brush Teeth')
    ...
    def test_task_handler_sort_tasks_by_priority(self):
        ...
        tasks = [
            ...
            {'title': 'Medium Priority', 'priority': 'medium', 'due_date': '2024-12-22'}
        ]
        
        sorted_tasks = handler._handle_list_tasks__sort_tasks(tasks)
        assert sorted_tasks[0]['priority'] == 'high', "High priority tasks should come first"
        assert sorted_tasks[1]['priority'] == 'medium', "Medium priority tasks should come second"
        assert sorted_tasks[2]['priority'] == 'low', "Low priority tasks should come last"
