...
class TestScheduleHandlerBehavior:
    ...
    def test_schedule_handler_can_handle_intents(self):
        ...
        for intent in expected_intents:
            assert handler.can_handle(
                intent
            ), f"ScheduleManagementHandler should handle {intent}"

        # Test that it doesn't handle other intents
        assert not handler.can_handle(
            "create_task"
        ), "ScheduleManagementHandler should not handle create_task"
        assert not handler.can_handle(
            "start_checkin"
        ), "ScheduleManagementHandler should not handle start_checkin"
    ...
    def test_schedule_handler_show_schedule_all_categories(
        ...
        assert (
            response.rich_data is not None
        ), "Should include rich data for Discord embeds"
        assert (
            response.rich_data.get("type") == "schedule"
        ), "Rich data should have type 'schedule'"
    ...
    def test_schedule_handler_show_schedule_specific_category(
        ...
        assert (
            "morning" in response.message.lower() or "09:00" in response.message.lower()
        ), "Should include period details"
    ...
    def test_schedule_handler_show_schedule_no_periods(
        ...
        assert response.completed, "Response should be completed"
        assert (
            "no schedule" in response.message.lower()
            or "no periods" in response.message.lower()
            or "configured" in response.message.lower()
        ), "Should indicate no periods"
    ...
    def test_schedule_handler_update_schedule_enable(
        ...
        assert response.completed, "Response should be completed"
        assert (
            "enabled" in response.message.lower()
        ), "Should indicate schedule was enabled"
        ...
        call_category = (
            call_args[0][1] if len(call_args[0]) > 1 else call_args[1].get("category")
        )
        periods = (
            call_args[0][2] if len(call_args[0]) > 2 else call_args[1].get("periods")
        )
        ...
        for period_name, period_data in periods.items():
            assert (
                period_data.get("active") is True
            ), f"Period {period_name} should be enabled (active=True)"
    ...
    def test_schedule_handler_update_schedule_disable(
        ...
        ), "Should indicate schedule was disabled"
        ...
        for period_name, period_data in periods.items():
            assert (
                period_data.get("active") is False
            ), f"Period {period_name} should be disabled (active=False)"
    ...
    def test_schedule_handler_update_schedule_missing_params(self, test_data_dir):
        ...
        assert (
            "specify" in response.message.lower() or "try" in response.message.lower()
    ...
    def test_schedule_handler_schedule_status(
        ...
        assert (
            "task" in response.message.lower() or "checkin" in response.message.lower()
    ...
    def test_schedule_handler_add_schedule_period_success(
        ...
        assert response.completed, "Response should be completed"
        assert (
            "added" in response.message.lower()
            or "added new" in response.message.lower()
        ), "Should indicate period was added"
        assert "morning" in response.message.lower(), "Should mention period name"
        assert (
            response.suggestions is not None
        ), "Should include suggestions for next actions"
        ...
        assert (
            morning_period.get("start_time") == "09:00 AM"
        ), "Should set correct start_time"
        assert (
            morning_period.get("end_time") == "11:00 AM"
        ), "Should set correct end_time"
        assert morning_period.get("active") is True, "Should set active=True"
        assert "days" in morning_period, "Should include days"
        assert set(morning_period["days"]) == set(
            ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]
        ), "Should set correct days"
    ...
    def test_schedule_handler_add_schedule_period_missing_params(self, test_data_dir):
        ...
        ), "Response should not be completed (needs clarification)"
        assert (
            "details" in response.message.lower() or "try" in response.message.lower()
        ), "Should ask for more details"
    ...
    def test_schedule_handler_parse_time_format_12hour(self):
        ...
        for input_time, expected in test_cases:
            result = handler._normalize_time_string_to_12h(input_time)
            ...
            assert (
                result_match and expected_match
            ), f"Could not parse time format: got {result}, expected {expected}"
            ...
            assert (
                result_hour == expected_hour
                and result_match.group(2) == expected_match.group(2)
                and result_match.group(3) == expected_match.group(3)
            ), f"Should parse {input_time} correctly (got {result}, expected {expected})"
    ...
    def test_schedule_handler_parse_time_format_24hour(self):
        ...
        for input_time, expected in test_cases:
            result = handler._normalize_time_string_to_12h(input_time)
            assert (
                result == expected or "AM" in result or "PM" in result
            ), f"Should convert {input_time} to 12-hour format"
    ...
    def test_schedule_handler_parse_time_format_hour_only(self):
        ...
        for input_hour in test_cases:
            result = handler._normalize_time_string_to_12h(str(input_hour))
            assert result is not None, f"Should parse hour {input_hour}"
            assert (
                "AM" in result or "PM" in result
            ), f"Should convert hour {input_hour} to 12-hour format"
    ...
    def test_schedule_handler_edit_schedule_period_success(
        ...
        assert response.completed, "Response should be completed"
        assert (
            "updated" in response.message.lower()
        ), "Should indicate period was updated"
        assert "morning" in response.message.lower(), "Should mention period name"
        assert response.suggestions is not None, "Should include suggestions"
        ...
        morning_period = periods["morning"]
        assert (
            morning_period.get("start_time") == "08:00 AM"
        ), "Should update start_time to 08:00 AM"
        assert (
            morning_period.get("end_time") == "10:00 AM"
        ), "Should update end_time to 10:00 AM"
    ...
    def test_schedule_handler_edit_schedule_period_not_found(
        ...
        parsed_command = ParsedCommand(
            intent="edit_schedule_period",
            entities={"category": "task", "period_name": "nonexistent"},
            confidence=0.9,
        ...
        assert (
            not response.completed
        ), "Response should not be completed (interactive flow with suggestions)"
        assert (
            "not found" in response.message.lower()
            or "what times should" in response.message.lower()
        ), "Should indicate period not found or prompt for times"
        assert (
            response.suggestions is not None and len(response.suggestions) > 0
        ), "Should provide suggestions for times"
    ...
    def test_schedule_handler_edit_schedule_period_no_changes(
        ...
        assert (
            "no changes" in response.message.lower()
            or "what would you like to update" in response.message.lower()
            or "current" in response.message.lower()
        ), "Should indicate no changes specified or prompt for updates"
        # When no changes are specified, should provide suggestions for what can be updated
        assert (
            response.suggestions is not None and len(response.suggestions) > 0
        ), "Should provide suggestions for updates"
    ...
    def test_schedule_handler_unknown_intent(self, test_data_dir):
        ...
        with patch.object(handler, "can_handle", return_value=True):
            ...
            ), "Should indicate command not understood"
