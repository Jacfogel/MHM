...
class TestComprehensiveQuantitativeAnalytics:
    ...
    def test_all_quantitative_questions_included(
        ...
        assert (
            "daily_reflection" not in summaries
        ), "Non-quantitative fields should not be included"
        ...
        assert (
            abs(summaries["sleep_schedule"]["average"] - 6.75) < 0.1
        ), f"Sleep schedule average should be ~6.75, got {summaries['sleep_schedule']['average']}"
        assert (
            abs(summaries["sleep_schedule"]["min"] - 6.0) < 0.1
        ), f"Sleep schedule min should be 6.0, got {summaries['sleep_schedule']['min']}"
        assert (
            abs(summaries["sleep_schedule"]["max"] - 7.5) < 0.1
        ), f"Sleep schedule max should be 7.5, got {summaries['sleep_schedule']['max']}"
        ...
        assert (
            summaries["exercise"]["average"] == 0.5
        ), "Exercise average should be 0.5 (1 yes, 1 no)"
    ...
    @pytest.mark.analytics
    @pytest.mark.file_io
    def test_yes_no_questions_converted_correctly(
        self, test_data_dir, fix_user_data_loaders
    ):
        ...
        assert (
            summaries["ate_breakfast"]["average"] == 1.0
        ), "Breakfast should average 1.0 (all yes)"
        assert summaries["ate_breakfast"]["min"] == 1.0, "Breakfast min should be 1.0"
        assert summaries["ate_breakfast"]["max"] == 1.0, "Breakfast max should be 1.0"

        # exercise: no, NO, 0 -> 0, 0, 0 -> average 0.0
        assert (
            summaries["exercise"]["average"] == 0.0
        ), "Exercise should average 0.0 (all no)"
        assert summaries["exercise"]["min"] == 0.0, "Exercise min should be 0.0"
        assert summaries["exercise"]["max"] == 0.0, "Exercise max should be 0.0"

        # medication_taken: y, true, Yes -> 1, 1, 1 -> average 1.0
        assert (
            summaries["medication_taken"]["average"] == 1.0
        ), "Medication should average 1.0 (all yes)"

        # hydration: n, false, No -> 0, 0, 0 -> average 0.0
        assert (
            summaries["hydration"]["average"] == 0.0
        ), "Hydration should average 0.0 (all no)"
