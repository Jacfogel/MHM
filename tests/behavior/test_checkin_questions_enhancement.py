...
class TestNewCheckinQuestions:
    ...
    def test_new_questions_validation(self):
        ...
        is_valid, value, error = dynamic_checkin_manager.validate_answer(
            "treatment_adherence", "yes"
        )
        assert is_valid is True
        assert value is True

        is_valid, value, error = dynamic_checkin_manager.validate_answer(
            "treatment_adherence", "no"
        )
        assert is_valid is True
    ...
    def test_new_questions_responses(self):
        ...
        response = dynamic_checkin_manager.get_response_statement(
...
class TestAnalyticsWithNewQuestions:
    ...
    def test_analytics_with_sleep_schedule(self, monkeypatch):
        ...
        checkins_file = os.path.join(
            self.test_data_dir, "users", self.user_id, "checkins.json"
        )
    ...
    def test_quantitative_summaries_includes_new_questions(self, monkeypatch):
        ...
        with patch(
            ...
            assert len(checkins) > 0, "Should have at least one check-in"
            assert (
                "hopelessness_level" in checkins[0] or "mood" in checkins[0]
            ), "Check-in should contain data"
