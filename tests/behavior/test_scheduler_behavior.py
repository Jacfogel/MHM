...
from core.service_utilities import TIME_HM_FORMAT, READABLE_MINUTE_TIMESTAMP_FORMAT
...
class TestSchedulerEdgeCases:
    ...
    def test_random_time_generation_consistency(self, mock_communication_manager):
        ...
        with patch("core.scheduler.get_schedule_time_periods") as mock_get_periods:
            ...
            start_dt = datetime.strptime("10:00", TIME_HM_FORMAT).time()
            end_dt = datetime.strptime("12:00", TIME_HM_FORMAT).time()

            for time_str in times:
                result_dt = datetime.strptime(
                    time_str, READABLE_MINUTE_TIMESTAMP_FORMAT
                )
                result_time = result_dt.time()
                assert (
                    start_dt <= result_time <= end_dt
                ), f"Time {result_time} not in range {start_dt}-{end_dt}"
    ...
    def test_schedule_all_task_reminders_success(self, test_data_dir):
        ...
        with patch("core.scheduler.get_user_data") as mock_get_data:
            mock_get_data.return_value = {
                ...
                "preferences": {
                    "task_settings": {"enabled": True, "default_reminder_time": "18:00"}
                },
            }
