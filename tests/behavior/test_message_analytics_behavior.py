...
class TestMessageAnalyticsFrequencyBehavior:
    ...
    def test_message_frequency_category_filter_real_behavior(self, analytics, mock_messages):
        ...
        with patch('core.message_analytics.get_recent_messages', return_value=motivational_messages):
            result = analytics.get_message_frequency('test_user', days=30, category='motivational')
        
        assert 'error' not in result, "Should not have error with filtered data"
        assert result['total_messages'] == len(motivational_messages), "Should have correct filtered count"
