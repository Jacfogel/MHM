...
class TestCheckinHandlerBehavior:
    ...
    def test_checkin_handler_can_handle_intents(self):
        ...
        assert not handler.can_handle('show_schedule'), "CheckinHandler should not handle show_schedule"
    ...
    def test_checkin_handler_start_checkin_not_enabled(self, mock_is_enabled, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "not enabled" in response.message.lower(), "Should indicate check-ins are not enabled"
    ...
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_start_checkin_success(self, mock_is_enabled, mock_get_recent, mock_start_checkin, test_data_dir):
        ...
        mock_start_checkin.return_value = ("Let's start your check-in. How are you feeling today?", False)
        ...
        assert "check-in" in response.message.lower() or "feeling" in response.message.lower(), "Should indicate check-in started"
    ...
    @patch('communication.command_handlers.checkin_handler.get_recent_checkins')
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_start_checkin_already_done_today(self, mock_is_enabled, mock_get_recent, test_data_dir):
        ...
        mock_get_recent.return_value = [{
            'timestamp': f"{today_str} 10:00:00",
            'date': today_str,
            'mood': 4
        }]
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "already" in response.message.lower() or "today" in response.message.lower(), "Should indicate check-in already done"
    ...
    @patch('communication.command_handlers.checkin_handler.get_recent_checkins')
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_start_checkin_conversation_manager_error(self, mock_is_enabled, mock_get_recent, mock_start_checkin, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "trouble" in response.message.lower() or "try again" in response.message.lower(), "Should indicate error"
    ...
    def test_checkin_handler_continue_checkin(self, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "not yet implemented" in response.message.lower() or "use" in response.message.lower(), "Should indicate not implemented"
    ...
    @patch('communication.command_handlers.checkin_handler.get_recent_checkins')
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_checkin_status_not_enabled(self, mock_is_enabled, mock_get_recent, test_data_dir):
    ...
    @patch('communication.command_handlers.checkin_handler.get_recent_checkins')
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_checkin_status_no_checkins(self, mock_is_enabled, mock_get_recent, test_data_dir):
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "no check-ins" in response.message.lower() or "recorded" in response.message.lower(), "Should indicate no check-ins"
    ...
    @patch('communication.command_handlers.checkin_handler.get_recent_checkins')
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_checkin_status_with_checkins(self, mock_is_enabled, mock_get_recent, test_data_dir):
        ...
        today_str = date.today().strftime("%Y-%m-%d")
        yesterday_str = (date.today() - date.resolution).strftime("%Y-%m-%d")
        mock_get_recent.return_value = [
            {'date': today_str, 'mood': 4, 'timestamp': f"{today_str} 10:00:00"},
            {'date': yesterday_str, 'mood': 3, 'timestamp': f"{yesterday_str} 14:00:00"},
            {'date': (date.today() - date.resolution * 2).strftime("%Y-%m-%d"), 'mood': 5, 'timestamp': f"{(date.today() - date.resolution * 2).strftime('%Y-%m-%d')} 09:00:00"}
        ]
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "check-in" in response.message.lower() or "recent" in response.message.lower(), "Should show check-in status"
        assert today_str in response.message or "mood" in response.message.lower(), "Should include check-in data"
    ...
    @patch('communication.command_handlers.checkin_handler.get_recent_checkins')
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_checkin_status_many_checkins(self, mock_is_enabled, mock_get_recent, test_data_dir):
        ...
        today = date.today()
        mock_get_recent.return_value = [
            {'date': (today - date.resolution * i).strftime("%Y-%m-%d"), 'mood': i % 5 + 1, 
             'timestamp': f"{(today - date.resolution * i).strftime('%Y-%m-%d')} 10:00:00"}
            for i in range(7)
        ]
        ...
        assert isinstance(response, InteractionResponse), "Should return InteractionResponse"
        assert response.completed, "Response should be completed"
        assert "more" in response.message.lower() or "and" in response.message.lower(), "Should indicate more check-ins exist"
    ...
    @patch('communication.command_handlers.checkin_handler.get_recent_checkins')
    @patch('communication.command_handlers.checkin_handler.is_user_checkins_enabled')
    def test_checkin_handler_start_checkin_with_old_checkin(self, mock_is_enabled, mock_get_recent, test_data_dir):
        ...
        yesterday_str = (date.today() - date.resolution).strftime("%Y-%m-%d")
        mock_get_recent.return_value = [{
            'timestamp': f"{yesterday_str} 10:00:00",
            'date': yesterday_str,
            'mood': 4
        }]
        
        # Mock conversation manager starting check-in
        with patch('communication.message_processing.conversation_flow_manager.conversation_manager.start_checkin') as mock_start_checkin:
            mock_start_checkin.return_value = ("Let's start your check-in. How are you feeling today?", False)
