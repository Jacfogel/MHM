...
class TestFalseNegativeDetection:
    ...
    def test_error_handling_detection(self):
        ...
        fixture_path = (
            ...
            / "tests"
            / "fixtures"
            / "development_tools_demo"
            / "false_negative_test_code.py"
        )
        ...
        try:
            error_handling_module = load_development_tools_module(
            ...
            missing_handling_functions = [
                f
                for f in functions
                if f.get("missing_error_handling", False)
    ...
    def test_documentation_detection(self):
        ...
        try:
            functions_module = load_development_tools_module(
            ...
            # Find functions without docstrings (excluding special methods and generated code)
            ...
            undocumented = [
                ...
                and not (
                    is_generated_file(f["file"]) or is_generated_function(f["name"])
                )
            ]
            ...
            assert (
                len(undocumented) > 0
            ), "Documentation analyzer should detect missing docstrings"
        ...
        try:
            ...
            for file_path, file_issues in detailed_issues.items():
                if test_file_path in file_path or test_file_path_win in file_path:
                    fixture_issues.extend(
                        file_issues if isinstance(file_issues, list) else [file_issues]
                    )
...
class TestFalsePositivePrevention:
    ...
    def test_test_fixtures_excluded_from_path_drift(self):
        ...
        try:
            exclusion_module = load_development_tools_module(
                "shared.standard_exclusions"
            )
    ...
    def test_special_methods_excluded_from_documentation(self):
        ...
        try:
            ...
            assert is_special_python_method(
                "__repr__", 10
            ), "__repr__ should be identified as special"
            assert is_special_python_method(
                "__str__", 10
            ), "__str__ should be identified as special"
            assert is_special_python_method(
                "__eq__", 10
            ), "__eq__ should be identified as special"
            assert is_special_python_method(
                "__init__", 10
            ), "Simple __init__ should be identified as special"

            # Test that context managers are NOT excluded (should be documented)
            assert not is_special_python_method(
                "__enter__", 10
            ), "__enter__ should NOT be excluded (needs documentation)"
            assert not is_special_python_method(
                "__exit__", 10
            ), "__exit__ should NOT be excluded (needs documentation)"

            # Test that complex __init__ is NOT excluded
            assert not is_special_python_method(
                "__init__", 50
            ), "Complex __init__ should NOT be excluded (needs documentation)"

            # Test that normal methods are NOT excluded
            assert not is_special_python_method(
                "handle_message", 10
            ), "Normal methods should NOT be excluded"
    ...
    def test_auto_generated_code_excluded(self):
        """Verify generated files/functions are excluded from analysis."""
        # Test exclusion logic directly rather than scanning entire codebase
        try:
            exclusion_utilities = load_development_tools_module(
                "shared.exclusion_utilities"
            ...
            assert is_generated_file(
                "ui/generated/widget_pyqt.py"
            ), "Generated directory should be detected (and _pyqt.py pattern also applies)"
            assert is_generated_file(
                "ui/generated/widget.py"
            ), "Generated directory should be detected"

            # File-name pattern detection (suffix-based)
            assert is_generated_file("widget_ui.py"), "_ui.py files should be detected"
            assert is_generated_file(
                "widget_generated.py"
            ), "_generated.py files should be detected"
            assert is_generated_file(
                "widget_auto.py"
            ), "_auto.py files should be detected"

            # Explicit _pyqt.py detection (substring-based)
            assert is_generated_file(
                "widget_pyqt.py"
            ), "_pyqt.py files should be detected"

            # --- Function-level detection ---
            assert is_generated_function(
                "setupUi"
            ), "setupUi should be detected as generated"
            assert is_generated_function(
                "retranslateUi"
            ), "retranslateUi should be detected as generated"

            assert is_generated_function(
                "setup_ui_helper"
            ), "setup_ui_helper should be detected as generated"
            assert is_generated_function(
                "do_generated_work"
            ), "do_generated_work should be detected as generated"

            # --- Normal code should NOT be excluded ---
            ...
            assert not is_generated_function(
                "handle_message"
...
class TestAccuracyMetrics:
    ...
    def test_accuracy_targets_defined(self):
        ...
        assert (
            "false_negative_rate" in ACCURACY_TARGETS
        ), "False negative rate target should be defined"
    ...
    def test_path_drift_false_positive_rate(self):
        ...
        assert (
            "path_drift_false_positive_rate" in ACCURACY_TARGETS
        ), "Path drift false positive rate target should be defined"
        assert (
            ACCURACY_TARGETS["path_drift_false_positive_rate"] == 0.05
        ), "Target should be <5%"
        ...
        try:
            ...
            assert should_exclude_file(
                "tests/fixtures/test.md", "documentation"
...
class TestFunctionCountingAccuracy:
    ...
    def test_function_counting_uses_canonical_method(self, tmp_path):
        ...
        try:
            ...
            service.results_cache = {
                "analyze_functions": {"details": {"total_functions": 1497}},
            ...
            assert (
                metrics.get("total_functions") == 1497
    ...
    def test_both_tools_use_scan_all_functions(self):
        ...
        try:
            ...
            decision_support_module = load_development_tools_module(
                "reports.decision_support"
            )

            # Both should have access to scan_all_functions
            assert hasattr(
                functions_module, "scan_all_functions"
            ...
            assert (
                "scan_all_functions" in decision_support_source
...
class TestThresholdValidation:
    ...
    def test_thresholds_loaded_from_config(self):
        ...
        try:
            ...
            assert (
                doc_threshold >= 0 and doc_threshold <= 100
            ), f"Documentation threshold should be 0-100, got {doc_threshold}"
            assert (
                test_threshold >= 0 and test_threshold <= 100
            ), f"Test coverage threshold should be 0-100, got {test_threshold}"
            assert (
                dev_tools_threshold >= 0 and dev_tools_threshold <= 100
    ...
    def test_threshold_consistency_in_reports(self, tmp_path):
        ...
        try:
            from development_tools.shared.service.report_generation import (
            ...
            assert hasattr(
                service, "_generate_ai_status_document"
            ), "Service should have status document generation"
            assert hasattr(
                service, "_generate_ai_priorities_document"
            ), "Service should have priorities document generation"

            # Verify config is accessible
            assert hasattr(service, "audit_config") or hasattr(
                config, "development_tools_config"
    ...
    def test_recommendations_have_required_structure(self):
        ...
        try:
            ...
            recommendations = validator.generate_recommendations(
                tools_analysis, config_validation, completeness
            )
            ...
            if recommendations:
                for rec in recommendations[:3]:  # Check first 3
                    ...
                    assert len(rec.strip()) > 0, "Recommendation should not be empty"
