...
# Load the module
shared_module = load_development_tools_module("shared.exclusion_utilities")
is_generated_file = shared_module.is_generated_file
is_generated_function = shared_module.is_generated_function
...
class TestGeneratedDetection:
    """Test generated file/function detection helpers."""

    @pytest.mark.unit
    def test_pyqt_generated_file(self):
        """Test detection of generated directory files (including PyQt-related paths)."""
        result = is_generated_file("ui/generated/test_pyqt.py")
        assert result is True, "Should detect files under generated directories"

    @pytest.mark.unit
    def test_generated_function_patterns(self):
        """Test detection of generated function-name patterns."""
        result = is_generated_function("setupUi")
        assert result is True, "Should detect setupUi as generated"

        result = is_generated_function("retranslateUi")
        assert result is True, "Should detect retranslateUi as generated"

    @pytest.mark.unit
    def test_generated_directory(self):
        """Test detection of files in generated directories."""
        result = is_generated_file("some/path/generated/test.py")
        assert result is True, "Should detect files in generated directories"

    @pytest.mark.unit
    def test_generated_file_name_patterns(self):
        """Test detection of generated file-name patterns."""
        result = is_generated_file("test_pyqt.py")
        assert result is True, "Should detect _pyqt.py files"

        result = is_generated_file("test_ui.py")
        assert result is True, "Should detect _ui.py files"

        result = is_generated_file("test_generated.py")
        assert result is True, "Should detect _generated.py files"

    @pytest.mark.unit
    def test_regular_function_not_generated(self):
        """Test that regular file + function name are not detected as generated."""
        result = is_generated_file("test.py") or is_generated_function(
            "regular_function"
        )
        assert (
            result is False
        ), "Regular files/functions should not be detected as generated"
...
class TestExtractDecoratorDocumentation:
    ...
    def test_handle_errors_decorator(self):
        ...
        assert (
            "@handle_errors: test description" in result
        ), "Should extract handle_errors documentation"
    ...
    def test_error_handler_decorator(self):
        ...
        assert (
            result == ""
        ), "Current implementation doesn't extract error_handler (known bug)"
...
class TestGenerateFunctionTemplate:
    ...
    def test_test_function_template(self):
        ...
        result = generate_function_template(
            "test_function", "test_something", "test.py", []
        )
...
"""
        ...
        assert (
            functions[0]["name"] == "simple_function"
        ), "Should extract correct function name"
        assert (
            "A simple function" in functions[0]["docstring"]
...
"""
        ...
        assert len(functions) == 1, "Should extract one function"
        assert (
            functions[0]["func_type"] == "test_function"
...
class TestCategorizeFunctions:
    ...
    def test_categorize_by_complexity(self):
        ...
        categories = categorize_functions(functions)
        assert (
            len(categories["critical_complex"]) == 1
        ), "Should categorize critical complexity"
    ...
    def test_categorize_undocumented(self):
        ...
        categories = categorize_functions(functions)
        assert (
            len(categories["undocumented"]) == 1
        ), "Should categorize undocumented functions"
        ...
        with (
            ...
            with patch("sys.argv", ["analyze_functions.py", "--include-tests"]):
                ...
                try:
                    ...
                    assert mock_scan.called, "scan_all_functions should be called"
                    call_kwargs = (
                        mock_scan.call_args[1] if len(mock_scan.call_args) > 1 else {}
                    )
                    assert (
                        call_kwargs.get("include_tests") is True
                    ), "Should include tests when flag is set"
                finally:
                    sys.stdout = old_stdout
