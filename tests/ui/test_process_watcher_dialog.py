...
class TestProcessWatcherDialogInitialization:
    ...
    def dialog(self, qapp):
        ...
        with patch('ui.dialogs.process_watcher_dialog.psutil.process_iter') as mock_process_iter:
            with patch('core.service_utilities.get_service_processes') as mock_get_service:
    ...
    def test_dialog_initialization(self, dialog):
        ...
        assert dialog.windowTitle() == "Process Watcher - Python Processes", "Dialog should have correct title"
        ...
        assert hasattr(dialog, 'auto_refresh_checkbox'), "Auto-refresh checkbox should exist"
        assert hasattr(dialog, 'tab_widget'), "Tab widget should exist"
        assert hasattr(dialog, 'all_processes_table'), "All processes table should exist"
        assert hasattr(dialog, 'mhm_processes_table'), "MHM processes table should exist"
        assert hasattr(dialog, 'process_details_text'), "Process details text should exist"
        assert hasattr(dialog, 'refresh_timer'), "Refresh timer should exist"
        
        # Assert: Verify initial auto-refresh state
        assert dialog.auto_refresh_enabled is False, "Auto-refresh should be disabled initially"
    ...
    def test_tab_widget_setup(self, dialog):
        ...
        assert dialog.tab_widget.tabText(0) == "All Python Processes", "First tab should be All Python Processes"
        assert dialog.tab_widget.tabText(1) == "MHM Services", "Second tab should be MHM Services"
    ...
    def test_table_widget_setup(self, dialog):
        ...
        # All processes table
        assert dialog.all_processes_table.columnCount() == 6, "All processes table should have 6 columns"
        assert dialog.all_processes_table.horizontalHeaderItem(0).text() == "PID", "First column should be PID"
        assert dialog.all_processes_table.horizontalHeaderItem(1).text() == "Name", "Second column should be Name"
        assert dialog.all_processes_table.horizontalHeaderItem(2).text() == "Command Line", "Third column should be Command Line"
        
        # MHM processes table
        assert dialog.mhm_processes_table.columnCount() == 7, "MHM processes table should have 7 columns"
        assert dialog.mhm_processes_table.horizontalHeaderItem(0).text() == "PID", "First column should be PID"
        assert dialog.mhm_processes_table.horizontalHeaderItem(1).text() == "Type", "Second column should be Type"
        assert dialog.mhm_processes_table.horizontalHeaderItem(6).text() == "Status", "Last column should be Status"
...
class TestProcessWatcherRefresh:
    ...
    def test_update_all_processes_with_python_processes(self, dialog):
        ...
        with patch('ui.dialogs.process_watcher_dialog.psutil.process_iter', return_value=[mock_proc]):
            ...
            assert dialog.all_processes_table.item(0, 0).text() == "12345", "PID should be correct"
            assert dialog.all_processes_table.item(0, 1).text() == "python.exe", "Name should be correct"
    ...
    def test_update_mhm_processes_with_service_processes(self, dialog):
        ...
        with patch('core.service_utilities.get_service_processes', return_value=[mock_service_process]):
            with patch('ui.dialogs.process_watcher_dialog.psutil.Process') as mock_psutil_proc:
                ...
                assert dialog.mhm_processes_table.item(0, 0).text() == "54321", "PID should be correct"
                assert dialog.mhm_processes_table.item(0, 1).text() == "headless", "Type should be correct"
    ...
    def test_update_mhm_processes_handles_psutil_errors(self, dialog):
        ...
        with patch('core.service_utilities.get_service_processes', return_value=[mock_service_process]):
            with patch('ui.dialogs.process_watcher_dialog.psutil.Process', side_effect=psutil.NoSuchProcess(54321)):
                # Should not raise exception
                dialog.update_mhm_processes()
                
                # Table should still be updated with default values
                assert dialog.mhm_processes_table.rowCount() == 1, "Should handle psutil errors gracefully"
                assert dialog.mhm_processes_table.item(0, 3).text() == "0.0%", "CPU should default to 0.0%"
                assert dialog.mhm_processes_table.item(0, 4).text() == "0.0%", "Memory should default to 0.0%"
                assert dialog.mhm_processes_table.item(0, 6).text() == "Unknown", "Status should default to Unknown"
...
class TestProcessWatcherAutoRefresh:
    ...
    def test_toggle_auto_refresh_enables(self, dialog):
        ...
        assert dialog.auto_refresh_checkbox.text() == "Auto Refresh: ON", "Button text should show ON"
    ...
    def test_toggle_auto_refresh_disables(self, dialog):
        ...
        assert dialog.auto_refresh_checkbox.text() == "Auto Refresh: OFF", "Button text should show OFF"
    ...
    def test_auto_refresh_timer_triggers_refresh(self, dialog):
        ...
        assert dialog.refresh_timer.isActive(), "Timer should be active"
        assert dialog.refresh_timer.interval() == 5000, "Timer should have 5 second interval"
        ...
        assert dialog.refresh_timer.isActive(), "Timer should be active when auto-refresh is enabled"
...
class TestProcessWatcherDetails:
    ...
    def test_show_process_details_with_valid_process(self, dialog):
        ...
        with patch('ui.dialogs.process_watcher_dialog.psutil.Process', return_value=mock_proc):
            dialog.show_process_details(12345)
            
            # Verify details text was updated
            details_text = dialog.process_details_text.toPlainText()
            assert "Process Details for PID 12345" in details_text, "Should show process details header"
    ...
    def test_show_process_details_with_no_such_process(self, dialog):
        ...
        with patch('ui.dialogs.process_watcher_dialog.psutil.Process', side_effect=psutil.NoSuchProcess(12345)):
            dialog.show_process_details(12345)
            
            # Verify error message was displayed
            details_text = dialog.process_details_text.toPlainText()
            assert "Error accessing process 12345" in details_text, "Should show error message"
    ...
    def test_show_process_details_with_access_denied(self, dialog):
        ...
        with patch('ui.dialogs.process_watcher_dialog.psutil.Process', side_effect=psutil.AccessDenied(12345)):
            dialog.show_process_details(12345)
    ...
    def test_show_process_details_with_environ_error(self, dialog):
        ...
        with patch('ui.dialogs.process_watcher_dialog.psutil.Process', return_value=mock_proc):
            ...
            details_text = dialog.process_details_text.toPlainText()
            assert "Process Details for PID 12345" in details_text, "Should show process details"
            assert "Access denied" in details_text, "Should show access denied for environ"
...
class TestProcessWatcherClose:
    ...
    def test_close_event_stops_timer(self, dialog):
        ...
        assert not dialog.refresh_timer.isActive(), "Timer should be stopped after close"
...
class TestProcessWatcherIntegration:
    ...
    def test_complete_refresh_workflow(self, dialog):
        ...
        with patch('ui.dialogs.process_watcher_dialog.psutil.process_iter', return_value=[mock_python_process]):
            with patch('core.service_utilities.get_service_processes', return_value=[mock_service_process]):
                with patch('ui.dialogs.process_watcher_dialog.psutil.Process') as mock_psutil_proc:
                    mock_proc = MagicMock()
                    mock_proc.cpu_percent.return_value = 3.2
        ...
        assert dialog.all_processes_table.rowCount() == 1, "All processes table should have 1 row"
        assert dialog.mhm_processes_table.rowCount() == 1, "MHM processes table should have 1 row"
        assert dialog.all_processes_table.item(0, 0).text() == "12345", "Python process PID should be displayed"
    ...
    def test_complete_selection_workflow(self, dialog):
        ...
        mock_proc_details.create_time.return_value = datetime.now().timestamp()
        ...
        dialog.all_processes_table.setCurrentCell(0, 0)
        with patch('ui.dialogs.process_watcher_dialog.psutil.Process', return_value=mock_proc_details):
            dialog.on_process_selected()
    ...
    def test_auto_refresh_workflow(self, dialog):
        ...
        assert dialog.all_processes_table.rowCount() == 1, "Table should be refreshed with process data"
