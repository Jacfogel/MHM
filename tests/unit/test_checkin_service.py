...
from core import checkin_service
...
def test_get_checkin_start_status_detects_disabled_user():
    with patch("core.checkin_service.is_user_checkins_enabled", return_value=False):
...
def test_get_checkin_start_status_detects_today_checkin():
    with patch("core.checkin_service.is_user_checkins_enabled", return_value=True), patch(
        "core.checkin_service.get_recent_checkins",
        return_value=[{"timestamp": "2026-05-11 08:00:00"}],
    ), patch(
        "core.checkin_service.checkin_runtime_timestamp",
        return_value="2026-05-11 08:00:00",
    ), patch(
        "core.checkin_service.parse_timestamp_full",
