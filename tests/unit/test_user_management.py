...
class TestUserManagement:
    ...
    def test_get_user_preferences_success(self, mock_user_data, mock_config):
        ...
        if preferences is not None:
            ...
            if features.get("automated_messages") == "enabled":
                ...
                ), "Categories should be present when automated_messages is enabled"
            else:
    ...
    def test_hybrid_get_user_data_success(self, mock_user_data):
        ...
        if features.get("automated_messages") == "enabled":
            assert (
                "categories" in user_data["preferences"]
            ), "Categories should be present when automated_messages is enabled"
    ...
    def test_hybrid_get_user_data_nonexistent_user(self, mock_config):
        ...
        filtered = {
            ...
            if k in {"account", "preferences", "context", "schedules"}
        }
    ...
    def test_save_user_data_success(self, test_data_dir, mock_config):
        ...
        assert os.path.exists(
            account_file
        ), f"Account file should be created. User dir: {actual_user_dir}, Files: {os.listdir(actual_user_dir) if os.path.exists(actual_user_dir) else 'N/A'}"
        assert os.path.exists(
            prefs_file
        ), f"Preferences file should be created. User dir: {actual_user_dir}, Files: {os.listdir(actual_user_dir) if os.path.exists(actual_user_dir) else 'N/A'}"
        assert os.path.exists(
            context_file
    ...
    def test_update_user_preferences_success(self, mock_user_data, mock_config):
        ...
        # Verify the update
        updated_preferences = get_user_data(mock_user_data["user_id"], "preferences")[
            "preferences"
        ]
...
class TestUserManagementEdgeCases:
    ...
    def test_update_user_preferences_nonexistent_user(self, mock_config):
        ...
        result = update_user_preferences(
            unique_user_id, {"test": "data"}, auto_create=False
        )
    ...
    def test_user_lifecycle(self, test_data_dir, mock_config):
        ...
        assert os.path.exists(
            test_data_dir
        ), f"Test directory should exist: {test_data_dir}"
        ...
        assert os.path.exists(
            actual_user_dir
        ), f"User directory should be created: {actual_user_dir}"
        assert os.path.isdir(
            actual_user_dir
        ), f"User path should be a directory: {actual_user_dir}"

        # [OK] VERIFY REAL BEHAVIOR: Check directory permissions
        assert os.access(
            actual_user_dir, os.R_OK
        ), f"User directory should be readable: {actual_user_dir}"
        assert os.access(
            actual_user_dir, os.W_OK
        ), f"User directory should be writable: {actual_user_dir}"
        ...
        for file_name in expected_files:
            ...
            assert os.path.isfile(
                file_path
            ), f"Required path should be a file: {file_path}"
            assert os.access(
                file_path, os.R_OK
            ), f"Required file should be readable: {file_path}"
            assert os.access(
                file_path, os.W_OK
            ), f"Required file should be writable: {file_path}"
        ...
        for dir_name in expected_dirs:
            ...
            assert os.path.isdir(
                dir_path
            ), f"Required path should be a directory: {dir_path}"
            assert os.access(
                dir_path, os.R_OK
            ), f"Required directory should be readable: {dir_path}"
            assert os.access(
                dir_path, os.W_OK
            ), f"Required directory should be writable: {dir_path}"
        ...
        if "categories" in file_preferences:
            ...
            assert (
                "motivational" in file_preferences["data"]["categories"]
            ), "Should contain motivational category"
        else:
        ...
        assert isinstance(
            consistency_data["preferences"]["categories"], list
        ), "Categories should be a list"
        assert (
        ...
        partial_updates = {"task_settings": {"enabled": True}}
        ...
        assert isinstance(
            partial_updated_data["preferences"]["categories"], list
        ), "Categories should be a list"
        assert (
            "motivational" in partial_updated_data["preferences"]["categories"]
        ), "Should contain motivational category"
        # Note: 'health' category might be filtered out by validation logic
        ...
        for file_name in expected_files:
            ...
            assert (
                file_size > 0
            ), f"File should not be empty: {file_name} - {file_size} bytes"
            assert (
                file_size < 10000
            ), f"File should not be unreasonably large: {file_name} - {file_size} bytes"
        ...
        for result in concurrent_results:
            assert (
                result == "America/Regina"
            ), f"Concurrent read should return consistent data: {result}"
        ...
        unexpected_items = [
            ...
            and not f.endswith(
                ".corrupted_"
            )  # Allow corrupted file backups (format: filename.corrupted_TIMESTAMP)
            and ".corrupted_" not in f  # Also catch any corrupted backup files
        ]
        assert (
            len(unexpected_items) == 0
        ), f"No unexpected non-JSON files/directories should be created: {unexpected_items}"
        ...
        for file_name in expected_files:
            ...
            assert os.path.exists(
                file_path
            ), f"File should exist in final state: {file_path}"
            assert os.access(
                file_path, os.R_OK
            ), f"File should be readable in final state: {file_path}"
            assert os.access(
                file_path, os.W_OK
            ), f"File should be writable in final state: {file_path}"

        for dir_name in expected_dirs:
            dir_path = os.path.join(actual_user_dir, dir_name)
            assert os.path.exists(
                dir_path
            ), f"Directory should exist in final state: {dir_path}"
            assert os.access(
                dir_path, os.R_OK
            ), f"Directory should be readable in final state: {dir_path}"
            assert os.access(
                dir_path, os.W_OK
    ...
    def test_get_user_data_single_type(self, mock_user_data, mock_config):
        ...
        assert (
            "preferences" in user_data
        ), f"Expected preferences in user_data, got: {list(user_data.keys())}"
