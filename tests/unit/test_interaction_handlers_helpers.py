...
# These helpers are production-behavior-sensitive and must not rely on wall-clock time.
...
class TestTaskManagementHandlerHelpers:
    ...
    def test_parse_relative_date_next_month(self):
        ...
        with (
            ...
            # Deterministic month boundary behavior (avoid wall-clock dependence).
            today = TEST_NOW_DT
            if today.month == 12:
                expected_dt = today.replace(year=today.year + 1, month=1)
            else:
                expected_dt = today.replace(month=today.month + 1)

            expected = format_timestamp(expected_dt, DATE_ONLY)
    ...
    def test_sort_tasks_with_none_due_date(self):
        ...
        result = self.handler._handle_list_tasks__sort_tasks(tasks)
        assert (
            result[0]["title"] == "Task 2"
        ), "Task with due_date should come before None"
        assert (
            result[1]["title"] == "Task 1"
        ), "Task with None due_date should come last"
    ...
    def test_get_task_candidates_by_partial_title(self):
        ...
        assert any(
            t["title"] == "Buy groceries" for t in result
        ), "Should include matching task"
