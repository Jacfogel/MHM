...
class TestUserContextLegacyPreferences:
    """Test UserContext legacy preference methods"""
    ...
    @pytest.mark.unit
    def test_set_preference_with_preferences(self, context):
        """Test: set_preference delegates to UserPreferences"""
        # Arrange
        user_id = "test_pref_user"
        context.set_user_id(user_id)
        
        with patch.object(context.preferences, 'set_preference') as mock_set:
            # Act
            context.set_preference('test_key', 'test_value')
            
            # Assert
            mock_set.assert_called_once_with('test_key', 'test_value')
    
    @pytest.mark.unit
    def test_set_preference_without_preferences(self, context):
        """Test: set_preference logs warning when preferences not initialized"""
        # Arrange
        context.preferences = None
        
        with patch('user.user_context.logger') as mock_logger:
            # Act
            context.set_preference('test_key', 'test_value')
    ...
    @pytest.mark.unit
    def test_get_preference_with_preferences(self, context):
        """Test: get_preference delegates to UserPreferences"""
        # Arrange
        user_id = "test_pref_user"
        context.set_user_id(user_id)
        
        with patch.object(context.preferences, 'get_preference', return_value='test_value') as mock_get:
            # Act
            result = context.get_preference('test_key')
            
            # Assert
            mock_get.assert_called_once_with('test_key')
            assert result == 'test_value', "Should return preference value"
    
    @pytest.mark.unit
    def test_get_preference_without_preferences(self, context):
        """Test: get_preference returns None when preferences not initialized"""
        # Arrange
        context.preferences = None
        ...
        # Assert
        assert result is None, "Should return None when preferences not initialized"
    
    @pytest.mark.unit
    def test_update_preference_with_preferences(self, context):
        """Test: update_preference delegates to UserPreferences"""
        # Arrange
        user_id = "test_pref_user"
        context.set_user_id(user_id)
        
        with patch.object(context.preferences, 'update_preference') as mock_update:
            # Act
            context.update_preference('test_key', 'test_value')
            
            # Assert
            mock_update.assert_called_once_with('test_key', 'test_value')
    
    @pytest.mark.unit
    def test_update_preference_without_preferences(self, context):
        """Test: update_preference logs warning when preferences not initialized"""
        # Arrange
        context.preferences = None
