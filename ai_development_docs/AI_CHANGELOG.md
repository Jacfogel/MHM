...
- Home chat shows website, Discord, and email messages in one timeline, oldest first. That includes messages you sent on those channels and messages MHM sent back.
- Home uses one check-in action: start, continue, or you're checked in. Account deletion is on Account, under Your data, after typing DELETE. Pages use system fonts and no longer load Google Fonts.
