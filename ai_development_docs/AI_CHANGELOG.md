...
- **Round 8 (4 fixes)**: Added `integrations` to `.example` `local_module_prefixes`. Health handler reads `GOOGLE_HEALTH_ENABLED` at call time from `core.config`. System signals test pins `core_files` so CI config key_files do not mark CRITICAL. User index updates use threading lock + `safe_json_read`/`safe_json_write` (fixes Linux concurrent JSON corruption).
