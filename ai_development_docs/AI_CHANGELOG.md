...
- Logout checks drafts before revocation, releases navigation guards on logout/session expiry, and restores the button after failures. Workerd reproduced unsupported `redirect: 'error'`; gateway fetches now use `manual` with redirect rejection. Cloudflare confirms the fix is active on Worker `mhm` (Wrangler name aligned). Published account GET returns 401 and empty-email login POST returns 400 from the gateway; real email delivery/login remain user verification.
