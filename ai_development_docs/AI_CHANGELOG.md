...
- Logout checks drafts before revocation, releases navigation guards on logout/session expiry, and restores the button after failures. Isolated workerd reproduced the production proxy failure: `redirect: 'error'` is unsupported. Gateway fetches now use `manual`, rejecting unexpected redirects while preserving the Discord callback. Live verification requires deployment.
