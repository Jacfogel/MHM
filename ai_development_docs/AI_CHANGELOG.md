...
- Notebook groups are retired end to end from runtime models, services, commands, Discord note creation, the website API, and active tests/docs. Task groups remain. The shared v2 notebook record keeps a blank compatibility key so older files still validate; legacy notebook group values are ignored and cleared on the next save.
