# AI Priorities - Immediate Next Steps

> **File**: `ai_development_tools/AI_PRIORITIES.md`
> **Generated**: This file is auto-generated by ai_tools_runner.py. Do not edit manually.
> **Generated by**: ai_tools_runner.py - AI Development Tools Runner
> **Last Generated**: 2025-11-18 06:38:04
> **Source**: `python ai_development_tools/ai_tools_runner.py audit`

## Immediate Focus (Ranked)
1. **Harden critical error handling**  -  117 functions still lack structured error recovery.
   - Focus modules: discord/checkin_view.py (20.0%, missing 4/5), discord/task_reminder_view.py (20.0%, missing 4/5), discord/account_flow_handler.py (35.3%, missing 11/17)
   - Convert remaining basic handlers to `@handle_errors` and add guard tests.
2. **Raise coverage for low-performing modules**  -  3 key modules remain below the 80% target.
   - Target modules: communication (66.4%, 2963 lines missing), core (70.7%, 2708 lines missing), ui (71.6%, 1739 lines missing)
   - Add scenario tests before the next full audit to lift module coverage above 80%.
3. **Retire remaining legacy references**  -  5 files still depend on legacy compatibility markers.
   - 1 legacy markers still surface during scans.
   - Review C:\Users\Julie\projects\MHM\MHM\development_docs\LEGACY_REFERENCE_REPORT.md for exact locations.
   - Use `python ai_development_tools/ai_tools_runner.py legacy --apply` to replace deprecated helpers.

## Quick Wins
- No immediate quick wins identified. Re-run doc-sync after tackling focus items.

## Watch List
- Documentation coverage sits at 93.57% (target 90%).
- Overall test coverage is 70.2% (18786 / 26754 statements).
- Keep an eye on untested files: communication/communication_channels/discord/checkin_view.py (16.7%), communication/command_handlers/account_handler.py (20.9%).

## Follow-up Commands
- `python ai_development_tools/ai_tools_runner.py doc-sync`  -  refresh drift, pairing, and ASCII metrics.
- `python ai_development_tools/ai_tools_runner.py legacy --apply`  -  update legacy references in-place.
- `python ai_development_tools/ai_tools_runner.py audit --full`  -  rebuild coverage and hygiene data after fixes.
- `python ai_development_tools/ai_tools_runner.py status`  -  confirm the latest health snapshot.