...
- *Known flakes*: ~10 ordering-dependent full-suite failures caused by v1/v2 schedule format mismatch (tests write v1 flat format but read back v2 envelope after earlier tests trigger v2 validation). Affected files: `test_account_lifecycle.py`, `test_account_management_real_behavior.py`, `test_user_management_coverage_expansion.py`, `test_user_creation.py`, `test_account_creation_ui.py`, `test_health_sync_schedule.py`, `test_google_health_sync_manager.py`, `test_storage_scenarios.py`, `test_user_data_read_scenarios.py`. All pass individually; fail only during full suite runs.
