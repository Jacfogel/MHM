...
def _save_updated_entry(user_id: str, entry: Entry, all_entries: list[Entry]) -> Entry:
    ...
    entry.updated_at = now_timestamp_full()
...
def create_entry(
    ...
    if kind == "list":
        ...
        logger.warning(
            f"Items provided for non-list entry kind '{kind}'. Ignoring items."
        )
...
def append_to_entry_body(user_id: str, ref: str, text: str) -> Entry | None:
    ...
    if not is_valid_string_length(
        text, MAX_BODY_LENGTH, field_name="Appended text", allow_none=False
    ):
        logger.error(
            f"Text to append exceeds maximum length of {MAX_BODY_LENGTH} characters"
        )
        return None
...
def toggle_list_item_done(
    ...
    entry.items[normalized_index].updated_at = now_timestamp_full()
    ...
    for e in entries:
        if not e.archived and not e.tags:
            ...
            try:
                ...
                logger.warning(
                    f"Invalid timestamp format for entry {e.id}: {e.updated_at} - {err}"
                )
