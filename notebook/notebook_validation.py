...
def is_valid_entry_reference(ref: str) -> bool:
    ...
    # Fragment must be at least MIN_SHORT_ID_LENGTH characters
    short_id_fragment = re.compile(rf'^[0-9a-f]{{{MIN_SHORT_ID_LENGTH},{MAX_SHORT_ID_LENGTH}}}$', re.IGNORECASE)
...
def is_valid_entry_title(title: Optional[str]) -> bool:
    ...
    """
    return is_valid_string_length(title, MAX_TITLE_LENGTH, field_name="Entry title", allow_none=True)
...
def is_valid_entry_body(body: Optional[str], kind: EntryKind = 'note') -> bool:
    ...
    return is_valid_string_length(body, MAX_BODY_LENGTH, field_name="Entry body", allow_none=True)
...
def is_valid_entry_group(group: Optional[str]) -> bool:
    ...
    """
    return is_valid_category_name(group, max_length=MAX_GROUP_LENGTH, field_name="Entry group", allow_none=True)
...
def is_valid_list_item_index(index: int, list_length: int) -> bool:
    ...
    if not isinstance(index, int):
        logger.warning(f"List item index must be an integer, got {type(index).__name__}")
        return False
    ...
    logger.warning(f"List item index {index} is out of range for list of length {list_length}")
    return False
