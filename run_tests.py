...
@handle_errors("killing process tree on Windows", user_friendly=False, default_return=False)
def kill_process_tree_windows(pid):
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"{GREEN}[CLEANUP]{RESET} Successfully killed process tree for PID {pid}")
            return True
        elif "not found" in result.stdout.lower() or "not found" in result.stderr.lower():
            # Process already gone - that's fine
            print(f"{YELLOW}[CLEANUP]{RESET} Process {pid} not found (already terminated)")
            return True
        else:
            print(f"{YELLOW}[CLEANUP]{RESET} taskkill returned code {result.returncode} for PID {pid}: {result.stderr}")
            return False
...
@handle_errors("cleaning up orphaned pytest processes", user_friendly=False, default_return=0)
            ...
            try:
                ...
                if len(parts) >= 2:
                    ...
                    if cmd_result.returncode == 0 and cmd_result.stdout:
                        ...
                        if ('pytest' in cmdline or 'gw' in cmdline) and 'xdist' in cmdline:
                            ...
                            try:
                                ...
                                if kill_result.returncode == 0:
                                    killed_count += 1
                                    print(f"{GREEN}[CLEANUP]{RESET} Killed orphaned pytest worker PID {pid}")
                            except Exception as e:
                                print(f"{YELLOW}[CLEANUP]{RESET} Failed to kill orphaned process {pid}: {e}")
            except (ValueError, IndexError):
                continue
        
        if killed_count > 0:
            print(f"{GREEN}[CLEANUP]{RESET} Cleaned up {killed_count} orphaned pytest worker process(es)")
        elif killed_count == 0:
...
def interrupt_handler(signum, frame):
    ...
    if _current_junit_xml:
        save_partial_results(_current_junit_xml, interrupted=True, output_text=output_text, test_context=_current_test_context)
    ...
    if _current_process and _current_process.poll() is None:
        print(f"{YELLOW}[INTERRUPT]{RESET} Terminating test process and all worker processes...")
        try:
        ...
        if _current_process.poll() is None:
            print(f"{YELLOW}[INTERRUPT]{RESET} Process did not terminate, force killing process tree...")
        ...
        if sys.platform == 'win32':
            ...
            if orphaned_count == 0:
                print(f"{GREEN}[CLEANUP]{RESET} Verified: No orphaned processes remaining")
        ...
        try:
            ...
            print(f"{YELLOW}[CLEANUP]{RESET} Could not consolidate worker logs automatically: {e}")
            print(f"{YELLOW}[CLEANUP]{RESET} Run manually: python development_tools/consolidate_worker_logs.py")
...
def check_resource_warnings(resources: Dict) -> bool:
    ...
    if 'system_memory_percent' in resources:
        ...
        if mem_percent > 98:
            warnings.append(f"System memory usage: {mem_percent:.1f}% (CRITICAL - terminating to prevent system crash)")
            critical = True
        elif mem_percent > 95:
            warnings.append(f"System memory usage: {mem_percent:.1f}% (CRITICAL - consider terminating)")
            critical = True
        elif mem_percent > 80:
...
@handle_errors("checking critical resource limits", user_friendly=False, default_return=False)
...
@handle_errors("extracting results from output text", user_friendly=False, default_return={'passed': 0, 'failed': 0, 'skipped': 0, 'warnings': 0, 'errors': 0, 'total': 0, 'deselected': 0})
def extract_results_from_output(output_text: str) -> Dict[str, int]:
    ...
    full_pattern = r'(\d+)\s+failed[,\s]+(\d+)\s+passed[,\s]+(\d+)\s+skipped[,\s]+(\d+)\s+warnings'
...
def extract_pytest_session_info(output_text: str) -> Dict:
    ...
    workers_match = re.search(r'created:\s*(\d+)/(\d+)\s+workers', output_plain, re.IGNORECASE)
    if workers_match:
    ...
    items_match = re.search(r'(\d+)\s+workers\s+\[(\d+)\s+items?\]', output_plain, re.IGNORECASE)
    if items_match:
        session_info['test_items'] = int(items_match.group(2))
    
    # Extract collected/deselected counts
    collected_match = re.search(r'collected\s+(\d+)\s+items?(?:\s+/\s+(\d+)\s+deselected)?', output_plain, re.IGNORECASE)
    if collected_match:
...
def save_partial_results(junit_xml_path: str, interrupted: bool = False, output_text: str = None, test_context: Dict = None):
    """Save partial test results from JUnit XML, falling back to output text parsing."""
    global _captured_output_lines
    
    results = {'passed': 0, 'failed': 0, 'skipped': 0, 'warnings': 0, 'errors': 0, 'total': 0, 'deselected': 0}
    ...
    try:
        ...
        if interrupted:
            ...
            print(f"{YELLOW}[PARTIAL RESULTS]{RESET} Timestamped copy: {timestamped_partial_file.name}")
...
def detect_stuck_process(last_output_time: float, current_time: float, threshold: float = 600) -> bool:
...
@handle_errors("extracting failure details from JUnit XML", user_friendly=False, default_return=[])
...
def run_command(cmd, description, progress_interval: int = 30, capture_output: bool = True, test_context: Dict = None):
    """
    ...
    try:
        ...
        if sys.platform == 'win32':
            ...
            print(f"{GREEN}[PROCESS]{RESET} Creating process with CREATE_NEW_PROCESS_GROUP flag for tree termination")
        ...
        if sys.platform == 'win32':
            print(f"{GREEN}[PROCESS]{RESET} Started pytest process PID {process.pid} (can kill tree)")
        ...
        next_resource_check = start_time + 60  # Check resources every minute (more frequent if memory high)
        ...
        while process.poll() is None:
            ...
            if _interrupt_requested:
                ...
                output_text = ''.join(_captured_output_lines) if _captured_output_lines else None
                save_partial_results(junit_xml_path, interrupted=True, output_text=output_text, test_context=_current_test_context)
            ...
            if elapsed > max_duration:
                ...
                save_partial_results(junit_xml_path, interrupted=True, test_context=_current_test_context)
                # Kill entire process tree (especially important for pytest-xdist workers on Windows)
                if sys.platform == 'win32':
                    kill_process_tree_windows(process.pid)
                else:
            ...
            if detect_stuck_process(_last_output_time, now, threshold=600) and not stuck_warning_shown:
            ...
            if now >= next_resource_check:
                ...
                if resources:
                    ...
                    if check_critical_resources(resources):
                        ...
                        print(f"{RED}[CRITICAL]{RESET} Saving partial results before termination...")
                        # Collect output before saving
                        output_text = ''.join(_captured_output_lines) if _captured_output_lines else None
                        save_partial_results(junit_xml_path, interrupted=True, output_text=output_text, test_context=_current_test_context)
                        # Kill entire process tree (especially important for pytest-xdist workers on Windows)
                    ...
                    next_resource_check = now + 60  # Default check every minute if monitoring unavailable
            ...
            if now >= next_partial_save:
                ...
                next_partial_save = now + 300  # Save every 5 minutes (reduced from 2 minutes)
        ...
        if sys.platform == 'win32':
            ...
            try:
                ...
                print(f"{YELLOW}[CLEANUP]{RESET} Could not consolidate worker logs: {e}")
                print(f"{YELLOW}[CLEANUP]{RESET} Run manually: python scripts/testing/consolidate_worker_logs.py")
        ...
        if _interrupt_requested:
            ...
            print(f"{YELLOW}[INTERRUPTED]{RESET} Partial results saved to {_partial_results_file}")
            ...
            save_partial_results(junit_xml_path, interrupted=False, test_context=_current_test_context)
        ...
        if os.path.exists(junit_xml_path):
            try:
                ...
                backups = sorted(_backups_dir.glob("junit_results_*.xml"), key=os.path.getmtime)
                if len(backups) > 5:
...
def print_combined_summary(parallel_results: Optional[Dict], no_parallel_results: Optional[Dict], description: str):
    ...
    if no_parallel_results and isinstance(no_parallel_results, dict):
        ...
        p_interrupted = parallel_results.get('interrupted', False) if parallel_results and isinstance(parallel_results, dict) else False
    ...
    if parallel_results and isinstance(parallel_results, dict) and parallel_results.get('interrupted', False):
...
def main():
    ...
    if not _auto_terminate_enabled:
        print(f"[INFO] Automatic termination on critical memory disabled (use --critical-memory-limit to set threshold)")
    else:
        print(f"[INFO] Automatic termination enabled at {_critical_memory_limit}% system memory")
    ...
    if not args.no_parallel:
        ...
        if args.workers == "auto":
            ...
            print(f"[INFO] Auto-detected {cpu_count} CPUs, using {max_workers} workers to prevent OOM")
        else:
            try:
    ...
    parallel_results = run_command(cmd, description, progress_interval=args.progress_interval, test_context=parallel_test_context)
    ...
    if not args.no_parallel:
        ...
        no_parallel_test_context = {**base_test_context, 'phase': 'serial', 'parallel': False}
        no_parallel_results = run_command(no_parallel_cmd, "No-Parallel Tests (Serial)", progress_interval=args.progress_interval, test_context=no_parallel_test_context)
