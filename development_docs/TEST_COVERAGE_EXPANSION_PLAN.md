...
|--------|----------|-----------------|--------------------|
| ui/generate_ui_files.py | 0.0% | 0 / 63 | Add smoke tests for generator to keep UI build scripts honest |
...
## Priority Actions
1. Add smoke and behavior tests that import `communication/command_handlers/profile_handler.py` and `analytics_handler.py`; without execution data these high-complexity handlers remain completely unverified.
2. Target UI bootstrapping gaps: extend tests for `ui/ui_app_qt.py`, `ui/dialogs/task_crud_dialog.py`, and `ui/generate_ui_files.py` so critical UI flows participate in coverage.
3. Strengthen communication pathways by covering `interaction_manager.handle_message`, `channel_monitor`, and the Discord bot lifecycle to guard complex routing logic.
4. For modules already above 90%, keep regression tests lightweight and stable; use them as patterns when refactoring lower-coverage counterparts.
