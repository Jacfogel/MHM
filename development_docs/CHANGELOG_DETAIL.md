...
- **Impact**: Fixes all 12 CI-only test failures. Root cause pattern: tests that passed locally (where env vars/config files/Windows paths exist) but failed on CI (Linux, no credentials, no gitignored config). Also documented ~10 pre-existing ordering-dependent schedule v1/v2 schema flakes in TODO.md.
