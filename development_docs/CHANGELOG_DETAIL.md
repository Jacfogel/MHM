...
- **Fix (round 10 — 5 CI failures)**: Added `core.config.is_google_health_enabled()` (env at call time) for `HealthHandler._handle_connect`. `TestUserFactory.create_basic_user__update_index` retries lock acquisition with backoff and logs instead of raising on timeout; `create_basic_user__verify_creation` avoids duplicate index writes and accepts on-disk account files when index lookup lags under xdist.
