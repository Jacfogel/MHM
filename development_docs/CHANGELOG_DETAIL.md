...
- **Cleanup**: Moved scheduler-facing `InvalidTimeFormatError` and `load_and_localize_datetime()` ownership into [`core/time_utilities.py`](../core/time_utilities.py), and updated [`core/scheduler.py`](../core/scheduler.py) to import localization from the time utility module.
...
- **Policy fix**: Replaced direct datetime/formatting calls outside `core/time_utilities.py` in [`core/message_management.py`](../core/message_management.py), [`core/scheduler.py`](../core/scheduler.py), [`core/logger.py`](../core/logger.py), and [`communication/core/welcome_manager.py`](../communication/core/welcome_manager.py). Message retention now uses a canonical UTC-aware now helper; welcome tracking stores the machine-readable ISO timestamp as UTC-aware ISO.
...
- **Cleanup**: Retired ignored `scripts/cleanup_windows_tasks.py` as superseded by tracked scheduler maintenance cleanup in [`core/scheduler_maintenance.py`](../core/scheduler_maintenance.py), which is called from [`core/scheduler.py`](../core/scheduler.py) and covered by scheduler-maintenance tests.
...
- **Feature (scheduler split)**: [`SchedulerManager`](../core/scheduler.py) delegates task reminder scheduling, cleanup, selection, and delivery logic to [`core/scheduler_task_reminders.py`](../core/scheduler_task_reminders.py); weekly backup/log archival/stale wake-task cleanup to [`core/scheduler_maintenance.py`](../core/scheduler_maintenance.py); and common maintenance job registration to [`core/scheduler_jobs.py`](../core/scheduler_jobs.py).
...
- **Feature**: `SchedulerManager` delegates daily maintenance to [`core/scheduler_maintenance.py`](../core/scheduler_maintenance.py), orphaned task-reminder cleanup to [`core/scheduler_task_reminders.py`](../core/scheduler_task_reminders.py), and system job registration to [`core/scheduler_jobs.py`](../core/scheduler_jobs.py). [`core/service_requests.py`](../core/service_requests.py) `cleanup_test_message_requests` continues per-file removal on `OSError` so all flag files are attempted.
