...
### 2026-03-24 - Dev tools: import boundary; Tier 3 coverage contract; docs and portability
...
- **Documentation**: Removed unapproved standalone files (`EXCLUSION_RULES.md` earlier; `DEV_TOOLS_DEPENDENCIES.md` content folded into paired guides §2, plan §7.6). **§9 Directory taxonomy** (paired); `AI_DEVELOPMENT_TOOLS_GUIDE.md` relative links; `DIRECTORY_TREE.md` updated. `doc-sync` persists `development_tools/docs/jsons/analyze_documentation_sync_results.json` via `_persist_documentation_sync_aggregate_json()` (`tool_wrappers.py`) so standalone doc-sync matches audit output.
...
- **Tests & tooling**: Flaky cache-helper test stabilized (Windows fs + path). Added coverage for audit orchestration, `run_test_coverage`, and commands helpers. Lightweight `test_audit_status_updates.py` asserts the three status files land under `development_tools/` (black-box; `evaluate_lock_set` stub; mocked generators; `DISABLE_LOG_ROTATION`). `test_pyright_config_paths.py` smoke-checks Pyright configs. `max_lines_per_module` default 500 → 1000 in config + `development_tools_config.json`; plan V4 updated.
- **Impact**: Run `doc-sync` and `audit --full` as needed to refresh generated priorities and JSON; run dev-tools coverage script to refresh percentages after large tool edits.
