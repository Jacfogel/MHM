...
- **Feature (scheduler/service delivery boundary)**: Added [`core/delivery.py`](../core/delivery.py) with `MessageSendOutcome`, `SchedulerDeliveryPort`, and `ServiceRequestDeliveryPort` protocols. [`SchedulerManager`](../core/scheduler.py) now uses a narrow `delivery` surface while preserving the existing `communication_manager` alias for compatibility and simple startup wiring. [`core/scheduler_task_reminders.py`](../core/scheduler_task_reminders.py) calls the delivery port instead of assuming the full communication manager.
- **Maintenance (public delivery wrappers)**: Added public `get_recipient_for_service()` and `send_checkin_prompt()` wrappers on [`CommunicationManager`](../communication/core/channel_orchestrator.py) so [`core/service_requests.py`](../core/service_requests.py) can prefer public delivery-port methods and fall back to legacy private methods only for compatibility with existing mocks.
- **Planning / generated docs**: Marked [PLANS.md](PLANS.md) Phase 7 and Phase 8 complete. Regenerated function registry, module dependency, directory tree, unused-import, AI status/priority, and consolidated report artifacts after adding new dispatcher and delivery modules.
- **Audit cleanup**: Removed the unused check-in dispatcher alias from [`reminder_dispatcher.py`](../communication/reminders/reminder_dispatcher.py). Added `ERROR_HANDLING_EXCLUDE` comments to protocol stubs in [`core/delivery.py`](../core/delivery.py) because the executable implementations carry the actual error handling. Standard audit now reports 100% error-handling coverage and no `delivery.py` missing-handler priority.
- **Validation**: Ruff passed on touched files and whole-repo checks where run; `py_compile` passed for touched communication/core modules. Focused scheduler/service/reminder/orchestrator validation passed, including the new slim-delivery scheduler test. `python development_tools/run_development_tools.py docs`, `audit --quick`, and standard `audit` completed successfully. Earlier full-audit failures were unrelated network recovery behavior tests that call real network probing in the current environment.
