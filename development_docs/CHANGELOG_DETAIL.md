...
- **Audit hygiene**: Declared `nightly_result` on `_StubService` in `test_cli_interface.py` (clears Pyright warning). Ran `doc-fix --fix-ascii` and `doc-sync` on paired changelogs.
