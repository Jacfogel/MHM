...
- **Boundaries**: The catalog reads live parser intent names through [`ai/prompts/command_registry.py`](../ai/prompts/command_registry.py) and does not import communication handlers, execute actions, or write domain data. [`PRODUCT_AI_RESPONSE_INFLUENCE_AUDIT.md`](../ai/PRODUCT_AI_RESPONSE_INFLUENCE_AUDIT.md) now states that the AI action planner/executor remain the next slice; adapter and non-chat flow wiring are in place.
