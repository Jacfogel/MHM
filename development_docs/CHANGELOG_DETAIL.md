...
- **Fix (parallel flake)**: [`test_user_with_all_features`](../tests/integration/test_user_creation.py) retries schedule persistence after preferences flush (same pattern as [`TestUserFactory.create_user_with_schedules`](../tests/test_helpers/test_utilities/test_user_factory.py)); schedule `days` values use schema-valid capitalization (`Monday` not `monday`).
- **Impact**: Product AI now has a clearer single context source and capability metadata without creating messy action-execution layers prematurely; follow-up work can focus on tested action routing and prompt-category composition.
