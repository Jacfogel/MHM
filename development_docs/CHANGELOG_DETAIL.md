...
- **Service delegate cleanup (planned)**: Added [TODO.md](../TODO.md) task to remove thin `MHMService._check_test_message_requests__*` / `_check_reschedule_requests__*` wrappers in favor of direct `service_requests` usage, with test updates (clears remaining duplicate-function pair when done).
