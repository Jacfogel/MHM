...
- **Product AI foundation**: Added the canonical `AIContextEnvelope` in [`ai/context_service.py`](../ai/context_service.py), metadata-only action capability catalog in [`ai/action_catalog.py`](../ai/action_catalog.py), and named product-AI prompt-flow contracts in [`ai/prompt_flows.py`](../ai/prompt_flows.py). [`ai/context_builder.py`](../ai/context_builder.py) now adapts from the envelope to the existing `ContextData` shape, and [`ai/conversational_context/assembly.py`](../ai/conversational_context/assembly.py) phrases chat context from one envelope instead of loading domains directly through `user_context_manager`.
- **Boundaries**: The catalog reads live parser intent names through [`ai/command_registry.py`](../ai/command_registry.py) and does not import communication handlers, execute actions, or write domain data. [`PRODUCT_AI_RESPONSE_INFLUENCE_AUDIT.md`](../ai/PRODUCT_AI_RESPONSE_INFLUENCE_AUDIT.md) now states that action planner/executor work, `AIActionRequest -> ParsedCommand` routing, and full `PromptManager` category composition remain future slices.
...
- **Fix (identical test personalized messages, same session)**: Admin-panel test sends use `skip_ai_cache=True` on [`handle_message_sending()`](../communication/core/channel_orchestrator.py) so [`generate_personalized_message()`](../ai/chatbot.py) bypasses the response cache (5-minute TTL) and includes a variation token for fresh wording each click.
- **Fix (placeholder sign-offs, same session)**: Personalized system/user prompts forbid letter-style closings; [`strip_letter_signoffs()`](../ai/response_postprocess.py) strips trailing `Take care, [Your Name]` / `Best wishes, [Your Name]` from model output.
- **Fix (multi-draft model output, same session)**: [`keep_first_personalized_block()`](../ai/response_postprocess.py) keeps only the first `Hi/Hey/Dear` block when the model returns several complete messages in one reply.
- **Audit hygiene**: Declared `nightly_result` on `_StubService` in `test_cli_interface.py` (clears Pyright warning). Ran `doc-fix --fix-ascii` and `doc-sync` on paired changelogs. Regenerated function registry (`docs`); removed unused `get_recent_responses` from [`chatbot.py`](../ai/chatbot.py); `@handle_errors` + docstrings on [`_TestMessageRequestWorker`](../ui/ui_app_qt.py); marked `test_add_schedule_period` / `test_remove_schedule_period` as `no_parallel` (parallel schedules cache flake).
...
- **Feature**: Expanded mode-detection constants in [`ai/command_interpreter.py`](../ai/command_interpreter.py) (notebook/check-in/analytics keywords, natural-language task phrases); [`resources/prompts/command.txt`](../resources/prompts/command.txt) uses runtime-injection placeholder instead of a stale static action list
- **Feature**: Actionability guidance in [`ai/conversational_context/instructions.py`](../ai/conversational_context/instructions.py) for recent automated messages and disabled features
...
- **Feature**: [`is_automated_messages_enabled()`](../core/response_tracking.py); [`context_phraser.py`](../ai/conversational_context/context_phraser.py) gates recent automated sends and schedule details when automated messages are disabled; feature block lists all three flags
...
- **Feature**: Added [`analyze_recent_checkin_rows`](../ai/context_builder.py) and refactored [`ai/fallback_responses/checkin_summary.py`](../ai/fallback_responses/checkin_summary.py) to take [`ContextAnalysis`](../ai/context_builder.py) (removed parallel `CheckinMetrics` / `compute_checkin_metrics`); [`coordinator.py`](../ai/fallback_responses/coordinator.py) routes through the canonical analytics path.
...
- **Fix (legacy alignment)**: Registered the explicit [`_get_fallback_response`](../ai/chatbot.py) compatibility bridge in `DEPRECATION_INVENTORY.json`, widened legacy wording detection to catch `backwards compatibility`, unignored the canonical inventory file for version control, and fixed legacy analyzer cache invalidation so inventory changes refresh scan results. The generated legacy report now surfaces the chatbot fallback bridge under the legacy workflow instead of relying on facade/shim review.
