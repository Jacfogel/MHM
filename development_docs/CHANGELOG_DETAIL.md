...
- **Docs**: Refreshed [LIST_OF_LISTS.md](LIST_OF_LISTS.md) against live config/code: clarified `development_tools/config/development_tools_config.json` shorthand, removed stale "wrong inventory path" warning (code default is `config/jsons/DEPRECATION_INVENTORY.json`; live JSON correctly omits the key), documented guard trigger-keyword defaults vs optional config override, closed Section 12/Section 15 contradiction (all relocation items Done/Accepted), listed remaining deferred SSOT opportunities, fixed broken V6 Section 7.8 pointer.
...
- **Docs**: [LIST_OF_LISTS.md](LIST_OF_LISTS.md) Section 7c inventories cache dependency paths, suite profiles, static-analysis shards, audit tool matrix, tool cache inventory, backup policy, and Windows process-group scripts.
...
- **Fix**: Repointed V5 changelog links to [`AI_DEV_TOOLS_IMPROVEMENT_PLAN_V6.md`](../archive/AI_DEV_TOOLS_IMPROVEMENT_PLAN_V6.md); fixed V6 hrefs in [`AI_LEGACY_COMPATIBILITY_GUIDE.md`](../ai_development_docs/AI_LEGACY_COMPATIBILITY_GUIDE.md) and [`LIST_OF_LISTS.md`](LIST_OF_LISTS.md); replaced dead test-file links with plain backticks in [`CHANGELOG_DETAIL.md`](CHANGELOG_DETAIL.md). `doc-fix --convert-links` and `doc-sync` verified clean (`markdown_link_target_issues: 0`).
...
- **Chore (legacy scan)**: Dropped `legacy_scan_patterns.user_data_v1_runtime_adapters` from [DEPRECATION_INVENTORY.json](../development_tools/config/jsons/DEPRECATION_INVENTORY.json) (noisy heuristics); `fix_legacy_references.REQUIRED_LEGACY_PATTERN_KEYS` now requires `dashed_short_id_display` only. Recorded `legacy_scan_patterns_user_data_v1_runtime_adapters` in `removed_inventory`. Updated [AI_LEGACY_COMPATIBILITY_GUIDE.md](../ai_development_docs/AI_LEGACY_COMPATIBILITY_GUIDE.md) and [LIST_OF_LISTS.md](LIST_OF_LISTS.md). Regenerated [LEGACY_REFERENCE_REPORT.md](LEGACY_REFERENCE_REPORT.md).
...
- **Docs / doc-sync (2026-03-29 follow-up)**: `analyze_documentation_sync_results.json` - replaced bare `` `config.py` `` / shorthand in [development_docs/LIST_OF_LISTS.md](LIST_OF_LISTS.md) with `development_tools/config/config.py` where path-drift treated `development_docs/config.py` as missing; [ai/SYSTEM_AI_GUIDE.md](../ai/SYSTEM_AI_GUIDE.md) - CONFIGURATION_REFERENCE citations use `[text](../CONFIGURATION_REFERENCE.md)` without nested backticks (unconverted-link analyzer). Removed root `custom.md` (directory-tree test artifact); [.gitignore](../.gitignore) `/custom.md`.
...
- **Docs**: [development_docs/LIST_OF_LISTS.md](LIST_OF_LISTS.md) section 12b investigation queue; sections 3, 9b, and 13 updates for inventory and derived `default_docs`.
...
- **Docs**: [LIST_OF_LISTS.md](LIST_OF_LISTS.md) updated to mark `TOOL_GUIDE` as derived-from `_TOOLS`. Added consolidation principle (3) and Section 9a (criteria, workflow, candidates); fix_version_sync category lists documented as consolidation candidates.
