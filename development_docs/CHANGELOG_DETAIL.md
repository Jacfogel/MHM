...
- **Compatibility**: Kept temporary `core.*` bridge modules only for legacy public storage import paths and registered them in [`DEPRECATION_INVENTORY.json`](../development_tools/config/jsons/DEPRECATION_INVENTORY.json) as active bridges with removal criteria. [`core/_storage_bridge.py`](../core/_storage_bridge.py) centralizes bridge aliasing, quiet debug logging, and standardized `@handle_errors` behavior. Bridge re-exports now use explicit same-name aliases so Ruff treats them as intentional public compatibility exports.
...
- **Cleanup**: Retired ignored `scripts/utilities/user_data_cli.py` instead of migrating it. The user does not want the optional admin CLI; tracked [`core/user_data_operations.py`](../core/user_data_operations.py) remains the canonical owner for user-data backup, summary, index, and reference operations.
...
- **Refactor (module layout + docs)**: Renamed `core/user_data_v2.py` to [`core/user_data_v2_envelopes.py`](../core/user_data_v2_envelopes.py); `core/user_data_manager.py` to [`core/user_data_operations.py`](../core/user_data_operations.py); `core/schedule_management.py` to [`core/schedule_runtime.py`](../core/schedule_runtime.py); `core/user_data_schedule_defaults.py` to [`core/schedule_document_defaults.py`](../core/schedule_document_defaults.py). Merged `core/user_data_updates.py` into [`core/user_data_write.py`](../core/user_data_write.py). Added [USER_DATA_MODEL.md](../core/USER_DATA_MODEL.md) Section 0 (module map, tolerant vs strict validation, dependency leaf); [ARCHITECTURE.md](../ARCHITECTURE.md) and [AI_ARCHITECTURE.md](../ai_development_docs/AI_ARCHITECTURE.md) Section 2 point to it. `get_user_info_for_schedule_management` renamed to `get_user_info_for_schedule_runtime`. [DEPRECATION_INVENTORY.json](../development_tools/config/jsons/DEPRECATION_INVENTORY.json) records `core_user_data_module_renames_and_updates_merge_2026_05_01`.
- **Refactor (v2 schemas)**: Split shared v2 item primitives into [`core/user_data_v2_base.py`](../core/user_data_v2_base.py) (`SCHEMA_VERSION`, `ItemKind`, `SourceModel`, `BaseItemModel`, `generate_short_id`). Task persistence models moved to [`tasks/task_schemas.py`](../tasks/task_schemas.py); notebook/journal v2 models to [`notebook/notebook_schemas.py`](../notebook/notebook_schemas.py). Added [`tasks/task_validation.py`](../tasks/task_validation.py) `validate_tasks_v2_document` and [`notebook/notebook_validation.py`](../notebook/notebook_validation.py) `validate_notebook_v2_document`; [`core/user_data_v2_envelopes.py`](../core/user_data_v2_envelopes.py) delegates those document types and keeps check-in/message/delivery models plus `validate_v2_document`. Call sites import `SCHEMA_VERSION` / `generate_short_id` from `user_data_v2_base`; `core/error_handling.py` docstring aligned. [USER_DATA_MODEL.md](../core/USER_DATA_MODEL.md) Sections 2.5-2.6 updated; `core.user_data_v2_envelopes` still re-exports task/notebook models via `__all__` where applicable.
...
- **Feature**: Added canonical v2 user-data models and migration helpers, covering tasks, notebook entries, journal entries, check-ins, message templates, and delivery records. The long-lived schema models now live in `core/user_data_v2.py` (since split into [`core/user_data_v2_base.py`](../core/user_data_v2_base.py) and [`core/user_data_v2_envelopes.py`](../core/user_data_v2_envelopes.py)), while one-time migration transforms live in `scripts.user_data_migration`. Added an intentionally untracked `scripts/migrate_user_data_v2.py` utility for dry-run/write migration with backup creation and migration report output.
