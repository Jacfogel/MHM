...
- **Policy fix**: Replaced direct datetime/formatting calls outside `core/time_utilities.py` in [`core/message_management.py`](../core/message_management.py), [`core/scheduler.py`](../scheduler/manager.py), [`core/logger.py`](../core/logger.py), and [`communication/core/welcome_manager.py`](../communication/core/welcome_manager.py). Message retention now uses a canonical UTC-aware now helper; welcome tracking stores the machine-readable ISO timestamp as UTC-aware ISO.
...
- **Feature (message preview extraction)**: Removed the remaining `MHMService` test-message preview wrapper so predefined-message preview/content resolution lives in [`core/message_preview.py`](../core/message_preview.py), preserving the v2-aware message-loading path.
