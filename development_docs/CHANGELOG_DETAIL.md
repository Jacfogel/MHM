...
- **Fix (round 5 — 10 dev-tools CI failures)**: Complexity tests now pin thresholds at 100/200/300 (generic config defaults are 50/100/200). Config import-order tests load `development_tools_config.json.example` on CI; `load_external_config()` no longer wipes an already-loaded config when the default gitignored file is absent. Path-drift legacy-doc test supplies inline config. Shared logging tests ignore pytest `LogCaptureHandler`, reset before setup, and mock lock detection for defer rollover.
- **Impact**: Fixes all 11+1 CI failures from round 4 and 10 failures from round 5. Root cause pattern: gitignored config/fixtures, platform-specific filesystem behavior, and Windows-only test logic running on Linux CI.
