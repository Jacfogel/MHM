...
- **Impact**: Fixes all 15 CI-only test failures across two rounds. Root cause pattern: tests that passed locally (where env vars/config files/Windows paths exist) but failed on CI (Linux, no credentials, no gitignored config, no root write access). Also documented ~10 pre-existing ordering-dependent schedule v1/v2 schema flakes in TODO.md.
