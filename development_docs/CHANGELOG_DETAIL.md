...
- **Feature**: Removed additional backwards-compatibility paths aligned with v2-native milestones and `development_docs/LEGACY_REFERENCE_REPORT.md` triage: `core/message_management.archive_old_messages` no longer archives pre-v2 top-level `messages[]` sent logs; `_message_template_default_to_v2` and `_normalize_message_update_payload` require v2 template fields (`text`, nested `schedule`, `id`); `get_timestamp_for_sorting` uses `sent_at` only. `core/user_data_read.ensure_unique_ids` no longer promotes `message_id` to `id` (still drops stray `message_id` after assigning a canonical `id`). Check-in runtime uses `submitted_at` in `checkin_runtime_timestamp`, `_checkin_to_runtime_response`, and `get_checkins_by_days`; `_response_to_v2_checkin` no longer maps legacy top-level `timestamp` into `submitted_at`. `core/checkin_analytics`, `user/context_manager`, `core/user_data_manager` (sent-message analytics), and `ai/chatbot` recent-delivery context use the same canonical fields.
