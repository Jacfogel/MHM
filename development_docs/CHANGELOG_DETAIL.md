...
### 2026-03-24 - Dev Tools Option A: Coverage 60%+; flaky test fix; import-boundary integration; doc consolidation
- **Import-boundary check (Phase 2.1 + 2.2)**: Wired `analyze_dev_tools_import_boundaries` into audit Tier 1, tool_metadata, tool_wrappers, and report_generation (AI_STATUS, AI_PRIORITIES, CONSOLIDATED_REPORT). Created `development_tools/shared/time_helpers.py` and `development_tools/shared/error_helpers.py`; refactored ~17 files to replace `core.time_utilities` and `core.error_handling`; isolated `core.backup_manager` via `importlib.import_module()` in `commands.py`. Policy: `DEVELOPMENT_TOOLS_GUIDE.md` §8.5 and `AI_DEVELOPMENT_TOOLS_GUIDE.md` §8; `test_import_boundary_policy.py`. Zero violations after refactor.
- **Docs consolidation**: Removed standalone `EXCLUSION_RULES.md` (content in paired guides §8/§8.5). Updated `report_generation.py`, policy test strings, `DIRECTORY_TREE.md`, `AI_DEV_TOOLS_IMPROVEMENT_PLAN_V4.md` §7.1, `standard_exclusions.py` docstring.
...
- **Follow-up (Tier 3 strict)**: `test_shared_maintenance_scripts.test_run_timing_analysis_with_mocked_service` failed after adding `analyze_dev_tools_import_boundaries` to Tier 1—extended `_FakeService` with `run_analyze_dev_tools_import_boundaries`. Ruff B009: `mod.backup_manager` instead of `getattr(mod, "backup_manager")` in `commands.py`. Removed unused `sys` import from `test_import_boundary_policy.py`.
- **Impact**: Development tools now compliant with Option A (core.logger only); pushes dev-tools coverage toward 60%+; stabilizes intermittent cache-helper failures. Run `audit --quick` to verify import-boundary; run `python development_tools/tests/run_test_coverage.py --dev-tools-only --no-parallel --no-domain-cache` to verify baseline.
