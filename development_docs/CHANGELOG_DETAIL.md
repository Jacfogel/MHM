...
- **Impact**: Narrows real legacy surface area called out by `development_docs/LEGACY_REFERENCE_REPORT.md` pattern hits without changing email/message bridges, flow-state migration, or `OLD_FIELD_NAMES_BY_DOCUMENT` metadata in `core/user_data_v2.py`. On-disk migration and `_message_template_default_to_v2` / `ensure_unique_ids` still accept legacy template keys where required for unmigrated JSON and ID repair.
