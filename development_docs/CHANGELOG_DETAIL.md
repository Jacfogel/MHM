...
- **Product AI foundation**: Added the canonical `AIContextEnvelope` in [`ai/context/service.py`](../ai/context/service.py), metadata-only action capability catalog in [`ai/prompts/action_catalog.py`](../ai/prompts/action_catalog.py), and named product-AI prompt-flow contracts in [`ai/prompts/flows.py`](../ai/prompts/flows.py). [`ai/context/builder.py`](../ai/context/builder.py) adapts from the envelope to the existing `ContextData` shape, and [`ai/context/assembly.py`](../ai/context/assembly.py) phrases chat context from one envelope instead of loading domains directly through `user_context_manager`.
...
- **Feature**: Added [`analyze_recent_checkin_rows`](../ai/context/builder.py) and refactored [`ai/fallback_responses/checkin_summary.py`](../ai/fallback/checkin_summary.py) to take [`ContextAnalysis`](../ai/context/builder.py) (removed parallel `CheckinMetrics` / `compute_checkin_metrics`); [`coordinator.py`](../ai/fallback/coordinator.py) routes through the canonical analytics path.
