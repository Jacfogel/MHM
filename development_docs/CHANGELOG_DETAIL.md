...
- **Follow-up (copied policy)**: [`_schedule_user_jobs`](../scheduler/manager.py) is the one categories -> check-ins -> task-reminders path for both `schedule_all_users_immediately` and `schedule_new_user` (non-list categories now warn on both). [`_phrase_task_data`](../ai/context/phraser.py) / [`_phrase_schedule_details`](../ai/context/phraser.py) own AI prompt wording for disk and envelope assembly. Due-date conversation flow [`_parse_date_time_from_text`](../communication/message_processing/flows/task_flow.py) uses [`parse_relative_date`](../tasks/task_service.py) for the date part (Jan 31 next-month and `tonight` match create-task).
- **Impact**: Adding a simple notebook intent is a map row; list-create and profile sections are independently testable. User-visible parsing, profile text, custom-question dialog, backup retention, reminder times, command routing, per-user scheduling, and AI task/schedule wording stay the same except the due-date flow now follows create-task date phrases.
