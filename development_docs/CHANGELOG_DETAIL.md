...
### 2026-04-19 - Dev-tools static-check sharding, cache alignment, V5 §3.19/§3.21 plan, HOW_TO_RUN
- **Plan ([`AI_DEV_TOOLS_IMPROVEMENT_PLAN_V5.md`](../development_tools/AI_DEV_TOOLS_IMPROVEMENT_PLAN_V5.md))**: Documented **§3.19.2** implementation outline and backlog item **7** for **directory-scoped** static-check caching (per-shard signatures and partial reuse in `tool_wrappers` — not implemented yet). Clarified **§3.19.1** that sharded Ruff/Bandit **execution** does not change the **global** `_compute_source_signature` invalidation story. Added **§3.21** to trace large `issues=` log lines for `analyze_documentation`, `analyze_functions`, `analyze_package_exports`, and `analyze_function_registry` against **AI_PRIORITIES.md**, **AI_STATUS.md**, and **CONSOLIDATED_REPORT.md** and record why counts may not surface verbatim.
...
- **Operator docs**: [`HOW_TO_RUN.md`](../HOW_TO_RUN.md) — static-check cache invalidation and dependency paths (§5.1.x per current guide structure).
