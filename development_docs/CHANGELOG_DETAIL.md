...
- **Audit hygiene**: Restored accidental `DISCORD_BOT_TOKEN` / `DISCORD_APPLICATION_ID` removal in `bot.py`; full Ruff and Pyright clean. Fixed parallel test failures: Discord action-row limit 25 (was 5), `pytestmark` on `test_flow_command_helpers.py`, bot init tests. Added `@handle_errors` on `_attach_flow_suggestions`; flow helper docstrings; removed unused `_matches_other_entity_commands`. Regenerated function registry; ASCII doc-fix on changelogs. Flow audit follow-up: docstrings on task/note flow helpers; `# error_handling_exclude` on pure keyword predicates and mixin callbacks (per `AI_ERROR_HANDLING_GUIDE` section 7); removed thin-wrapper `is_task_flow_skip_all_message` / `is_task_flow_skip_question_message` facades in favor of `is_skip_all_message` / `is_skip_question_message`. Security/docs: pinned `msgpack>=1.2.1` (GHSA-6v7p-g79w-8964, transitive via CacheControl); `report_generation` emits backtick paths for gitignored audit JSON instead of markdown links to ephemeral targets.
