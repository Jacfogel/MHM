...
- **Legacy**: [`DiscordEventHandler`](../communication/communication_channels/discord/event_handler.py) marked deprecated-in-use in [`DEPRECATION_INVENTORY.json`](../development_tools/config/jsons/DEPRECATION_INVENTORY.json) (canonical path is bot extracted handlers). `get_discord_event_handler` logs `LEGACY COMPATIBILITY` on invocation.
- **Circular imports**: [`discord_handler_protocol.py`](../communication/communication_channels/discord/discord_handler_protocol.py) defines `DiscordHandlerHost` with full method docstrings; handler modules type against the protocol instead of importing [`bot.py`](../communication/communication_channels/discord/bot.py). [`analyze_error_handling.py`](../development_tools/error_handling/analyze_error_handling.py) excludes Protocol stubs with docstring+ellipsis bodies from missing-decorator reports. [`get_discord_event_handler`](../communication/communication_channels/discord/event_handler.py) reviewed and marked `# devtools: ignore[facade-shims]` while retained in deprecation inventory.
