...
- **Chore (legacy scan)**: Dropped `legacy_scan_patterns.user_data_v1_runtime_adapters` from [DEPRECATION_INVENTORY.json](development_tools/config/jsons/DEPRECATION_INVENTORY.json) (noisy heuristics); `fix_legacy_references.REQUIRED_LEGACY_PATTERN_KEYS` now requires `dashed_short_id_display` only. Recorded `legacy_scan_patterns_user_data_v1_runtime_adapters` in `removed_inventory`. Updated [AI_LEGACY_COMPATIBILITY_GUIDE.md](ai_development_docs/AI_LEGACY_COMPATIBILITY_GUIDE.md) and [LIST_OF_LISTS.md](development_docs/LIST_OF_LISTS.md). Regenerated [LEGACY_REFERENCE_REPORT.md](development_docs/LEGACY_REFERENCE_REPORT.md).
...
- **Feature**: Split one-time user-data migration transforms, report helpers, backup orchestration, and user-root writes into [`scripts/user_data_migration.py`](../scripts/user_data_migration.py), keeping migration tooling out of `core/` while long-lived schemas/validators stay in `core.user_data_v2`.
...
- **Feature**: Added canonical v2 user-data models and migration helpers, covering tasks, notebook entries, journal entries, check-ins, message templates, and delivery records. The long-lived schema models now live in [`core/user_data_v2.py`](../core/user_data_v2.py), while one-time migration transforms live in `scripts.user_data_migration`. Added an intentionally untracked `scripts/migrate_user_data_v2.py` utility for dry-run/write migration with backup creation and migration report output.
...
- **Validation**: Added focused migration and runtime-adoption tests in [`test_user_data_v2_migration.py`](../tests/unit/test_user_data_v2_migration.py). Verified with targeted pytest runs for v2 migration, task behavior, notebook behavior/validation, message management, and check-in analytics conversion tests.
...
- **Docs / ASCII**: [ai_development_docs/AI_CHANGELOG.md](ai_development_docs/AI_CHANGELOG.md) and [development_docs/CHANGELOG_DETAIL.md](development_docs/CHANGELOG_DETAIL.md) - replaced section sign and em dash with ASCII phrasing; [ai/SYSTEM_AI_GUIDE.md](ai/SYSTEM_AI_GUIDE.md) - links use `../...` paths from `ai/` and correct [tests/ai/SYSTEM_AI_FUNCTIONALITY_TESTING_GUIDE.md](tests/ai/SYSTEM_AI_FUNCTIONALITY_TESTING_GUIDE.md) target (removed erroneous `tests/tests/ai/` segment).
- **Docs / doc-sync (2026-03-29 follow-up)**: [analyze_documentation_sync_results.json](development_tools/docs/jsons/analyze_documentation_sync_results.json) - replaced bare `` `config.py` `` / shorthand in [development_docs/LIST_OF_LISTS.md](development_docs/LIST_OF_LISTS.md) with `development_tools/config/config.py` where path-drift treated `development_docs/config.py` as missing; [ai/SYSTEM_AI_GUIDE.md](ai/SYSTEM_AI_GUIDE.md) - CONFIGURATION_REFERENCE citations use `[text](../CONFIGURATION_REFERENCE.md)` without nested backticks (unconverted-link analyzer). Removed root `custom.md` (directory-tree test artifact); [.gitignore](.gitignore) `/custom.md`.
...
- **Docs**: [development_docs/LIST_OF_LISTS.md](development_docs/LIST_OF_LISTS.md) section 12b investigation queue; sections 3, 9b, and 13 updates for inventory and derived `default_docs`.
...
- **Legacy / deprecation inventory**: [development_tools/config/jsons/DEPRECATION_INVENTORY.json](development_tools/config/jsons/DEPRECATION_INVENTORY.json) `root_ruff_compat_mirror` uses empty `search_terms` while the Ruff root-compat bridge remains documented (`active_bridge`); removes docstring/help grep duplicates from [development_tools/legacy/analyze_legacy_references.py](development_tools/legacy/analyze_legacy_references.py) injection. Regenerated [development_docs/LEGACY_REFERENCE_REPORT.md](development_docs/LEGACY_REFERENCE_REPORT.md) (**0** files / **0** markers).
...
- **Docs**: [LIST_OF_LISTS.md](development_docs/LIST_OF_LISTS.md) updated to mark `TOOL_GUIDE` as derived-from `_TOOLS`. Added consolidation principle (3) and Section 9a (criteria, workflow, candidates); fix_version_sync category lists documented as consolidation candidates.
