...
### 2026-04-10 - Pyright warnings in verify_process_cleanup
- **Feature**: [verify_process_cleanup.py](../development_tools/tests/verify_process_cleanup.py) parses CIM JSON `ProcessId` without passing `int()` a `None` possibility from nested `dict.get` (fixes `reportArgumentType` warnings from Pyright under the repo analyzer).
- **Impact**: Tier 3 `analyze_pyright` can report **0 warnings** for that file after the next `audit --full`; **AI_PRIORITIES** should drop the Pyright follow-up item when diagnostics are clean.
- **Tests**: `python -m pyright development_tools/tests/verify_process_cleanup.py` (0 errors, 0 warnings).
