...
- **Import-boundary (Option A, Tier 1)**: Wired `analyze_dev_tools_import_boundaries`; added `time_helpers` / `error_helpers`; refactored imports off `core.time_utilities` / `core.error_handling`; `backup_manager` via `importlib` in `commands.py`. Policy in paired guides §8 / §8.5; `test_import_boundary_policy.py`. Follow-ups: `_FakeService` gained `run_analyze_dev_tools_import_boundaries`; Ruff B009 in `commands.py`; removed unused import in policy test.
...
- **V4 roadmap (`AI_DEV_TOOLS_IMPROVEMENT_PLAN_V4.md`)**: AI_STATUS/AI_PRIORITIES as authoritative snapshot refs. §1.1 (60% floor vs ~80% advisory), §1.3 **MONITORING**, skip-count reconciliation, test_config migration note deferred. §2.8 DEV_TOOLS report scoping **deferred**. §6 legacy workflow + live counts. §1.5 domain-cache `Measure-Command` methodology. §7.6 portability partial (Ruff tests). §2.9 spot-check: no raw dict/list dumps in `run_development_tools.py` / `audit_orchestration.py` print paths; progress indicators still open.
...
- **List consolidation (continued)**: Removed `file_patterns.exclude_patterns` from config and config.py. Moved `BASE_EXCLUDE_GLOBS` to `standard_exclusions.DOC_EXPORT_BASE_EXCLUDE_GLOBS`; export_docs_snapshot imports it. DOCUMENTATION_GUIDE §4.1 and AI_DOCUMENTATION_GUIDE §4.1 now state config is canonical for paired_docs.
- **Directory list consolidation**: `local_module_prefixes` is now the single canonical list. `scan_directories`, `core_modules`, and `project_directories` are derived in `constants.py` (exclusions: data, development_tools, notebook, scripts for scan; data, development_tools, scripts, tests for core; data, development_tools, scripts for project). Removed `paths.scan_directories`, `constants.project_directories`, and `constants.core_modules` from config; `config.get_scan_directories()` derives from `get_scan_directories_derived()` when config override absent. LIST_OF_LISTS §9a updated.
