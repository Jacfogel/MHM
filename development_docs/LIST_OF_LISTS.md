...
| **Behavior specifications (`specs/`)** | `development_tools_config.json` — `fix_version_sync.docs` includes `specs/SPECS_GUIDE.md` and topic files (for example `specs/discord-checkin-flow.md`, `specs/discord-welcome-and-onboarding.md`); `constants.fix_version_sync_directories` includes `specs/` | Derived into `DEFAULT_DOCS` via `_derive_default_docs_list()`; doc-sync, path-drift, heading/ASCII tools | Not paired (shared human + AI). Index: [specs/SPECS_GUIDE.md](../specs/SPECS_GUIDE.md) section 6 lists Discord specs. `path_drift.ignored_path_patterns` includes spec heading fragments. Use full project-root paths in spec body text (not bare `module.py` filenames). |
