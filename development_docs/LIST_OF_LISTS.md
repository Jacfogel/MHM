...
| **Tools that use standardized storage**                  | `development_tools/shared/verify_tool_storage.py` - `EXPECTED_TOOLS` (dict: tool_name -> domain)                                                     | Storage verification script; policy consistency tests                                | Every key ⊆ `_TOOLS` (enforced by `test_storage_and_cache_tool_names_subset_of_catalog`). |
...
| **Cache-aware tools (invalidation)** | `development_tools/shared/tool_metadata.py` - `CACHE_AWARE_TOOLS` (frozenset of tool names) | `audit_orchestration` (cache invalidation when those tools run) | Subset of `_TOOLS` (same policy test). `generate_dev_tools_coverage` is catalogued but excluded from `get_script_registry()` (service method; shares path with `run_test_coverage`). |

**Alignment note**: `_TOOLS` is the single source for "all known tools". EXPECTED_TOOLS and CACHE_AWARE_TOOLS must be subsets (policy test). SCRIPT_REGISTRY is derived from `_TOOLS` paths minus `_SCRIPT_REGISTRY_EXCLUDE`.

---

## 3. 2b. Audit tier tool lists (Tier 1 / 2 / 3 - which tools run in each audit tier)
...
**Other locations**: Consumers import from `audit_tiers` only. Flat `TIER*_TOOL_NAMES` are membership for expected/reporting; `TIER*_GROUP_MAP` drives orchestration order; `TIER*_ORCHESTRATION_OMIT` documents intentional non-scheduled members (e.g. Tier 1 `quick_status`, Tier 3 `analyze_test_markers` owned by the `coverage` command). Import-time validation requires groups ∪ omit == flat set.

---

## 4. Deprecation / legacy inventory


| What                                                                               | Canonical source                                                                                                                                                | Uses                                                                                                                                                                                                       | Other locations / overlap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Deprecation inventory (active/candidate/removed items, search terms, evidence)** | `development_tools/config/jsons/DEPRECATION_INVENTORY.json` - top-level keys: `active_or_candidate_inventory`, `removed_inventory`, `global_sweep_keywords`, `legacy_scan_patterns` (no `sources` key) | `development_tools/legacy/analyze_legacy_references.py` (loads inventory); `development_tools/legacy/generate_legacy_reference_report.py`; facade-shim / guard consumers | **Path default**: code resolves `legacy_cleanup.deprecation_inventory_file` or falls back to `development_tools/config/jsons/DEPRECATION_INVENTORY.json` (live JSON omits - correct). **Guard keywords**: code defaults in `tool_wrappers._check_deprecation_inventory_sync`; optional config override (live JSON omits). Distinct from inventory `global_sweep_keywords`. **Docs**: AI_LEGACY_COMPATIBILITY_GUIDE.md, AI_DEVELOPMENT_WORKFLOW.md, DEVELOPMENT_TOOLS_GUIDE.md. |
| **Legacy scan patterns + fixer knobs** | **Patterns**: inventory `legacy_scan_patterns` (optional config `legacy_cleanup.legacy_patterns` override). **Live MHM `legacy_cleanup`**: `required_pattern_keys`, `replacement_mappings` (path/guard/skip_extensions omitted - code defaults). | Analyzer merges inventory patterns + optional override + inventory terms; `fix_legacy_references` uses `replacement_mappings` / `required_pattern_keys` | Never add a second top-level `"legacy_cleanup"` key - `json.load` keeps only the last. |


**Alignment note**: Inventory path and guard keyword defaults live in code; omit them from JSON unless overriding. Keep `global_sweep_keywords` (inventory) and guard `trigger_keywords` (code/optional config) as separate purposes.

---

## 5. Paired documentation list


| What                                              | Canonical source                                                                                                                                                                                                              | Uses                                                                         | Other locations / overlap                                                                                                                                                                                            |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Paired docs (human doc -> AI doc mapping)**      | **Runtime**: `development_tools_config.json` - `constants.paired_docs` (object); loaded by `development_tools/shared/constants.py` as `PAIRED_DOCS`. **Human-facing**: [DOCUMENTATION_GUIDE.md](../DOCUMENTATION_GUIDE.md) Section 4.1 (bullet list). | `development_tools/docs/analyze_documentation_sync.py` (uses PAIRED_DOCS); doc-sync; heading checks | Config is canonical for tooling. A future generic list-sync mechanism (config-driven, multiple list types) could keep DOCUMENTATION_GUIDE Section 4.1 and other doc lists in sync. |
...
| **Pyright exclude**                                 | Root `pyproject.toml` - `[tool.pyright]` `exclude` array only.                                                                                                                                                                                                      | Pyright type checking                                    | **Single SSOT**: `[tool.pyright]` in `pyproject.toml` (nested `development_tools/config/pyrightconfig.json` removed). Update excludes here when changing Pyright scope.            |
...
| **Pytest collection ignores**                       | **Canonical collect lists**: `tests/conftest.py` - `collect_ignore`, `collect_ignore_glob`. **Also**: `pytest.ini` `addopts` `--ignore=...` for a few paths (e.g. scripts, two coverage scripts).                                                                                                                                  | Pytest collection                                        | Two mechanisms; conftest owns `collect_ignore*`; pytest.ini owns CLI `--ignore` / markers / addopts.                 |
...
## 8. Code-only lists in development_tools (align or pull from canonical)
...
|------|------|---------------------|--------|
| **Tool guide (usage, when_to_use, output_interpretation)** | `development_tools/shared/tool_guide.py` - `TOOL_GUIDE_OVERRIDES` (canonical guide-only fields) -> builds `TOOL_GUIDE_BY_TOOL_NAME` enriched from `tool_metadata.get_tool_metadata()`; `TOOL_GUIDE_BY_SCRIPT_BASENAME` derived from basenames | Path/tier/trust/description from `_TOOLS`; purpose/when_to_use/etc. only for tools listed in `TOOL_GUIDE_OVERRIDES` (not every `_TOOLS` entry). | Basename compat map + import-time collision guard. |
...
| **Required legacy pattern keys** | `development_tools_config.json` - `legacy_cleanup.required_pattern_keys` | Project-specific config; default empty so generic dev_tools stays portable. | MHM project lists `dashed_short_id_display`; missing keys produce a one-time inventory-drift warning in `fix_legacy_references` (`user_data_v1_runtime_adapters` removed from inventory scan patterns 2026-05-01, see `DEPRECATION_INVENTORY.json` `removed_inventory`). |
...
| **Static check allowlists/sets** | `development_tools/static_checks/check_channel_loggers.py` - `LOG_METHODS`, `IGNORED_DIR_NAMES` (generic, code-only); `excluded_dirs`, `allowed_logging_import_paths` via `get_static_check_channel_loggers_config()` -> JSON `static_checks.channel_loggers` + defaults in `development_tools/config/config.py` (`STATIC_CHECK_CHANNEL_LOGGERS_DEFAULT`). Allowed `get_component_logger` names are AST-parsed from `core/logger.py` (`CANONICAL_COMPONENT_NAMES` + `COMPONENT_NAME_ALIASES`) without importing `core`. | Config + `development_tools/config/config.py` merge for project-specific paths; component-name allowlist lives in `core/logger.py` (single source). | **Done** - script loads `development_tools/config/development_tools_config.json` via `REPO_ROOT` so cwd does not drift lists; unknown component names fail CI. |
...
## 9. 7b. Test and coverage lists (development_tools/tests)
...
| What | File | Canonical / overlap | Notes |
|------|------|---------------------|--------|
| **Source -> test dir/markers mapping** | `development_tools_config.json` - `domain_mapper.source_to_test_mapping` (domain -> [test_dirs[], markers[]]) | Config canonical; domain_mapper.py loads via `get_domain_mapper_config()`. | JSON lists core domains; runtime may augment (e.g. `integrations` when that package exists) - JSON alone is not always exhaustive. |
...
| **Domain keyword map** | `development_tools_config.json` - `domain_mapper.keyword_map` (domain -> keywords for matching) | Config canonical; domain_mapper.py loads via `get_domain_mapper_config()`. | development_tools, communication, ai, tasks, ui, user, notebook, scheduler, checkins, messages, storage, core. |
| **Product package roots (policy)** | `tests/development_tools/test_domain_mapper_product_packages.py` - `PRODUCT_PACKAGE_ROOTS` | Asserts every root package with `__init__.py` (except data/scripts/tests/dev_tools) appears in `local_module_prefixes` and `domain_mapper.source_to_test_mapping`. | Align after refactors that add top-level packages. |
...
## 10. 7c. Cache, suite, static-analysis, and backup policy lists

Operational lists that drive cache invalidation, Tier 3 pytest profiles, sharded static analysis, audit-matrix generation, and backup CLI policy. Distinct from Section 2 tool catalog and Section 7b domain/coverage test maps.

| What | Canonical source | Uses | Notes |
|------|------------------|------|-------|
| **Static-check cache config paths** | `development_tools/shared/cache_dependency_paths.py` - `STATIC_CHECK_CONFIG_RELATIVE_PATHS` | Hashed by `tool_wrappers` for Ruff/Pyright/Bandit caches; coverage caches import the same paths for alignment | Add a path here when a config change must bust static-check caches |
| **Coverage cache config paths** | same module - `COVERAGE_CONFIG_RELATIVE_PATHS` | Coverage invalidation (`coverage.ini`) | Keep in sync with Section 5 coverage omit files conceptually |
| **Pip-audit dependency paths** | same module - `PIP_AUDIT_DEPENDENCY_RELATIVE_PATHS` (`requirements.txt`, `pyproject.toml`) | `requirements_lock_signature()` for pip-audit JSON cache | |
| **Suite profiles / test-run defaults** | `development_tools/config/config.py` - `SUITE_PROFILES`, `TEST_RUN_DEFAULTS`; optional JSON `test_run` override via `get_test_run_config()` | Tier 3 `run_test_suite`, nightly suite | Profiles: `quick` (exclude e2e+slow), `full` (exclude e2e). Prefer code defaults; omit JSON when defaults match |
| **Ruff / Pyright path shards** | Project JSON `static_analysis.ruff_path_shards` / `pyright_path_shards` (MHM: `development_tools_config.json` + `.example`); code default `_DEFAULT_RUFF_PATH_SHARDS` is empty (monolithic fallback) | Sharded static-analysis subprocesses | Keep shard roots aligned with first-party packages in project config, not portable code defaults |
| **Audit tool matrix (generated)** | **Builders**: `development_tools/shared/audit_tool_matrix.py` - `build_audit_tool_matrix()` from `audit_tiers` + `_TOOLS` / script registry; override maps `_NON_AUDIT_REASON_OVERRIDES`, `_REPORT_HINT_OVERRIDES`. **Artifact**: `development_tools/config/audit_tool_matrix.json` | Reporting / cross-check which catalog tools participate in audits | Regenerate after tier or report-wiring changes; do not hand-edit the JSON as source of truth |
| **Tool cache inventory** | `development_tools/config/tool_cache_inventory.json` - `entries`, `phase2_cache_aware_followups`, `expansion_candidates`; helpers in `development_tools/shared/tool_cache_inventory.py` | Documents cache strategy / invalidation per tool | Overlaps conceptually with `CACHE_AWARE_TOOLS` (Section 2); inventory is descriptive, CACHE_AWARE is runtime |
| **Backup policy lists** | `development_tools_config.json` - `backup_policy.artifact_rules`, `categories`, `ownership_map`, `restore_drill` | Backup CLI (`inventory` / `retention` / `drill` / `verify` in Section 1) | Project-specific operational policy; keep in config |
| **Windows process-group scripts** | `development_tools/shared/service/tool_wrappers.py` - `_WIN_PROCESS_GROUP_SCRIPTS` | Isolates selected subprocesses on Windows | Small frozenset; keep in code (not project-specific paths) |

**Alignment note**: Cache path tuples belong in `development_tools/shared/cache_dependency_paths.py` (one place for invalidation). Suite profiles stay in `development_tools/config/config.py` defaults; first-party Ruff/Pyright/Bandit roots live in project JSON (portable code defaults are empty). Audit matrix JSON is an output of `development_tools/shared/audit_tool_matrix.py` + tier/catalog sources - edit the builders or tiers, then regenerate.

---

## 11. Other list-like assets
...
## 12. Alignment workflow (summary)

1. **Code/config first**: For each list type, fix canonical source and all code consumers (e.g. COMMAND_REGISTRY vs COMMAND_GROUPS, PAIRED_DOCS vs config). Prefer code defaults over duplicating correct paths/keywords in JSON.
2. **Audit tier tool lists**: Canonical source is `development_tools/shared/audit_tiers.py`; `measure_tool_timings.py` and `audit_orchestration.py` use it. Keep `verify_tool_storage.EXPECTED_TOOLS` as a subset of tools from `tool_metadata._TOOLS` / tier names. See Section 2b.
...
5. **Tooling**: Prefer structural tests (e.g. COMMAND_GROUPS ⊆ COMMAND_REGISTRY; guide section parity; inventory path defaults) over copying lists into docs. First **list-sync** slice: `doc-sync` flags DOCUMENTATION_GUIDE Section 4.1 vs `constants.paired_docs` drift.

---

## 13. 9a. Consolidation criteria and workflow
...
**Consolidation candidates** (see Section 4, Section 6, Section 10 for details):
...
## 14. 9b. Consolidation scan: standard_exclusions, constants, audit_tiers, development_tools_config
...
### 14.2. Consolidation opportunities
...
| **audit_tiers** group names vs flat TIER*_TOOL_NAMES | Groups are execution-order subsets | **Done** - `TIER*_GROUP_MAP` + omit sets; import-time coverage check |
...
## 15. Documentation-analysis and path-drift lists (candidates for canonical sources)
...
## 16. Other lists (config, functions, error_handling) - migration candidates
...
## 17. Lists that don't yet meet the principles
...
| **_CACHE_AWARE_TOOLS** | 2 | S / M | Was duplicated in audit_orchestration. | Move to `tool_metadata`; consumers import. | **Fixed**: `tool_metadata.CACHE_AWARE_TOOLS`; ⊆ `_TOOLS` (policy test). |
| **EXPECTED_TOOLS** vs **_TOOLS** | 2 | S | Storage catalog keys should match tool catalog names. | Rename/align keys; catalogue logical tools. | **Fixed (2026-07-18)**: `system_signals` -> `analyze_system_signals`; `generate_dev_tools_coverage` added to `_TOOLS` (script-registry excluded). |
| **Trigger keywords** (guard) vs **DEPRECATION_INVENTORY global_sweep_keywords** | 3 | S | Two sources for "keywords that trigger guard"; overlap. | Code defaults for guard; optional config override; inventory `global_sweep_keywords` for sweep scope. | **Fixed**: Guard uses code defaults + optional `trigger_keywords` override; purposes remain separate. |
| **DOCUMENTATION_GUIDE Section 4.1** (paired doc list in prose) | 4 | S | Doc list can drift from config `constants.paired_docs`. | Doc points to config only, or add tooling to sync from config. | **Fixed**: Section 4.1 now states config is canonical; list is human-readable summary; doc-sync validates. |
...
| **TOOL_GUIDE** (tool_guide.py) | 7 | S | Was keyed by script filename and drifted from the canonical `_TOOLS` catalog. | Guide fields from `TOOL_GUIDE_OVERRIDES`; enrich/validate against `_TOOLS`; basename compat map. | **Fixed**: overrides-driven guide + `_TOOLS` enrichment; not every catalog tool has a guide entry. |
...
**Summary (2026-07-18)**: Historical rows are **Fixed** or optional. Catalog subset invariant restored (`EXPECTED_TOOLS` / `CACHE_AWARE_TOOLS` ⊆ `_TOOLS`, policy test). Doc-sync placeholders remain optional. Guard keywords use code defaults + optional override. Duplicate `legacy_cleanup` JSON key fixed.

---

## 18. 12b. Config blocks - value vs complexity (investigation queue)
...
## 19. Config list inventory (consolidation scan)

Config keys that are list-like or array-valued. Use for consolidation audits.

| Config path | Purpose | Overlap / notes |
|-------------|---------|-----------------|
| `project.key_files` | Entry points, important files | Overlaps `core_system_files`; see Section 9a |
...
| `constants.default_docs` | Default docs for analysis (often **derived** in `get_constants_config()`) | Same union as paired + version-sync lists + `default_docs_extra`; see Section 12b |
...
| `backup_policy.artifact_rules` / `categories` / `ownership_map` / `restore_drill` | Backup CLI policy | See Section 7c; project-specific |
| `test_run` (optional in JSON) | Suite profile / pytest runner overrides | Defaults: `SUITE_PROFILES` / `TEST_RUN_DEFAULTS` in `development_tools/config/config.py`; see Section 7c |
| `static_analysis.ruff_path_shards` / `pyright_path_shards` (optional) | Sharded static-analysis roots | Empty in portable code defaults; MHM sets package trees in project JSON; see Section 7c |
...
| Add a paired doc | `development_tools_config.json` - `constants.paired_docs`; update DOCUMENTATION_GUIDE.md Section 4.1 |
...
| Bust static-check / coverage caches when a config path changes | `development_tools/shared/cache_dependency_paths.py` - add to `STATIC_CHECK_CONFIG_RELATIVE_PATHS` or `COVERAGE_CONFIG_RELATIVE_PATHS` |
| Change Tier 3 pytest exclude markers / suite profile | `development_tools/config/config.py` - `SUITE_PROFILES` / `TEST_RUN_DEFAULTS` (optional JSON `test_run`) |
| Change Ruff/Pyright shard roots | Project `development_tools_config.json` `static_analysis.*_path_shards` (keep `.example` aligned); portable code default stays empty |
| Regenerate audit tool matrix | Update `audit_tiers` / override maps in `development_tools/shared/audit_tool_matrix.py`, then regenerate JSON artifact |
| Edit backup retention / restore-drill policy | `development_tools_config.json` - `backup_policy.*` |

---

## 21. Next action steps (list relocation)

Historical relocation checklist - **all items Done or Accepted**. Kept for audit trail; do not reopen unless a regression appears.

| # | Item | Action | Status |
|---|------|--------|--------|
| 1 | **Doc-sync placeholders** | Leave in `development_tools/shared/standard_exclusions.py`; no action needed | **Accepted** |
| 2 | **Static check allowlists** (check_channel_loggers) | `excluded_dirs` / `allowed_logging_import_paths` -> config `static_checks.channel_loggers` | **Done** |
| 2a | | `LOG_METHODS` and `IGNORED_DIR_NAMES` stay in code (not project-specific) | **Accepted** |
...
| 4 | **generate_function_registry report lists** | Portable defaults in `AUDIT_FUNCTION_REGISTRY`; MHM `decision_trees` in JSON | **Done** |
| 5 | **fix_function_docstrings special_method_docs** | Single-use; no shared consumers | **Accepted** |
| 6 | **analyze_function_patterns patterns** | Defaults in `development_tools/config/config.py`; optional JSON override (live MHM omits) | **Done** |
| 7 | **exclusions.base_exclusions merge strategy** | `base_exclusions_additions` / `base_exclusions_removals` | **Done** |

### 21.1. Remaining SSOT opportunities

| Opportunity | Severity | When to act |
|-------------|----------|-------------|
| **Align EXPECTED_TOOLS / CACHE_AWARE_TOOLS with `_TOOLS`** | ~~High~~ **Done** | Renamed `system_signals` -> `analyze_system_signals`; catalogued `generate_dev_tools_coverage` |
| **Document cache/suite policy lists** | ~~Completeness~~ **Done** | Section 7c |
| **Generic list-sync** (config -> docs) | ~~Deferred~~ **Done (first slice)** | `doc-sync` compares DOCUMENTATION_GUIDE Section 4.1 prose to `constants.paired_docs` (`paired_doc_list_drift`) |
| **Product/runtime lists** | ~~Out of scope~~ **Done** | Sibling [PRODUCT_LIST_OF_LISTS.md](PRODUCT_LIST_OF_LISTS.md) |
| **CLI/help prose in guides** vs `COMMAND_REGISTRY` / `COMMAND_GROUPS` | ~~Low~~ **Done** | Name parity + `test_command_groups_match_development_tools_guide_sections` |
| **`audit_tiers` group structure** vs flat `TIER*_TOOL_NAMES` | ~~Defer~~ **Done** | `TIER*_GROUP_MAP` + `TIER*_ORCHESTRATION_OMIT`; import-time coverage validation |
