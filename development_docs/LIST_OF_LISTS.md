...
**Last updated**: 2026-05-16 (path-drift: fully qualified dev-tools paths in summary line and static-check table; static check `channel_loggers`: no duplicate allowlist in `development_tools/static_checks/check_channel_loggers.py`; canonical JSON + `get_static_check_channel_loggers_config()`)
...
| What | File | Canonical / overlap | Notes |
|------|------|---------------------|--------|
| **Source → test dir/markers mapping** | `development_tools_config.json` — `domain_mapper.source_to_test_mapping` (domain → [test_dirs[], markers[]]) | Config canonical; domain_mapper.py loads via `get_domain_mapper_config()`. | core, communication, ui, tasks, ai, user, notebook, scheduler, checkins, messages, storage, development_tools. |
