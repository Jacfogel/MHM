...
| **Ruff / Pyright path shards** | `development_tools/config/config.py` - `_DEFAULT_RUFF_PATH_SHARDS` copied into `STATIC_ANALYSIS.ruff_path_shards` / `pyright_path_shards` | Sharded static-analysis subprocesses | Optional JSON `static_analysis` override; keep shard roots aligned with first-party packages |
...
**Alignment note**: Cache path tuples belong in `development_tools/shared/cache_dependency_paths.py` (one place for invalidation). Suite profiles and shards stay in `development_tools/config/config.py` defaults unless a project needs JSON overrides. Audit matrix JSON is an output of `development_tools/shared/audit_tool_matrix.py` + tier/catalog sources - edit the builders or tiers, then regenerate.
...
| `static_analysis.ruff_path_shards` / `pyright_path_shards` (optional) | Sharded static-analysis roots | Defaults from `_DEFAULT_RUFF_PATH_SHARDS`; see Section 7c |
...
| Change Ruff/Pyright shard roots | `development_tools/config/config.py` - `_DEFAULT_RUFF_PATH_SHARDS` / `STATIC_ANALYSIS` shards |
