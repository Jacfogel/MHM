...
| **Required legacy pattern keys** | `development_tools/legacy/fix_legacy_references.py` — `REQUIRED_LEGACY_PATTERN_KEYS` | Fixed schema. | As of 2026-05-01: `dashed_short_id_display` only; `user_data_v1_runtime_adapters` removed from inventory scan patterns (see `DEPRECATION_INVENTORY.json` `removed_inventory`). |
