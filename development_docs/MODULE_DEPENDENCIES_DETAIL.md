...
- **Dependencies**:
  - **Local**:
    - `ai.chat.action_planner (ActionPlanner, get_action_planner, parse_action_plan_from_text)` (NEW)
    ...
    - `ai.fallback (FallbackCategory, FallbackResponses, build_contextual_fallback, get_fallback_responses)` (NEW)
    - `ai.prompts.action_catalog (AIActionCatalog, AIActionDefinition, AIActionField, AIActionPlan, AIActionRequest, build_action_catalog, get_action_catalog)` (NEW)
    - `ai.prompts.command_interpreter (CommandInterpreter, get_command_interpreter)` (NEW)
    - `ai.prompts.command_registry (format_command_actions_for_prompt, get_command_intent_names, inject_command_actions_into_prompt)` (NEW)
    - `ai.prompts.flows (PRODUCT_AI_PROMPT_FLOWS, ProductAIPromptFlow, get_product_ai_prompt_flow)` (NEW)
    - `ai.prompts.manager (PromptManager, PromptTemplate, get_prompt_manager)` (NEW)
- **Used by**: None (not imported by other modules)

**Dependency Changes**:
- Added: ai.chat.action_planner, ai.chat.chatbot, ai.chat.interaction_types, ai.chat.response_generator, ai.client.cache_manager, ai.client.lm_studio_manager, ai.context.analytics, ai.context.assembly, ai.context.history, ai.context.service, ai.fallback, ai.prompts.action_catalog, ai.prompts.command_interpreter, ai.prompts.command_registry, ai.prompts.flows, ai.prompts.manager
...
- **Dependencies**:
  - **Local**:
    - `ai.chat.action_boundaries (FALSE_CRUD_SUCCESS_SUBSTRINGS, find_false_crud_claims, response_has_false_crud_claim)` (NEW)
    - `ai.chat.action_planner (ActionPlanner, answer_only_plan, clarify_plan, get_action_planner, parse_action_plan_from_text)` (NEW)
    - `ai.chat.chatbot (AIChatBotSingleton, get_ai_chatbot)` (NEW)
    - `ai.chat.interaction_types (AIInteractionType, interaction_type_for_mode)` (NEW)
    - `ai.chat.response_generator (ResponseGenerator, get_response_generator)` (NEW)
...
**Dependency Changes**:
- Added: ai.chat.action_boundaries, ai.chat.action_planner, ai.chat.chatbot, ai.chat.interaction_types, ai.chat.response_generator, ai.chat.response_postprocess
...
- Removed: ai/chat/__init__.py, ai/chat/chatbot.py, ai/chat/response_postprocess.py
...
- **Purpose**: Communication channel implementation for action_planner
- **Dependencies**:
  - **Local**:
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.health_context_builder (PERSONALIZED_WELLNESS_CONTEXT_MAX_CHARS, build_personalized_checkin_context, build_personalized_google_health_context, build_personalized_profile_context)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: ai/__init__.py, ai/chat/__init__.py, ai/chat/chatbot.py
...
**Dependency Changes**:
- Added: ai.context.assembly, ai.prompts.manager, core.error_handling, core.logger
- Removed: ai/__init__.py, ai/chat/__init__.py, ai/chat/chatbot.py
...
**Dependency Changes**:
- Added: ai.client.cache_manager, ai.client.lm_studio_client, ai.client.lm_studio_manager
...
- Removed: ai/__init__.py, ai/chat/chatbot.py, ai/client/__init__.py
...
**Dependency Changes**:
- Added: ai.context.analytics, ai.context.assembly, ai.context.history, ai.context.service
...
- **Purpose**: Communication channel implementation for history
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
...
- Removed: ai/__init__.py, ai/context/__init__.py
...
#### `ai/context/phraser.py`
- **Purpose**: Communication channel implementation for phraser
- **Dependencies**:
  - **Local**:
...
#### `ai/context/service.py`
- **Purpose**: Communication channel implementation for service
- **Dependencies**:
  - **Local**:
    ...
    - `core.time_utilities (DATE_ONLY, TIME_ONLY_MINUTE, format_datetime_for_ai_prompt, format_timestamp, now_timestamp_full)` (NEW)
    - `messages.message_data_manager (get_recent_messages, is_automated_messages_enabled, load_user_messages)` (NEW)
    - `notebook (notebook_service)` (NEW)
    - `scheduler.user_timezone (localized_now_for_user, resolve_user_timezone_str)` (NEW)
    - `tasks (are_tasks_enabled, task_service)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `dataclasses (dataclass, field)`
    - `typing (Any)`
...
- **Dependencies**:
  - **Local**:
    ...
    - `ai.fallback.coordinator (build_contextual_fallback)` (NEW)
    - `ai.fallback.data_access (get_recent_responses, get_user_data)` (NEW)
    - `ai.fallback.personalized (build_personalized_message)` (NEW)
    - `ai.fallback.profile_helpers (personalize_with_profile_name)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
- **Used by**: None (not imported by other modules)

**Dependency Changes**:
- Added: ai.fallback.categories, ai.fallback.context, ai.fallback.coordinator, ai.fallback.data_access, ai.fallback.personalized, ai.fallback.profile_helpers, core.error_handling, core.logger
...
- **Purpose**: Communication channel implementation for action_hints
- **Dependencies**:
  - **Local**:
    - `ai.fallback.categories (FallbackCategory)` (NEW)
    - `ai.fallback.context (FallbackContext)` (NEW)
    - `ai.prompts.action_catalog (get_action_catalog)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
...
**Dependency Changes**:
- Added: ai.fallback.categories, ai.fallback.context, ai.prompts.action_catalog, core.error_handling
...
**Dependency Changes**:
- Removed: ai/fallback/__init__.py, ai/fallback/action_hints.py, ai/fallback/checkin_summary.py, ai/fallback/conversational.py, ai/fallback/coordinator.py, ai/fallback/envelope_summaries.py, ai/fallback/personalized.py
...
- Removed: ai/fallback/__init__.py, ai/fallback/coordinator.py, ai/fallback/personalized.py, ai/fallback/profile_helpers.py
...
#### `ai/fallback/envelope_summaries.py`
- **Purpose**: Communication channel implementation for envelope_summaries
- **Dependencies**:
  - **Local**:
    - `ai.fallback.categories (FallbackCategory)` (NEW)
...
- **Dependencies**:
  - **Local**:
    ...
    - `ai.fallback.profile_helpers (name_prefix_from_context)` (NEW)
    - `core.error_handling (ValidationError, handle_errors)` (NEW)
    - `core.health_context_builder (build_user_facing_signal_wellness_snippet)` (NEW)
- **Used by**:
  - `ai/fallback/__init__.py`

**Dependency Changes**:
- Added: ai.fallback.categories, ai.fallback.data_access, ai.fallback.profile_helpers, core.error_handling, core.health_context_builder
- Removed: ai/fallback/__init__.py
...
**Dependency Changes**:
- Added: ai.fallback.data_access, core.error_handling
- Removed: ai/chat/chatbot.py, ai/fallback/__init__.py, ai/fallback/context.py, ai/fallback/coordinator.py, ai/fallback/personalized.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `ai.prompts.command_interpreter (CommandInterpreter, get_command_interpreter)` (NEW)
    - `ai.prompts.command_registry (format_command_actions_for_prompt, get_command_intent_names, inject_command_actions_into_prompt)` (NEW)
    - `ai.prompts.flows (PRODUCT_AI_PROMPT_FLOWS, ProductAIPromptFlow, RUNTIME_PROMPT_CATEGORIES, get_product_ai_prompt_flow)` (NEW)
    - `ai.prompts.manager (PromptManager, PromptTemplate, get_prompt_manager)` (NEW)
- **Used by**: None (not imported by other modules)

**Dependency Changes**:
- Added: ai.prompts.action_catalog, ai.prompts.command_interpreter, ai.prompts.command_registry, ai.prompts.flows, ai.prompts.manager
...
- **Purpose**: Communication channel implementation for action_catalog
- **Dependencies**:
  - **Local**:
    - `ai.prompts.command_registry (get_initialized_command_intent_names)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `dataclasses (dataclass, field)`
...
**Dependency Changes**:
- Added: ai.prompts.command_registry, core.error_handling
- Removed: ai/__init__.py, ai/chat/action_planner.py, ai/context/service.py, ai/fallback/action_hints.py, ai/prompts/__init__.py, ai/prompts/command_interpreter.py, communication/message_processing/action_plan_executor.py, communication/message_processing/action_request_adapter.py
...
**Dependency Changes**:
- Added: ai.context.service, ai.prompts.action_catalog, ai.prompts.manager, core.error_handling
- Removed: ai/__init__.py, ai/chat/action_planner.py, ai/chat/chatbot.py, ai/prompts/__init__.py
...
#### `ai/prompts/flows.py`
- **Purpose**: Communication channel implementation for flows
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
  - **Standard Library**:
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: ai/__init__.py, ai/context/assembly.py, ai/prompts/__init__.py, ai/prompts/manager.py
...
#### `ai/prompts/manager.py`
- **Purpose**: Communication channel implementation for manager
- **Dependencies**:
  - **Local**:
    - `ai.prompts.command_registry (inject_command_actions_into_prompt)` (NEW)
    - `ai.prompts.flows (RUNTIME_PROMPT_CATEGORIES, get_product_ai_prompt_flow)` (NEW)
...
- Removed: checkins/checkin_analytics.py, communication/message_processing/flows/checkin_flow.py, core/web_user_settings.py, ui/widgets/checkin_settings_widget.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.error_handling (handle_errors)` (NEW)
    - `core.time_utilities (parse_timestamp_full)` (NEW)
    - `scheduler.user_timezone (user_local_date)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `dataclasses (dataclass)`
    - `typing (Any)`
...
- Removed: command_handlers.analytics_handler, command_handlers.base_handler, command_handlers.checkin_handler, command_handlers.interaction_handlers, command_handlers.profile_handler, command_handlers.schedule_handler, command_handlers.shared_types, command_handlers.task_handler, communication_channels.base.base_channel, communication_channels.base.command_registry, communication_channels.base.message_formatter, communication_channels.base.rich_formatter, communication_channels.discord.api_client, communication_channels.discord.bot, communication_channels.discord.events.status, communication_channels.email.bot, message_processing.command_parser, message_processing.conversation_flow_manager, message_processing.interaction_manager
...
- Removed: communication/command_handlers/account_handler.py, communication/command_handlers/analytics_handler.py, communication/command_handlers/checkin_handler.py, communication/command_handlers/create_menu_handler.py, communication/command_handlers/handler_registry.py, communication/command_handlers/health_handler.py, communication/command_handlers/interaction_handlers.py, communication/command_handlers/natural_language_handler.py, communication/command_handlers/notebook_handler.py, communication/command_handlers/profile_handler.py, communication/command_handlers/schedule_handler.py
...
**Dependency Changes**:
- Added: communication.command_handlers.base_handler, communication.command_handlers.shared_types, core.error_handling
...
**Dependency Changes**:
- Added: communication.command_handlers.base_handler, communication.command_handlers.shared_types, core.config, core.error_handling, core.logger
...
#### `communication/command_handlers/natural_language_handler.py`
- **Purpose**: Communication channel implementation for natural_language_handler
- **Dependencies**:
  - **Local**:
    - `communication.command_handlers.base_handler (InteractionHandler)` (NEW)
    - `communication.command_handlers.shared_types (InteractionResponse, ParsedCommand)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.natural_language_defaults (apply_natural_language_defaults_update, format_natural_language_defaults_message, get_natural_language_defaults)` (NEW)
  - **Standard Library**:
    - `typing (Any)`
- **Used by**: None (not imported by other modules)

**Dependency Changes**:
- Added: communication.command_handlers.base_handler, communication.command_handlers.shared_types, core.error_handling, core.logger, core.natural_language_defaults
...
- **Dependencies**:
  - **Local**:
    ...
    - `communication.message_processing.conversation_flow_manager (conversation_manager)`
    - `communication.message_processing.flows.flow_constants (ENTRY_EDIT_SUGGESTIONS, JOURNAL_BODY_SUGGESTIONS, LIST_ITEMS_SUGGESTIONS, NOTEBOOK_BODY_SUGGESTIONS)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
...
**Dependency Changes**:
- Removed: communication/command_handlers/account_handler.py, communication/command_handlers/analytics_handler.py, communication/command_handlers/base_handler.py, communication/command_handlers/checkin_analytics_handler.py, communication/command_handlers/checkin_handler.py, communication/command_handlers/create_menu_handler.py, communication/command_handlers/health_handler.py, communication/command_handlers/interaction_handlers.py, communication/command_handlers/natural_language_handler.py, communication/command_handlers/notebook_handler.py, communication/command_handlers/profile_handler.py, communication/command_handlers/schedule_handler.py, communication/command_handlers/task_analytics_handler.py, communication/command_handlers/task_handler.py, communication/command_handlers/trend_analytics_handler.py, communication/communication_channels/discord/events/interaction_router.py, communication/communication_channels/discord/onboarding/account_flow_handler.py, communication/communication_channels/discord/ui/create_item_ui.py, communication/communication_channels/discord/ui/helpers.py, communication/communication_channels/discord/ui/task_list_ui.py, communication/message_processing/action_plan_executor.py, communication/message_processing/action_request_adapter.py, communication/message_processing/command_parser.py, communication/message_processing/email_reply_routing.py, communication/message_processing/flow_message_dispatcher.py, communication/message_processing/flows/checkin_flow.py, communication/message_processing/help_responses.py, communication/message_processing/interaction_manager.py, communication/message_processing/parsing_shortcuts.py, communication/message_processing/prefix_command_processor.py, communication/message_processing/response_enhancer.py, communication/message_processing/structured_command_dispatcher.py, communication/message_processing/user_suggestions.py
...
- **Dependencies**:
  - **Local**:
    - `communication.command_handlers.shared_types (PaginationAction)` (NEW)
...
**Dependency Changes**:
- Added: core.error_handling, core.logger
- Removed: communication/communication_channels/discord/bot.py, communication/communication_channels/discord/events/connection_health.py, communication/communication_channels/discord/events/lifecycle.py, communication/communication_channels/discord/events/protocol.py, communication/communication_channels/email/bot.py, communication/core/channel_monitor.py, communication/core/channel_orchestrator.py, communication/core/factory.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `communication.communication_channels.discord.events.message_reactions (handle_message_reaction)` (NEW)
...
**Dependency Changes**:
- Added: communication.communication_channels.discord.events.command_registration, communication.communication_channels.discord.events.connection_health, communication.communication_channels.discord.events.interaction_router, communication.communication_channels.discord.events.lifecycle, communication.communication_channels.discord.events.message_handler, communication.communication_channels.discord.events.message_reactions, communication.communication_channels.discord.events.status, communication.communication_channels.discord.ui.rich_delivery, communication.communication_channels.discord.webhooks.tunnel, core, core.config, core.error_handling, core.logger
- Removed: communication/communication_channels/discord/ui/create_item_ui.py, communication/communication_channels/discord/ui/helpers.py, communication/communication_channels/discord/ui/task_list_ui.py, discord.ext
...
- Removed: collections.abc, communication/communication_channels/discord/bot.py, dns.resolver
...
- **Dependencies**:
  - **Local**:
    ...
    - `communication.communication_channels.discord.ui.create_item_ui (handle_create_hub_modal_submit)` (NEW)
...
**Dependency Changes**:
- Added: communication.command_handlers.interaction_handlers, communication.command_handlers.shared_types, communication.communication_channels.discord.events.protocol, communication.communication_channels.discord.onboarding.account_flow_handler, communication.communication_channels.discord.onboarding.welcome_handler, communication.communication_channels.discord.ui.create_item_ui, communication.message_processing.interaction_manager, core, core.error_handling, core.logger
- Removed: communication/communication_channels/discord/bot.py
...
- **Purpose**: Communication channel implementation for message_reactions
- **Dependencies**:
  - **Local**:
    - `communication.communication_channels.discord.events.protocol (DiscordHandlerHost)` (NEW)
    - `core (get_user_id_by_identifier)` (NEW)
...
**Dependency Changes**:
- Added: communication.communication_channels.discord.events.protocol, core, core.error_handling, core.logger, messages.message_reactions
...
- Removed: communication/communication_channels/discord/events/command_registration.py, communication/communication_channels/discord/events/interaction_router.py, communication/communication_channels/discord/events/lifecycle.py, communication/communication_channels/discord/events/message_handler.py, communication/communication_channels/discord/events/message_reactions.py, discord.ext
...
**Dependency Changes**:
- Removed: communication/communication_channels/discord/bot.py, communication/communication_channels/discord/events/connection_health.py, communication/communication_channels/discord/events/lifecycle.py, communication/communication_channels/discord/events/protocol.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `communication.communication_channels.discord.ui.task_reminder_view (get_task_reminder_view, get_task_simplify_view, get_task_snooze_choice_view)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `asyncio`
...
- Removed: communication/communication_channels/discord/events/interaction_router.py, communication/communication_channels/discord/onboarding/welcome_handler.py
...
- Removed: communication/communication_channels/discord/events/interaction_router.py, communication/communication_channels/discord/events/message_handler.py, communication/communication_channels/discord/webhooks/handler.py
...
- Removed: communication/communication_channels/discord/events/interaction_router.py, communication/communication_channels/discord/interaction_views.py
...
- Removed: communication/communication_channels/discord/ui/create_item_ui.py, communication/communication_channels/discord/ui/task_list_ui.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `tasks (get_task_by_id, task_service)` (NEW)
    - `tasks.task_data_handlers (runtime_task_due_date)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `typing (Any, TYPE_CHECKING)`
...
- Removed: communication/communication_channels/discord/webhooks/server.py, concurrent.futures
...
- Removed: communication/communication_channels/discord/webhooks/tunnel.py, http.server, nacl.exceptions, nacl.signing
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: communication/communication_channels/email/bot.py, communication/communication_channels/email/inbound_processor.py
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: communication/communication_channels/discord/ui/rich_delivery.py, communication/reminders/checkin_prompt_dispatcher.py, communication/reminders/reminder_dispatcher.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core (get_user_data)` (NEW)
    - `core.config (AI_PERSONALIZED_MESSAGE_TIMEOUT, DISCORD_BOT_TOKEN, EMAIL_SMTP_SERVER, get_available_channels)` (NEW)
    ...
    - `messages.message_data_manager (get_personalized_message_source, is_ai_generated_message_category, is_personalized_message_category_available, store_sent_message)`
    - `messages.message_reactions (personalized_reaction_instructions, text_is_retired)` (NEW)
  - **Standard Library**:
    - `asyncio`
    - `contextlib`
    - `threading`
...
- Removed: communication/communication_channels/discord/onboarding/welcome_handler.py, communication/communication_channels/discord/webhooks/handler.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `messages.message_reactions (exclude_retired_messages)` (NEW)
...
**Dependency Changes**:
- Added: core, core.error_handling, core.logger
- Removed: communication/core/channel_orchestrator.py, integrations/google_health/notifications.py
...
#### `communication/message_processing/action_plan_executor.py`
- **Purpose**: Communication channel implementation for action_plan_executor
- **Dependencies**:
  - **Local**:
...
#### `communication/message_processing/action_request_adapter.py`
- **Purpose**: Communication channel implementation for action_request_adapter
- **Dependencies**:
  - **Local**:
    - `ai.prompts.action_catalog (AIActionRequest)` (NEW)
    - `communication.command_handlers.shared_types (InteractionResponse, ParsedCommand)` (NEW)
...
**Dependency Changes**:
- Added: ai.prompts.action_catalog, communication.command_handlers.shared_types, communication.message_processing.command_parser, core.error_handling
...
- Removed: communication/communication_channels/email/inbound_processor.py
...
- Removed: collections.abc, communication/message_processing/flow_message_dispatcher.py, communication/message_processing/flows/flow_control_mixin.py, communication/message_processing/flows/note_flow.py, communication/message_processing/flows/task_flow.py
...
- Removed: collections.abc, communication/message_processing/flows/note_flow.py, communication/message_processing/flows/task_flow.py
...
- Removed: communication/message_processing/flows/checkin_flow.py, communication/message_processing/flows/flow_control_mixin.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `communication.message_processing.flows.flow_constants (FLOW_ENTRY_EDIT, FLOW_JOURNAL_BODY, FLOW_LIST_ITEMS, FLOW_NOTE_BODY)` (NEW)
...
**Dependency Changes**:
- Added: communication.message_processing.flows.flow_command_helpers, communication.message_processing.flows.flow_constants, communication.message_processing.flows.flow_control_mixin, core.error_handling, core.ids, core.logger, core.tags, core.time_utilities, notebook.notebook_data_manager, notebook.notebook_service
- Removed: communication/message_processing/conversation_flow_manager.py
...
- **Dependencies**:
  - **Local**:
    - `communication.message_processing.flows.flow_command_helpers (TASK_NOT_SAVED_MESSAGE, TASK_SAVED_MESSAGE, TASK_STEP_BACK_MESSAGE, is_skip_all_message, is_skip_question_message, is_task_flow_delete_in_progress_message, is_task_flow_step_back_message, is_task_flow_undo_creation_message, is_unrelated_task_due_date_message, is_unrelated_task_priority_message, is_unrelated_task_reminder_message)` (NEW)
    - `communication.message_processing.flows.flow_constants (FLOW_TASK_DUE_DATE, FLOW_TASK_PRIORITY, FLOW_TASK_REMINDER, TASK_REMINDER_DAY_WINDOW_END_HOUR, TASK_REMINDER_DAY_WINDOW_START_HOUR)` (NEW)
    - `communication.message_processing.flows.flow_control_mixin (FlowControlMixin)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (DATE_ONLY, TIME_ONLY_MINUTE, format_timestamp, now_datetime_full, now_timestamp_full, parse_date_and_time_minute, parse_date_only, parse_flexible_date_only, parse_time_only_minute)` (NEW)
...
- **Dependencies**:
  - **Local**:
    ...
    - `communication.message_processing.action_plan_executor (handle_message_with_action_planner)` (NEW)
    ...
    - `communication.message_processing.conversation_flow_manager (conversation_manager)`
    - `communication.message_processing.flow_message_dispatcher (dispatch_flow_message, reattach_active_task_flow_suggestions)` (NEW)
    ...
    - `core.config (AI_ACTION_PLANNER_ENABLED)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
  - **Standard Library**:
    - `typing (Any)`
...
- Removed: communication/communication_channels/discord/events/command_registration.py, communication/communication_channels/discord/events/interaction_router.py, communication/communication_channels/discord/events/message_handler.py, communication/communication_channels/discord/ui/checkin_view.py, communication/communication_channels/discord/ui/task_reminder_view.py, communication/communication_channels/email/inbound_processor.py
...
- **Dependencies**:
  - **Local**:
    - `communication.command_handlers.shared_types (InteractionResponse, ParsedCommand)` (NEW)
    - `communication.command_handlers.task_handler (TaskManagementHandler, handle_pending_simplify, handle_pending_task_action, handle_pending_task_offer)` (NEW)
    - `communication.message_processing.command_parser (ParsingResult)` (NEW)
    - `communication.message_processing.conversation_flow_manager (conversation_manager)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
  - **Standard Library**:
...
**Dependency Changes**:
- Added: communication.command_handlers.shared_types, communication.command_handlers.task_handler, communication.message_processing.command_parser, communication.message_processing.conversation_flow_manager, core.error_handling
...
**Dependency Changes**:
- Added: ai.context.assembly, communication.command_handlers.shared_types, core.config, core.error_handling, core.logger
...
- Removed: communication/message_processing/action_plan_executor.py, communication/message_processing/interaction_manager.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `integrations.google_health.token_crypto (is_valid_fernet_key)`
  - **Standard Library**:
    - `contextlib`
    - `logging`
    - `os`
...
**Dependency Changes**:
- Added: checkins.checkin_data_manager, core, core.error_handling, core.health_signals, core.time_utilities, integrations.google_health.data_handlers
- Removed: ai/chat/chatbot.py, ai/chat/wellness_status.py, ai/context/chatbot_context.py, ai/context/phraser.py, ai/context/service.py, ai/fallback/checkin_summary.py, ai/fallback/coordinator.py, ai/fallback/envelope_summaries.py, ai/fallback/personalized.py, collections.abc
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (DATE_ONLY, now_datetime_full, parse_timestamp_full)` (NEW)
...
- Removed: communication/core/channel_orchestrator.py, core/health_context_builder.py
...
**Dependency Changes**:
- Added: core.error_handling, core.logger
- Removed: communication/command_handlers/notebook_handler.py, communication/communication_channels/discord/ui/task_reminder_view.py, communication/message_processing/flows/note_flow.py, storage/user_data_v2_base.py, tasks/task_service.py
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: core/headless_service.py
...
#### `core/natural_language_defaults.py`
- **Purpose**: Core system module for natural_language_defaults
- **Dependencies**:
  - **Local**:
    - `core (get_user_data)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `storage.user_data_write (update_user_preferences)` (NEW)
    - `tasks.task_time_parsing (parse_time_string)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `collections.abc (Callable)`
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: collections.abc, communication/command_handlers/notebook_handler.py, communication/command_handlers/task_handler.py
...
- Removed: core/profile_v2_io.py, storage/user_data_read.py, storage/user_data_registry.py, storage/user_data_v2_envelopes.py, storage/user_data_validation.py, storage/user_data_write.py
...
- Removed: core/user_management.py, storage/user_data_write.py
...
**Dependency Changes**:
- Added: core.error_handling, core.logger
- Removed: core/schedule_document_defaults.py, core/web_user_settings.py
...
**Dependency Changes**:
- Added: core.config, core.error_handling, core.logger
- Removed: core/headless_service.py, core/service.py, core/service_requests.py, core/web_account_service.py, ui/dialogs/process_watcher_dialog.py
...
- **Dependencies**:
  - **Local**:
    - `core.time_format_constants (DATETIME_DISPLAY_FOR_AI, DATE_ONLY, EXTERNAL_TIMESTAMP_VARIANTS, TIMESTAMP_FILENAME, TIMESTAMP_FULL, TIMESTAMP_MINUTE, TIMESTAMP_WITH_MICROSECONDS, TIME_COMPACT_HOUR_MINUTE, TIME_ONLY_MINUTE)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
...
- Removed: core/admin_account_provisioning.py, core/service.py, core/user_lookup.py, integrations/google_health/sync_manager.py, storage/user_data_index.py, storage/user_data_summaries.py
...
- **Purpose**: Module for integrations/__init__.py
- **Dependencies**: None (no imports)
- **Used by**: None (not imported by other modules)
...
- **Purpose**: Module for integrations/google_health/__init__.py
- **Dependencies**:
  - **Local**:
    - `integrations.google_health.sync_manager (sync_user_health_data)` (NEW)
- **Used by**: None (not imported by other modules)

**Dependency Changes**:
- Added: integrations.google_health.sync_manager
...
- **Dependencies**:
  - **Local**:
    - `core.config (GOOGLE_HEALTH_API_BASE_URL)` (NEW)
    - `core.error_handling (CommunicationError, handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (now_timestamp_full)` (NEW)
    - `integrations.google_health.schemas (DailySummaryModel, SleepStagesSummaryModel)` (NEW)
    - `integrations.google_health.testing (is_google_health_testing_mode)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `collections.abc (Callable)`
...
**Dependency Changes**:
- Added: core.config, core.error_handling, core.logger, core.time_utilities, integrations.google_health.schemas, integrations.google_health.testing
- Removed: collections.abc, integrations/google_health/sync_manager.py
...
#### `integrations/google_health/data_handlers.py`
- **Purpose**: Module for integrations/google_health/data_handlers.py
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (now_timestamp_full)` (NEW)
    - `integrations.google_health.schemas (AUTH_FILENAME, DAILY_SUMMARIES_FILENAME, DailySummariesCollectionModel, GoogleHealthAuthModel, HEALTH_SIGNALS_FILENAME, HealthSignalsCollectionModel, SYNC_STATE_FILENAME, SyncStateModel, empty_auth_document, empty_daily_summaries_document, empty_health_signals_document, empty_sync_state_document)` (NEW)
    - `integrations.google_health.token_crypto (prepare_auth_for_storage, prepare_auth_for_use)` (NEW)
    - `storage.user_data_validation (is_valid_user_id)` (NEW)
    - `storage.user_item_storage (ensure_user_subdir, get_user_subdir_path, load_user_json_file, save_user_json_file)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `shutil`
    - `typing (Any)`
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: communication/core/channel_orchestrator.py, core/health_context_builder.py, integrations/google_health/signal_builder.py
...
#### `integrations/google_health/schemas.py`
- **Purpose**: Module for integrations/google_health/schemas.py
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `storage.user_data_v2_base (SCHEMA_VERSION)` (NEW)
...
**Dependency Changes**:
- Added: core.error_handling, storage.user_data_v2_base
- Removed: integrations/google_health/client.py, integrations/google_health/data_handlers.py, integrations/google_health/signal_builder.py
...
#### `integrations/google_health/signal_builder.py`
- **Purpose**: Module for integrations/google_health/signal_builder.py
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (now_timestamp_full)` (NEW)
    - `integrations.google_health.personalization_rules (apply_personalization_rules)` (NEW)
    - `integrations.google_health.schemas (DailySummaryModel, HealthSignalModel, SleepStagesSummaryModel)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `statistics`
    - `typing (Any)`
...
**Dependency Changes**:
- Added: core.error_handling, core.logger, core.time_utilities, integrations.google_health.personalization_rules, integrations.google_health.schemas
- Removed: integrations/google_health/sync_manager.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.config (GOOGLE_HEALTH_ENABLED, GOOGLE_HEALTH_SYNC_FAILURE_PAUSE_THRESHOLD, GOOGLE_HEALTH_SYNC_LOOKBACK_DAYS)` (NEW)
    - `core.error_handling (CommunicationError, handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (now_timestamp_full)` (NEW)
    - `core.user_management (get_all_user_ids)` (NEW)
    ...
    - `integrations.google_health.testing (is_google_health_testing_mode)` (NEW)
    - `scheduler.health_sync_schedule (get_due_sync_slot_key)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `argparse`
    - `typing (Any)`
...
**Dependency Changes**:
- Added: core, core.config, core.error_handling, core.logger, core.time_utilities, core.user_management, integrations.google_health.auth, integrations.google_health.client, integrations.google_health.data_handlers, integrations.google_health.notifications, integrations.google_health.signal_builder, integrations.google_health.testing, scheduler.health_sync_schedule
- Removed: integrations/google_health/__init__.py, integrations/google_health/user_settings.py, scheduler/health_sync_jobs.py
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: integrations/google_health/auth.py, integrations/google_health/client.py, integrations/google_health/notifications.py, integrations/google_health/sync_manager.py
...
**Dependency Changes**:
- Added: core.error_handling, core.logger
- Removed: core/config.py, cryptography.fernet, integrations/google_health/data_handlers.py
...
#### `integrations/google_health/user_settings.py`
- **Purpose**: Module for integrations/google_health/user_settings.py
- **Dependencies**:
  - **Local**:
    - `core (get_user_data, update_user_account)` (NEW)
    ...
    - `integrations.google_health.data_handlers (delete_user_health_data, has_valid_auth, load_sync_state)` (NEW)
    - `integrations.google_health.sync_manager (sync_user_health_data)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `collections.abc (Callable)`
...
- **Dependencies**:
  - **Third-party**:
    ...
    - `message_data_manager (add_message, archive_old_messages, create_message_file_from_defaults, delete_message, edit_message, ensure_user_message_files, get_message_categories, get_personalized_message_source, get_recent_messages, get_timestamp_for_sorting, is_ai_generated_message_category, is_automated_messages_enabled, is_personalized_message_category_available, load_default_messages, load_user_messages, store_sent_message, update_message)`
...
#### `messages/message_reactions.py`
- **Purpose**: Module for messages/message_reactions.py
- **Dependencies**:
  - **Local**:
    - `ai.chat.chatbot (get_ai_chatbot)` (NEW)
    - `core.config (get_user_data_dir)` (NEW)
    ...
    - `core.file_operations (load_json_data, save_json_data)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `messages.message_data_manager (add_message, edit_message, get_recent_messages, is_ai_generated_message_category, load_user_messages, update_sent_message_metadata)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `pathlib (Path)`
    - `typing (Any)`
...
**Dependency Changes**:
- Added: core.time_utilities, storage.user_data_v2_base
- Removed: messages/message_data_manager.py, storage/user_data_v2_envelopes.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `integrations.google_health.sync_manager (sync_users_due_for_schedule)`
  - **Standard Library**:
    - `os`
  - **Third-party**:
    - `schedule`
...
- **Dependencies**:
  - **Local**:
    - `core.config (parse_google_health_sync_times)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (parse_time_only_minute)` (NEW)
    - `scheduler.user_timezone (localized_now_for_user)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `datetime`
- **Used by**:
...
**Dependency Changes**:
- Added: core.config, core.error_handling, core.logger, core.time_utilities, scheduler.user_timezone
- Removed: integrations/google_health/sync_manager.py
...
**Dependency Changes**:
- Added: core.error_handling
- Removed: core/admin_account_provisioning.py, core/service.py, scheduler/__init__.py, tasks/task_data_manager.py, tasks/task_occurrence_skip.py, tasks/task_reminder_snooze.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.logger (get_component_logger)` (NEW)
    - `core.schedule_runtime (get_schedule_time_periods)` (NEW)
    - `core.time_utilities (DATE_ONLY, TIMESTAMP_FULL, TIME_ONLY_MINUTE, format_timestamp, load_and_localize_datetime, now_datetime_full, parse_date_only, parse_time_only_minute)` (NEW)
    - `scheduler.user_timezone (localized_now_for_user, resolve_user_timezone_str)` (NEW)
    - `tasks (are_tasks_enabled, get_task_by_id, load_active_tasks, update_task)` (NEW)
    - `tasks.task_data_handlers (runtime_task_due_date, runtime_task_is_completed)` (NEW)
    - `tasks.task_reminder_snooze (parse_task_snooze_until, task_reminder_is_snoozed)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `datetime (datetime, timedelta)`
    - `random`
...
**Dependency Changes**:
- Added: core, core.error_handling, core.logger, core.time_utilities
- Removed: ai/context/phraser.py, ai/context/service.py, checkins/checkin_service.py, scheduler/health_sync_schedule.py, scheduler/manager.py, scheduler/task_reminders.py, tasks/task_data_manager.py
...
- Removed: core/headless_service.py, core/service_requests.py, core/web_account_service.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (now_timestamp_filename, now_timestamp_full)` (NEW)
    - `storage.user_data_index (remove_from_index)` (NEW)
    - `storage.user_data_user_info (get_user_info_for_data_manager, get_user_message_files)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `json`
    - `os`
...
#### `storage/user_data_index.py`
- **Purpose**: Module for storage/user_data_index.py
- **Dependencies**:
  - **Local**:
    - `core.config (BASE_DATA_DIR)` (NEW)
    - `core.error_handling (handle_errors)` (NEW)
    - `core.file_locking (safe_json_read, safe_json_write)` (NEW)
    - `core.file_operations (get_user_data_dir)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.time_utilities (now_timestamp_full)` (NEW)
    - `core.user_management (get_all_user_ids)` (NEW)
    - `storage.user_data_read (get_user_data)` (NEW)
    - `storage.user_data_summaries (get_user_data_summary)` (NEW)
    - `storage.user_data_user_info (_get_user_categories, get_user_info_for_data_manager)` (NEW)
    - `storage.user_data_v2_base (SCHEMA_VERSION)` (NEW)
...
**Dependency Changes**:
- Added: core.config, core.error_handling, core.file_locking, core.file_operations, core.logger, core.time_utilities, core.user_management, storage.user_data_read, storage.user_data_summaries, storage.user_data_user_info, storage.user_data_v2_base, storage.user_data_v2_envelopes
- Removed: storage/user_data_backup.py, storage/user_data_operations.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `storage.user_data_backup (backup_user_data, delete_user_completely, export_user_data)` (NEW)
    - `storage.user_data_index (build_user_index, rebuild_full_index, rebuild_user_index, remove_from_index, search_users, update_user_index)` (NEW)
    - `storage.user_data_summaries (_get_last_interaction, get_all_user_summaries, get_user_analytics_summary, get_user_data_summary, get_user_summary)` (NEW)
    - `storage.user_data_user_info (_get_user_categories, get_user_info_for_data_manager, get_user_message_files, update_message_references)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `os`
    - `pathlib (Path)`
...
#### `storage/user_data_summaries.py`
- **Purpose**: Module for storage/user_data_summaries.py
- **Dependencies**:
  - **Local**:
    ...
    - `messages.message_data_manager (get_recent_messages)` (NEW)
    - `storage.user_data_read (get_user_data)` (NEW)
    - `storage.user_data_user_info (_get_user_categories, get_user_info_for_data_manager, get_user_message_files)` (NEW)
    - `storage.user_data_v2_base (SCHEMA_VERSION)` (NEW)
    - `storage.user_data_v2_envelopes (validate_v2_document)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
...
**Dependency Changes**:
- Added: checkins.checkin_data_manager, core.error_handling, core.file_operations, core.logger, core.response_tracking, core.time_utilities, core.user_management, messages.message_data_manager, storage.user_data_read, storage.user_data_user_info, storage.user_data_v2_base, storage.user_data_v2_envelopes
- Removed: collections.abc, storage/user_data_index.py, storage/user_data_operations.py
...
**Dependency Changes**:
- Added: core.error_handling, core.file_operations, core.logger, storage.user_data_read
- Removed: storage/user_data_backup.py, storage/user_data_index.py, storage/user_data_operations.py, storage/user_data_summaries.py
...
- Removed: storage/user_data_index.py, storage/user_data_summaries.py
...
- Removed: communication/command_handlers/base_handler.py, core/web_user_settings.py, integrations/google_health/data_handlers.py, storage/user_data_write.py, tasks/task_data_handlers.py, ui/dialogs/account_creator_dialog.py, ui/dialogs/channel_management_dialog.py, ui/dialogs/checkin_management_dialog.py, ui/dialogs/schedule_editor_dialog.py, ui/dialogs/task_management_dialog.py, ui/dialogs/user_profile_dialog.py, ui/widgets/category_selection_widget.py
...
**Dependency Changes**:
- Added: core.config, core.error_handling, core.file_operations, core.logger
- Removed: integrations/google_health/data_handlers.py, tasks/task_data_handlers.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.logger (get_component_logger)` (NEW)
    - `core.natural_language_defaults (get_natural_language_defaults)` (NEW)
    - `core.time_utilities (DATE_ONLY, TIMESTAMP_FULL, TIME_ONLY_MINUTE, format_timestamp, now_datetime_full, parse_date_only)` (NEW)
    - `scheduler.runtime_access (get_scheduler_manager)` (NEW)
    - `tasks.task_data_handlers (runtime_task_due_date, runtime_task_due_time, runtime_task_is_completed, runtime_task_recurrence_interval, runtime_task_recurrence_pattern)` (NEW)
    - `tasks.task_data_manager (_calculate_next_due_date, get_task_by_id, update_task)` (NEW)
    - `tasks.task_reminder_snooze (snooze_task_reminder)` (NEW)
    - `tasks.task_time_parsing (parse_time_string)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
...
- Removed: ai/chat/action_planner.py, communication/command_handlers/task_handler.py, communication/message_processing/command_parser.py, communication/message_processing/flow_message_dispatcher.py
...
- **Dependencies**:
  - **Local**:
    ...
    - `core.time_utilities (DATE_ONLY, TIMESTAMP_FULL, TIME_ONLY_MINUTE, format_timestamp, now_datetime_full, parse_date_only, parse_timestamp_full)` (NEW)
    - `scheduler.runtime_access (get_scheduler_manager)` (NEW)
    - `tasks.task_data_handlers (runtime_task_due_date, runtime_task_is_completed)` (NEW)
    - `tasks.task_data_manager (get_task_by_id, update_task)` (NEW)
    - `tasks.task_service (parse_relative_date)` (NEW)
    - `tasks.task_time_parsing (parse_time_string)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
...
- **Dependencies**:
  - **Local**:
    ...
    - `tasks.task_link_helpers (normalize_task_link_label, normalize_task_url, sanitize_task_links)` (NEW)
    - `tasks.task_tag_helpers (sanitize_task_tags)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `typing (Any, Literal)`
...
- **Dependencies**:
  - **Local**:
    ...
    - `tasks` (NEW)
    - `tasks.task_data_handlers (runtime_task_due_date, runtime_task_due_time, runtime_task_scheduled_reminder_periods)` (NEW)
...
- Removed: communication/message_processing/flows/task_flow.py, core/web_account_service.py, tasks/task_reminder_snooze.py
...
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.tags (normalize_tag, normalize_tags)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
    - `re`
- **Used by**:
...
**Dependency Changes**:
- Added: core.error_handling, core.logger, core.time_utilities, tasks.task_link_helpers
- Removed: storage/user_data_v2_envelopes.py, tasks/task_data_manager.py, tasks/task_simplify.py
...
#### `ui/dialogs/google_health_settings_dialog.py`
- **Purpose**: Dialog component for google health settings dialog
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `integrations.google_health.user_settings (delete_health_integration, enable_health_integration, format_status_text, get_connect_authorization_url, get_connect_readiness, get_health_integration_status, pause_health_integration, run_connect_flow, sync_health_integration)`
  - **Standard Library**:
    - `__future__ (annotations)`
    - `webbrowser`
  - **Third-party**:
    - `PySide6.QtCore (QObject, QThread, Signal)`
    - `PySide6.QtWidgets (QDialog, QHBoxLayout, QLabel, QMessageBox, QPushButton, QTextEdit, QVBoxLayout)`
- **Used by**: None (not imported by other modules)
...
#### `ui/dialogs/natural_language_settings_dialog.py`
- **Purpose**: Dialog component for natural language settings dialog
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.natural_language_defaults (get_natural_language_defaults, save_natural_language_defaults_preferences)` (NEW)
    - `ui.widgets.natural_language_settings_widget (NaturalLanguageSettingsWidget)` (NEW)
  - **Third-party**:
    - `PySide6.QtCore (Signal)`
    - `PySide6.QtWidgets (QDialog, QDialogButtonBox, QMessageBox, QVBoxLayout)`
- **Used by**: None (not imported by other modules)

**Dependency Changes**:
- Added: core.error_handling, core.logger, core.natural_language_defaults, ui.widgets.natural_language_settings_widget
- Removed: PySide6.QtCore, PySide6.QtWidgets
...
- **Dependencies**:
  - **Local**:
    - `core.config (AI_PERSONALIZED_MESSAGE_TIMEOUT)` (NEW)
    - `messages.message_data_manager (is_ai_generated_message_category)` (NEW)
  - **Standard Library**:
    - `contextlib`
    - `dataclasses (dataclass, field)`
...
**Dependency Changes**:
- Added: core.config, messages.message_data_manager
- Removed: PySide6.QtWidgets
...
- **Dependencies**:
  - **Standard Library**:
    ...
    - `sys`
  - **Third-party**:
    - `PySide6.QtCore (QObject, QThread, QTimer, Signal)`
...
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `messages.message_data_manager (get_message_categories, is_ai_generated_message_category)` (NEW)
  ...
  - **Third-party**:
    - `PySide6.QtWidgets (QCheckBox, QWidget)`
- **Used by**:
  - `ui/dialogs/account_creator_dialog.py`
  - `ui/dialogs/category_management_dialog.py`
...
#### `ui/widgets/natural_language_settings_widget.py`
- **Purpose**: UI widget component for natural language settings widget
- **Dependencies**:
  - **Local**:
    - `core.error_handling (handle_errors)` (NEW)
    - `core.logger (get_component_logger)` (NEW)
    - `core.natural_language_defaults (NaturalLanguageDefaults, merge_natural_language_preferences, natural_language_defaults_to_preferences_dict)` (NEW)
    - `tasks.task_time_parsing (parse_time_string)` (NEW)
  - **Standard Library**:
    - `__future__ (annotations)`
  - **Third-party**:
    - `PySide6.QtWidgets (QCheckBox, QFormLayout, QGroupBox, QLabel, QLineEdit, QVBoxLayout, QWidget)`
- **Used by**:
  - `ui/dialogs/natural_language_settings_dialog.py`

**Dependency Changes**:
- Added: core.error_handling, core.logger, core.natural_language_defaults, tasks.task_time_parsing
- Removed: PySide6.QtWidgets, ui/dialogs/natural_language_settings_dialog.py
...
- Removed: ai/chat/action_planner.py, ai/chat/chatbot.py, ai/context/assembly.py
