...
class ReportGenerationMixin:
    ...
    def _generate_ai_priorities_document(self) -> str:
        ...
        # Add top unused-import offenders to quick wins only when there are
        # obvious removals (not purely contextual categories like type-only/test-mocking).
        if isinstance(unused_imports_data, dict):
            unused_summary = (
                unused_imports_data.get("summary", {})
                if isinstance(unused_imports_data.get("summary"), dict)
                else {}
            )
            unused_total = to_int(unused_summary.get("total_issues")) or 0
            unused_files = to_int(unused_summary.get("files_affected")) or 0
            unused_details = (
                unused_imports_data.get("details", {})
                if isinstance(unused_imports_data.get("details"), dict)
                else {}
            )
            by_category = (
                unused_details.get("by_category", {})
                if isinstance(unused_details.get("by_category"), dict)
                else {}
            )
            obvious_count = to_int(by_category.get("obvious_unused")) or 0
            findings = (
                unused_details.get("findings", {})
                if isinstance(unused_details.get("findings"), dict)
                else {}
            )
            obvious_findings = findings.get("obvious_unused", [])
            if obvious_count <= 0 and isinstance(obvious_findings, list):
            ...
            if unused_total > 0 and obvious_count > 0:
                ...
                top_unused_text = (
                    ", ".join(f"{path} ({count})" for path, count in top_unused)
                    if top_unused
                    else "No file-level details available"
                )
                quick_wins.append(
                    f"Unused imports (obvious): {obvious_count} obvious import(s) "
                    f"out of {unused_total} total across {unused_files} file(s). "
                    f"Top offenders: {top_unused_text}. "
                    "Fix: `python development_tools/run_development_tools.py unused-imports`. "
                    "Verify: `python development_tools/run_development_tools.py unused-imports-report`."
                )
