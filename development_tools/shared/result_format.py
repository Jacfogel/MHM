...
def normalize_to_standard_format(
    ...
    if not isinstance(data, dict):
        logger.warning(
            f"Data for {tool_name} is not a dict, returning empty standard format"
        )
...
def _create_empty_standard_format() -> Dict[str, Any]:
    ...
    return {"summary": {"total_issues": 0, "files_affected": 0}, "details": {}}
...
def _normalize_documentation_sync(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {
        "total_issues": data.get("total_issues", 0),
        "files_affected": (
            len(data.get("path_drift_files", []))
            if isinstance(data.get("path_drift_files"), list)
            else 0
        ),
...
def _normalize_functions(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {"total_issues": total_issues, "files_affected": 0}  # Not file-based
...
def _normalize_error_handling(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "missing_error_handling" in data and isinstance(
        data["missing_error_handling"], list
    ):
        unique_files = set()
...
def _normalize_function_registry(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    missing_count = (
        ...
        else (missing if isinstance(missing, int) else 0)
    )
    extra_count = (
        extra.get("count", 0)
        if isinstance(extra, dict)
        else (extra if isinstance(extra, int) else 0)
    )

    summary = {"total_issues": missing_count + extra_count, "files_affected": 0}
...
def _normalize_test_markers(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if missing_list:
        files_affected = len(
            set(item.get("file", "") for item in missing_list if item.get("file"))
        )
    elif data.get("files_needing_markers"):
        files_affected = data.get("files_needing_markers", 0)

    summary = {"total_issues": missing_count, "files_affected": files_affected}
...
def _normalize_module_dependencies(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    missing_sections = (
        len(data.get("missing_sections", []))
        if isinstance(data.get("missing_sections"), list)
        else 0
    )

    summary = {"total_issues": missing_deps + missing_sections, "files_affected": 0}
...
def _normalize_test_coverage(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {"total_issues": missed, "files_affected": 0}  # Lines not covered
...
def _normalize_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(tool_analysis, dict):
        if isinstance(tool_analysis, list):
            issues += sum(
                len(t.get("issues", [])) for t in tool_analysis if isinstance(t, dict)
            )
        else:
            issues += sum(
                len(t.get("issues", []))
    ...
    summary = {"total_issues": issues, "files_affected": 0}
...
def _normalize_package_exports(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    packages_with_issues = sum(
        1
        for pkg, report in packages.items()
        if isinstance(report, dict)
        and (report.get("missing_exports") or report.get("potentially_unnecessary"))
    )
    ...
    result = {"summary": summary, "details": data}  # Preserve all original data
...
def _normalize_fallback(tool_name: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {"total_issues": total_issues, "files_affected": files_affected}
