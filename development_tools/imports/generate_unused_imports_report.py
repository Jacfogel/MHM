...
class UnusedImportsReportGenerator:
    ...
    def __init__(self, analysis_data: Dict[str, Any]):
        ...
        if 'details' in analysis_data:
            ...
            has_full_findings = (
                self.findings and 
                isinstance(self.findings, dict) and
                any(isinstance(items, list) and len(items) > 0 for items in self.findings.values())
            )
            ...
            if not has_full_findings:
                ...
                self.findings = {cat: [] for cat in [
                    'obvious_unused', 'type_hints_only', 're_exports', 'conditional_imports', 
                    'star_imports', 'test_mocking', 'qt_testing', 'test_infrastructure', 
                    'production_test_mocking', 'ui_imports'
                ]}
    ...
    def generate_report(self) -> str:
        ...
        lines.append("> **Source**: `python development_tools/run_development_tools.py unused-imports-report` - Unused Imports Report Generator")
        lines.append("")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Generate unused imports report from analysis results')
    ...
    if args.input_file:
        ...
        analysis_data = load_tool_result('analyze_unused_imports', 'imports', 
                                        project_root=project_root_path, normalize=False)
        
        if analysis_data is None:
            logger.error("No analysis results found. Run 'python development_tools/run_development_tools.py unused-imports' first.")
            return 1
    ...
    if args.json:
        report_data = {
            ...
            'findings_counts': generator.findings_counts if hasattr(generator, 'findings_counts') else None
        }
