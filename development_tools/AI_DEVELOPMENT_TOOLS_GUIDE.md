...
**Code-level markers**: Prefer canonical suppressions from `shared/exclusion_utilities.py`: `# devtools: ignore[duplicate-functions]: <reason>`, `# devtools: ignore[facade-shims]: <reason>`, `# devtools: ignore[legacy-references]: <reason>`, and `# devtools: intentional[duplicate-functions]: <group_id>`. Existing duplicate-function aliases are still accepted but should not be added in new code.
