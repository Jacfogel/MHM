...
*User priority*: **60% dev-tools coverage** (roadmap floor) = achieved; ongoing **~80% module** nudges from AI_PRIORITIES = optional hardening. Stale-lock recovery = yes (prioritize). test_config.json fixture = optional (see note below). Tier 3/coverage split: consider audit Tier 3 without coverage; run coverage independently. Parallel workers: **6** concurrent fallback on typical hosts (done).
...
**Status**: **60% target MET** (see AI_STATUS dev-tools line); **optional** follow-up = modules still under ~80% in AI_PRIORITIES  
**User priority**: Medium/low for floor; low for advisory 80% nudges  
**Tasks**:
- [x] Use `development_tools/AI_PRIORITIES.md` (audit-generated) for actionable coverage priorities; do not duplicate module lists here—refer to AI_PRIORITIES + `generate_dev_tools_coverage_results.json`
...
**Status**: COMPLETED (replacement, not re-enable)  
**User priority**: Replace with simpler/integration tests (not fix-and-re-enable).  
**Tasks**:
- [x] Replaced commented-out `test_audit_status_updates.py` with a lightweight unit test (`create_output_file` call counts for quick audit; patches match `test_status_file_timing` style)
- [x] Documented retirement of `test_audit_tier_comprehensive.py` in `tests/DEVELOPMENT_TOOLS_TESTING_GUIDE.md`
- [ ] Re-enable `tests/development_tools/test_audit_tier_comprehensive.py` only if a narrow mocked subset is worth the maintenance cost (deferred)

#### 1.3 Investigate intermittent low coverage warning
**Status**: **MONITORING** (fixes landed; reopen if warning recurs).  
**User priority**: Defer unless reproduced.  
**Tasks**:
...
- [x] Reproduce with controlled parallel runs — **not required while stable**; if `generate_dev_tools_coverage_results.json` or audit shows suspicious low %, run `run_test_coverage.py` with `--no-parallel` and compare shard merge logs
...
- [x] Consider `scope="module"` for `temp_project_copy` where tests do not mutate shared state — **2026-03-24**: Deferred; no batch change (shared-state risk); prefer per-test review when touching slow tests
...
*User priority*: Logging (print→logger) = medium. System signals = investigate then decide keep/enhance/remove. development_tools-only mode = partial; DEV_TOOLS_* output naming done, but report content still project-wide (generators not scoped). Rename consolidated_report.md → CONSOLIDATED_REPORT.md = **COMPLETE**. Retire unapproved docs = **COMPLETE** (RESULT_FORMAT_STANDARD, OUTPUT_STORAGE_STANDARDS, EXCLUSION_RULES removed). Console output: timing/cache summary shifted to logging; progress indicators TBD. Tier 3 follow-ups = high (proper legacy removal, clean codebase).
...
- [x] Shift audit timing/cache summary (Total tool execution time, Slowest tools, Coverage/Cache mode) to logging instead of stdout
- [x] Replace print statements with logging (except intentional audit progress prints) — **2026-03-24**: `generate_directory_tree.py` and `fix_documentation.py` dispatcher `main()` use `logger.info` / `logger.error`
...
**Status**: IN PROGRESS (keep + clarify)  
...
- [x] Rename `consolidated_report.md` → `CONSOLIDATED_REPORT.md` to match AI_STATUS.md, AI_PRIORITIES.md uppercase convention
- [x] Expand consolidated report detail to at least AI_STATUS level
- [x] Format Executive Summary error-handling line as `**Error Handling Coverage**: X% (N functions need protection)`

#### 2.8 Run all tools in development_tools-only mode
**Status**: **DEFERRED (2026-03-25)** — CLI `--dev-tools-only`, separate DEV_TOOLS_* output paths, and dev-tools tree targeting remain; **scope-aware report content** (metrics/sections limited to `development_tools/`) is **not** in the current implementation backlog. Revisit when prioritized.  
**User priority**: Was Medium; revisit when un-deferred.  
**Tasks**:
- [x] Add a way to run the full audit (or equivalent) with `development_tools/` as the scan root (`audit --dev-tools-only`)
- [x] Separate this from the current "include dev tools in scan" behavior via a dedicated `--dev-tools-only` flag
- [x] When running in development_tools-only mode, write status files to DEV_TOOLS_* paths
- [ ] **Remaining (deferred)**: Report generators still produce project-wide content; DEV_TOOLS_* output mirrors AI_* until generators are scope-aware (pass dev_tools_only_mode and restrict metrics/sections to development_tools)
- [ ] **Remaining (deferred)**: Reuse existing coverage/test flow for development_tools where it already targets that tree; ensure report outputs reflect dev-tools-scoped data

#### 2.9 Console output polish (optional)
**Status**: IN PROGRESS (spot-check done; progress indicators still open)  
...
**Tier3 structured outcome contract (completed follow-ups)**:
...
- [x] **2026-03-24**: Removed state-only track synthesis: `run_test_coverage_results.json` payloads missing `classification` on parallel/no_parallel tracks are invalidated (cache file deleted), same as empty `coverage_outcome`
- [x] **2026-03-24**: Removed `details.coverage_outcome` fallback in `_extract_cached_main_coverage_state`; cache precheck uses `tier3_test_outcome` only
- [x] DEPRECATION_INVENTORY `tier3_coverage_outcome_compat_bridge` note updated; regression tests in `test_audit_strict_mode.py`, `test_commands_additional_helpers.py`
...
**Status**: COMPLETED  
**User priority**: Yes—investigate.  
**Tasks**:
- [x] Investigate "Only 60/82 manual enhancements found in written file"—validation now only checks modules present in written output; stale enhancements from removed/renamed modules no longer trigger false warnings
- [ ] Review "needs_enhancement" and "new_module" triggers (low priority)
...
**Tasks**:
- [x] Consolidated report now uses `error_details` for metrics (aligned with AI_STATUS/AI_PRIORITIES)
- [x] Executive Summary error-handling line formatted as `**Error Handling Coverage**: X% (N functions need protection)`
- [x] Ensure reports show consistent values across AI_STATUS, AI_PRIORITIES, CONSOLIDATED_REPORT
...
**Tasks**:
- [x] Review current AI work validation logic (structural summary only; domain tools hold file-level truth)
- [x] **2026-03-24**: AI_STATUS validation lines now point to doc-sync, coverage report, and error-handling analyzer when status is POOR / NEEDS ATTENTION
- [ ] Avoid overlap with domain-specific analyzers (ongoing; keep this tool thin)
...
- [ ] Reduce unused imports (see AI_PRIORITIES for current count) and verify via `unused-imports-report`
- [ ] Raise domain coverage for communication, ui, and core (below 80% target)
- [ ] Refactor critical-complexity functions (see AI_PRIORITIES for current counts; e.g. 159 critical)
...
- [x] Evaluate development-tools-specific dependency management — **2026-03-24**: Documented current decision (root `requirements.txt` only) and future options in paired guides **§2** (`DEVELOPMENT_TOOLS_GUIDE.md`, `AI_DEVELOPMENT_TOOLS_GUIDE.md`). Standalone `DEV_TOOLS_DEPENDENCIES.md` removed **2026-03-25** (content merged into approved docs).
...
**User priority**: High/medium.  
- [x] Define a target taxonomy for `development_tools/` — **2026-03-24**: Phase 1 in paired guides **§9 Directory taxonomy and configuration surface**
- [x] Document current pain point: `development_tools/config/` mixes public configuration surface with implementation concerns (see §9 narrative)
- [x] Establish and document a stable "configuration surface" directory contract (table in `DEVELOPMENT_TOOLS_GUIDE.md` §9; summary in `AI_DEVELOPMENT_TOOLS_GUIDE.md` §9)
...
- [x] Propose a phased migration plan (no big-bang move):
  - Phase 1: clarify docs + ownership map. **Done (2026-03-24)**
