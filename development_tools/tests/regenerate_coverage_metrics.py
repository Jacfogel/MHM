...
# Script is at: development_tools/tests/regenerate_coverage_metrics.py
...
class CoverageMetricsRegenerator:
    ...
    def run_coverage_analysis(self) -> Dict[str, Dict[str, any]]:
        ...
        try:
            ...
            if logger:
                logger.info(f"Running pytest coverage command (full): {' '.join(cmd)}")
            ...
            result = subprocess.run(
                ...
                env=env,
                timeout=900  # 15 minute timeout for full test suite
            )
