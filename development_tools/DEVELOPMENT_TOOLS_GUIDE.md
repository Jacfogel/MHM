...
- Optional direct check: `python development_tools/docs/analyze_documentation_sync.py --check-example-markers` - advisory scan for file paths in Example sections without `[OK]` / `[AVOID]` / etc. markers (V5 Section 3.0); paired-doc JSON includes `example_marker_findings` when used with `--json`.
