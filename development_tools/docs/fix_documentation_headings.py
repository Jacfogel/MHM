...
def main():
    ...
    parser = argparse.ArgumentParser(description='Number H2 and H3 headings in documentation')
    parser.add_argument('--dry-run', action='store_true', help='Show what would be changed without making changes')
    parser.add_argument('--start-at-zero', action='store_true', help='Start numbering at 0 instead of 1')
    
    args = parser.parse_args()
    
    fixer = DocumentationHeadingFixer()
    result = fixer.fix_number_headings(dry_run=args.dry_run, start_at_zero=args.start_at_zero)
