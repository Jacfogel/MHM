...
class FeatureSelectionView(discord.ui.View):
    ...
    @handle_errors(
        "handling feature selection view timeout", context={"component": "discord"}
    )
...
class TaskFeatureSelect(discord.ui.Select):
    ...
    def __init__(self, parent_view: FeatureSelectionView):
        ...
        options = [
            ...
            discord.SelectOption(
                label="Disable Task Management",
                value="false",
                description="Skip task management features",
            ),
        ]
        super().__init__(
    ...
    def __init__(self, parent_view: FeatureSelectionView):
        ...
        options = [
            discord.SelectOption(
                ...
                value="true",
                description="Receive regular check-in prompts",
                default=True,
            ),
            discord.SelectOption(
                label="Disable Check-ins",
                value="false",
                description="Skip check-in features",
            ),
        ]
        super().__init__(
            placeholder="Check-ins...", min_values=1, max_values=1, options=options
        )
        self.parent_view = parent_view
...
class MessageFeatureSelect(discord.ui.Select):
    ...
    def __init__(self, parent_view: FeatureSelectionView):
        ...
        options = [
            discord.SelectOption(
                ...
                value="true",
                description="Receive scheduled motivational messages",
            ),
            discord.SelectOption(
                label="Disable Automated Messages",
                value="false",
                description="Skip automated messages",
                default=True,
            ),
        ]
        super().__init__(
    ...
    @handle_errors(
        "selecting automated messages feature", context={"component": "discord"}
    )
    async def callback(self, interaction: discord.Interaction):
...
class TimezoneSelect(discord.ui.Select):
    ...
    def __init__(self, parent_view: FeatureSelectionView):
        ...
        from storage.user_data_presets import TIMEZONE_OPTIONS
...
class CreateAccountButton(discord.ui.Button):
    ...
    def __init__(self, parent_view: FeatureSelectionView):
        ...
        """
        super().__init__(label="Create Account", style=discord.ButtonStyle.success)
        self.parent_view = parent_view

    @handle_errors(
        "creating account with selected features", context={"component": "discord"}
    )
    async def callback(self, interaction: discord.Interaction):
        ...
        response = _account_handler.handle(
            self.parent_view.discord_user_id, parsed_command
        )
        ...
        if response.completed:
            ...
            logger.warning(
                f"Account creation incomplete for Discord user {self.parent_view.discord_user_id}: {response.message}"
            )
...
async def start_account_creation_flow(
    ...
    if not isinstance(interaction, discord.Interaction):
        logger.error(
            f"Invalid interaction type: {type(interaction)}, expected discord.Interaction"
        )
        return
...
async def start_account_linking_flow(
        ...
        async def on_submit(self, modal_interaction: discord.Interaction):
            ...
            try:
                ...
                if not response.completed:
                    ...
                    if "confirmation_code_sent" in (response.rich_data or {}).get(
                        "type", ""
                    ):
                        # Create a modal for confirmation code input
                        class ConfirmLinkModal(
                            discord.ui.Modal, title="Enter Confirmation Code"
                        ):
                            ...
                            @handle_errors(
                                ...
                                context={"component": "discord"},
                            )
                            async def on_submit(
                                self, confirm_interaction: discord.Interaction
                            ):
                                ...
                                try:
                                    ...
                                    link_response = _account_handler.handle(
                                        discord_user_id, parsed_command
                                    )

                                    await confirm_interaction.response.send_message(
                                        link_response.message, ephemeral=True
                                    )
                        ...
                        await modal_interaction.response.send_message(
                            response.message, ephemeral=True
                        )
