...
- `get_create_hub_view(user_id)` in `ui/create_item_ui.py` returns a `discord.ui.View` with:
  - The accompanying `create` / `new` / `add` message labels the first row as tasks. Template names and aliases are not listed in that message (use `list task templates` for the keyword list, including Meds and Appt).
  - Template buttons on the first row: Call, Clean, Forms, plus Custom task. They open a prefilled task modal; submit uses `create_task_from_template`. Modal custom IDs stay stable so submit still works after a bot restart or if the in-memory modal timed out.
  - "Quick note" / "New note" on the second button row - Open empty note modals.
...
- Resolves the internal user ID using `core.get_user_id_by_identifier` and the Discord user ID.  
...
### 6.2. Task Creation Examples

Task creation is expected to work from natural Discord messages, not only from explicit command syntax:

- `create task to call mom tomorrow`
