        ...
        try:
            ...
            if DISCORD_PUBLIC_KEY:
                try:
                    ...
                    logger.error(
                        "PyNaCl not installed - cannot verify webhook signatures. Install with: pip install PyNaCl"
                    )
                ...
                logger.warning(
                    "DISCORD_PUBLIC_KEY not configured - skipping signature verification (dev mode)"
                )
            ...
            if not event_type:
                logger.warning(
                    f"Webhook event missing 'event.type' field. Full payload: {json.dumps(event_data, indent=2)}"
                )
            ...
            logger.info(
                f"Received webhook event: {event_type} (numeric type={event_type_raw})"
            )
