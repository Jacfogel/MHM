...
def handle_application_authorized(
    ...
    try:
        ...
        if not discord_user_id:
            logger.warning(
                f"APPLICATION_AUTHORIZED event missing user ID. Event data: {json.dumps(event_data, indent=2)}"
            )
            return False

        logger.info(
            f"User authorized app: Discord ID {discord_user_id}, username: {discord_username}"
        )
        ...
        if internal_user_id:
            logger.debug(
                f"User {discord_user_id} already has MHM account: {internal_user_id}, but not yet welcomed - sending welcome message"
            )
            ...
            if discord_username:
                try:
                    ...
                    logger.debug(
                        f"Could not update discord_username for {internal_user_id}: {update_err}"
                    )
        ...
        if bot_instance and hasattr(bot_instance, "bot") and bot_instance.bot:
            ...
            if bot_instance.bot.loop and not bot_instance.bot.loop.is_closed():
                ...
                @handle_errors(
                    "sending welcome DM to newly authorized user",
                    context={"discord_user_id": discord_user_id},
                    user_friendly=False,
                ...
                async def _send_welcome_dm():
                    ...
                    if user:
                        ...
                        logger.info(
                            f"Sent welcome DM to newly authorized user: {discord_user_id}"
                        )
                        return True
                    return False
                ...
                try:
                    ...
                    if bot_instance.bot.loop.is_closed():
                        logger.warning(
                            f"Bot event loop closed before scheduling welcome DM for {discord_user_id}"
                        )
                    ...
                    if "closed" in error_msg.lower() or "loop" in error_msg.lower():
                        logger.warning(
                            f"Bot event loop unavailable when scheduling welcome DM for {discord_user_id}: {error_msg}"
                        )
                    else:
                        logger.error(
                            f"Error scheduling welcome DM for {discord_user_id}: {e}"
                        )
                    ...
                    logger.error(
                        f"Unexpected error scheduling welcome DM for {discord_user_id}: {e}"
                    )
                    # Clean up will happen in finally block
                    mark_as_welcomed(discord_user_id)  # Mark as welcomed to avoid retry
                    return False
                ...
                logger.warning(
                    f"Bot event loop not available for sending welcome DM to {discord_user_id}"
                )
                mark_as_welcomed(discord_user_id)  # Mark as welcomed to avoid retry
                return False
        else:
            logger.warning(
                f"Bot instance not available for sending welcome DM to {discord_user_id}"
            )
            mark_as_welcomed(discord_user_id)  # Mark as welcomed to avoid retry
            return False
...
def handle_webhook_event(
    ...
    try:
        if event_type == EVENT_APPLICATION_AUTHORIZED:
            ...
            discord_username = str(user_data.get("username", ""))
            logger.info(
                f"User deauthorized app: Discord ID {discord_user_id}, username: {discord_username}"
            )
            ...
            logger.debug(
                f"Cleared welcomed status for deauthorized user: {discord_user_id}"
            )
