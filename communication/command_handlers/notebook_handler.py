...
• `!qn` or `quick note Grocery idea` — quick capture
...
**Tags:** Tags are labels (`#work`). Inbox means active entries without tags from the last 30 days.
...
class NotebookHandler(InteractionHandler):
    ...
    def _handle_create_note(
        ...
        if not description and title:
            ...
            conversation_manager.start_note_body_flow(user_id, title=title, tags=tags)
            return InteractionResponse(
    ...
    def _handle_create_quick_note(
        ...
        """Handle quick note creation without requiring body text."""
    ...
    def _handle_create_list(
        ...
        if not items:
            ...
            conversation_manager.start_list_items_flow(user_id, title=title, tags=tags)
            return InteractionResponse(
    ...
    def _handle_create_journal(
        ...
        if not description and title:
            ...
            conversation_manager.start_journal_body_flow(user_id, title=title, tags=tags)
            return InteractionResponse(
    ...
    # not_duplicate: handle_list_by_tag
