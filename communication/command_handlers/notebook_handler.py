...
class NotebookHandler(InteractionHandler):
    ...
    def _handle_create_list(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            ...
            return InteractionResponse("❌ Failed to create list. Please try again.", True)

    @handle_errors("handling create journal")
    def _handle_create_journal(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            response = f"✅ Journal entry created: '{entry.title or 'Untitled'}' ({short_id})"
            if entry.tags:
                response += f"\nTags: {', '.join(entry.tags)}"
            return InteractionResponse(response, True)
        else:
            return InteractionResponse("❌ Failed to create journal entry. Please try again.", True)
    ...
    def _handle_show_entry(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        """Handle showing an entry."""
        entry_ref = entities.get('entry_ref')
        
        if not entry_ref:
            return InteractionResponse("Which entry would you like to see? (Provide ID or title)", False)
    ...
    def _handle_append_to_entry(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        """Handle appending to entry body."""
        entry_ref = entities.get('entry_ref')
        text = entities.get('text') or entities.get('body')
        
        if not entry_ref:
            return InteractionResponse("Which entry would you like to append to?", False)
        if not text:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ Appended to '{entry.title or 'Untitled'}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to append. Entry not found or invalid.", True)

    @handle_errors("handling set entry body")
    def _handle_set_entry_body(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ Updated '{entry.title or 'Untitled'}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to update. Entry not found or invalid.", True)

    @handle_errors("handling add tags")
    def _handle_add_tags(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ Tags added to '{entry.title or 'Untitled'}' ({short_id}): {', '.join(entry.tags)}", True)
        else:
            return InteractionResponse(f"❌ Failed to add tags. Entry not found.", True)

    @handle_errors("handling remove tags")
    def _handle_remove_tags(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ Tags removed from '{entry.title or 'Untitled'}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to remove tags. Entry not found.", True)
    ...
    def _handle_pin_entry(self, user_id: str, entities: Dict[str, Any], pinned: bool) -> InteractionResponse:
        ...
        if not entry_ref:
            return InteractionResponse("Which entry would you like to pin/unpin?", False)
        ...
        if entry:
            ...
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ {action.capitalize()} '{entry.title or 'Untitled'}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to pin/unpin. Entry not found.", True)

    @handle_errors("handling archive entry")
    def _handle_archive_entry(self, user_id: str, entities: Dict[str, Any], archived: bool) -> InteractionResponse:
        """Handle archiving/unarchiving entry."""
        entry_ref = entities.get('entry_ref')
        
        if not entry_ref:
            return InteractionResponse("Which entry would you like to archive/unarchive?", False)
        ...
        if entry:
            ...
            return InteractionResponse(f"❌ Failed to archive/unarchive. Entry not found.", True)

    # List item handlers
    @handle_errors("handling add list item")
    def _handle_add_list_item(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ Added item to '{entry.title}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to add item. List not found or invalid.", True)

    @handle_errors("handling toggle list item done")
    def _handle_toggle_list_item_done(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            ...
            return InteractionResponse(f"✅ Item {item_index + 1} {action} in '{entry.title}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to toggle item. List not found or invalid.", True)

    @handle_errors("handling remove list item")
    def _handle_remove_list_item(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ Removed item {item_index + 1} from '{entry.title}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to remove item. List not found or invalid.", True)

    # Organization handlers
    @handle_errors("handling set group")
    def _handle_set_group(self, user_id: str, entities: Dict[str, Any]) -> InteractionResponse:
        ...
        if entry:
            short_id = self._format_entry_id(entry)
            return InteractionResponse(f"✅ Set group '{group}' for '{entry.title or 'Untitled'}' ({short_id})", True)
        else:
            return InteractionResponse(f"❌ Failed to set group. Entry not found.", True)
