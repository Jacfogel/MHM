...
class ScheduleManagementHandler(InteractionHandler):
    ...
    @handle_errors(
        "handling schedule management interaction",
        default_return=InteractionResponse(
            "I'm having trouble with schedule management right now. Please try again.",
            True,
        ),
    )
    def handle(
        ...
        if intent == "show_schedule":
            ...
            return InteractionResponse(
                f"I don't understand that schedule command. Try: {', '.join(self.get_examples())}",
                True,
            )
    ...
    def _handle_show_schedule(
        ...
        try:
            ...
            rich_data = {
                "type": "schedule",
                "title": (
                    f"Schedule for {category.title()}"
                    if category != "all"
                    else "Your Current Schedules"
                ),
                "fields": [],
            }
            ...
            if category == "all":
                ...
                for cat in categories:
                    ...
                    active_periods += sum(
                        1 for p in periods.values() if p.get("active", True)
                    )
            ...
            return InteractionResponse(
                "I'm having trouble showing your schedule right now. Please try again.",
                True,
            )
    ...
    @handle_errors(
        "updating schedule",
        default_return=InteractionResponse(
            "I'm having trouble updating your schedule. Please try again.", True
        ),
    )
    def _handle_update_schedule(
        ...
        try:
            ...
            if action == "enable":
                ...
                return InteractionResponse(
                    f"✅ All {category} schedule periods have been enabled.", True
                )
                ...
                set_schedule_periods(user_id, category, periods)
                return InteractionResponse(
                    f"❌ All {category} schedule periods have been disabled.", True
                )
            ...
            return InteractionResponse(
                "I'm having trouble updating your schedule right now. Please try again.",
                True,
            )
    ...
    @handle_errors(
        ...
        default_return=InteractionResponse(
            "I'm having trouble checking your schedule status. Please try again.", True
        ),
    )
    def _handle_schedule_status(
        ...
        try:
            ...
            return InteractionResponse(
                "I'm having trouble checking your schedule status right now. Please try again.",
                True,
            )

    @handle_errors(
        "adding schedule period",
        default_return=InteractionResponse(
            "I'm having trouble adding that schedule period. Please try again.", True
        ),
    )
    def _handle_add_schedule_period(
        ...
        if not all([category, period_name, start_time, end_time]):
            return InteractionResponse(
                ...
                completed=False,
                suggestions=[
                    "Add morning period 9am-11am",
                    "Add work period 8am-5pm weekdays",
                    "Add evening period 7pm-9pm",
                ],
            )
        ...
        try:
            ...
            response = (
                f"✅ Added new schedule period '{period_name}' to {category.title()}:\n"
            )
            response += f"  • Time: {start_time} - {end_time}\n"
            response += f"  • Days: {days_str}\n"
    ...
    @handle_errors(
        ...
        default_return=InteractionResponse(
            "I'm having trouble editing that schedule period. Please try again.", True
        ),
    )
    def _handle_edit_schedule_period(
        ...
        try:
            ...
            return InteractionResponse(
                f"I'm having trouble editing the schedule period: {str(e)}", True
            )

    @handle_errors(
        "getting schedule help",
        default_return="Help with schedule management - manage your message, task, and check-in schedules",
    )
