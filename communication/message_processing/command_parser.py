...
# Notebook intents whose regex captures are copied onto entity keys.
...
# Keep a rule-based match for these intents even below the high-confidence threshold
# so short tokens like "r" are not reinterpreted as tasks by AI parsing.
_PARSE_HIGH_PRIORITY_INTENTS: frozenset[str] = frozenset(
    {
        "list_recent_entries",
        "list_recent_notes",
        "create_quick_note",
...
# Substrings that mean "this looks like a command" when rule-based confidence is 0.
# Includes synonyms, misspellings, and related action words. Order does not matter.
_PARSE_COMMAND_KEYWORDS: tuple[str, ...] = (
    # Entity types
    "note",
    "notes",
    "notebook",
...
class EnhancedCommandParser:
    ...
    def __init__(self):
        ...
        self.intent_patterns = {
            ...
            "create_quick_note": [
                ...
                # Keep anchored so quick-note text does not match unrelated phrases.
                r"^quick\s+notes?\s*(.*)$",
            ],
            "create_journal": [
                r"^j\s+(.+)$",
    ...
    def parse(self, message: str, user_id: str | None = None) -> ParsingResult:
        ...
        logger.info(
            f"COMMAND_PARSER: Parsing message for user {user_id}: '{message[:50]}...'"
        )
        ...
        # For certain intents, prefer rule-based results even with lower confidence
        # so short tokens like "r" are not reinterpreted as tasks by AI parsing.
        if (
            rule_result.parsed_command.intent in _PARSE_HIGH_PRIORITY_INTENTS
            and rule_result.confidence > 0.0
        ):
            return rule_result

        # Zero rule confidence: try AI only when the text still looks like a command.
        if rule_result.confidence == 0.0:
            message_lower = message.lower()
            looks_like_command = any(
                keyword in message_lower for keyword in _PARSE_COMMAND_KEYWORDS
            )

            if looks_like_command:
                # Message looks like a command but didn't match patterns - try AI parsing
                logger.debug(
                    f"Message '{message[:50]}...' has 0.0 confidence but contains command keywords, trying AI parsing"
                )
    ...
    def _rule_based_parse(self, message: str, user_id: str | None = None) -> ParsingResult:
        ...
        def _match_message(message_for_match: str) -> ParsingResult | None:
            ...
            if message_for_match == "help":
                return ParsingResult(
                    ParsedCommand("help", {}, 1.0, message), 1.0, "rule_based"
                )
            ...
            for intent, force_match in priority_intents:
                parsed = self._match_rule_based_intent(
                    intent,
                    message_for_match,
                    message,
            ...
            skip_intents = {intent for intent, _ in priority_intents}
    ...
    def _build_rule_based_result_from_pattern(
        ...
        *,
        ...
        match = (
            pattern.match(message_for_match)
            if pattern.pattern.startswith("^")
            else pattern.search(message_for_match)
        )
        if not match:
            return None
    ...
    @handle_errors("matching rule-based intent", default_return=None)
    def _match_rule_based_intent(
        self,
        intent: str,
        message_for_match: str,
        original_message: str,
    ...
    def _ai_enhanced_parse(
        ...
        try:
            ...
            try:
                ...
                if (
                    "command parser" in ai_response.lower()
                    or "available actions" in ai_response.lower()
                ):
                    logger.debug(
                        "AI returned system prompt instead of JSON, using fallback"
                    )
                    return ParsingResult(
                        ParsedCommand("unknown", {}, 0.0, message), 0.0, "ai_enhanced"
                    )
        ...
        *,
        ...
        if intent == "task_stats":
            if match.groups():
                ...
                entities.update(self._parse_time_period(time_period))
            else:
                entities["days"] = 7
    ...
    def _extract_schedule_entities_rule_based(
        ...
        *,
        ...
        if intent == "show_schedule":
            entities["category"] = match.group(1).strip() if match.groups() else "all"
            return True

        if intent == "update_schedule":
            if match.groups():
                if len(match.groups()) >= 2:
                    entities["action"] = match.group(1).strip()
                    entities["category"] = match.group(2).strip()
                else:
                    action_match = re.search(
                        r"(enable|disable|turn\s+(on|off))", message.lower()
                    )
                    if action_match:
                    ...
                    category_match = re.search(
                        r"(tasks?|check.?ins?|messages?)", message.lower()
                    )
                    if category_match:
                        entities["category"] = category_match.group(1)
            return True

        if intent == "add_schedule_period":
            if len(match.groups()) >= 2:
                entities["period_name"] = match.group(1).strip()
                entities["category"] = match.group(2).strip()
        ...
        if intent == "edit_schedule_period":
            if len(match.groups()) >= 2:
                entities["period_name"] = match.group(1).strip()
                entities["category"] = match.group(2).strip()
                time_match = re.search(
        ...
        *,
        ...
        if intent == "checkin_history":
            ...
            limit_match = re.search(
                r"(?:last|past)\s+(\d+)\s+check.?ins?", message_lower
            )
            if limit_match:
    ...
    @handle_errors("parsing time period", default_return={"days": 7, "period_name": "this week"})
    def _parse_time_period(self, time_period: str) -> dict[str, Any]:
        """Parse a time period string (e.g. 'this week', 'last week', '3 days') into days and period_name."""
        result: dict[str, Any] = {}
        period_lower = (time_period or "").lower().strip()
        days_match = re.search(r"(\d+)\s+days?", period_lower)
        weeks_match = re.search(r"(\d+)\s+weeks?", period_lower)
        months_match = re.search(r"(\d+)\s+months?", period_lower)
        if days_match:
            result["days"] = int(days_match.group(1))
            result["period_name"] = f"{days_match.group(1)} days"
        elif weeks_match:
            result["days"] = int(weeks_match.group(1)) * 7
            result["period_name"] = f"{weeks_match.group(1)} weeks"
        elif months_match:
            result["days"] = int(months_match.group(1)) * 30
            result["period_name"] = f"{months_match.group(1)} months"
        elif "this week" in period_lower or "current week" in period_lower:
            result["days"] = 7
            result["period_name"] = "this week"
        elif "last week" in period_lower or "past week" in period_lower:
            result["days"] = 7
            result["period_name"] = "last week"
        elif "this month" in period_lower:
            result["days"] = 30
            result["period_name"] = "this month"
        elif "last month" in period_lower or "past month" in period_lower:
            result["days"] = 30
            result["period_name"] = "last month"
        elif "this year" in period_lower:
            result["days"] = 365
            result["period_name"] = "this year"
        elif "last year" in period_lower or "past year" in period_lower:
            result["days"] = 365
            result["period_name"] = "last year"
        else:
            result["days"] = 7
    ...
    def _extract_update_entities(self, update_text: str) -> dict[str, Any]:
        ...
        try:
            ...
            due_match = re.search(
                r"(?:due\s+date|due)\s+(.+)", update_text, re.IGNORECASE
            )
            if due_match:
        ...
        try:
            ...
            if not partial_message:
                ...
                # Profile suggestions
                    ...
                    word in partial_lower for word in ["profile", "name", "pronoun"]
                ):
                    suggestions = [
                        "Show my profile",
                ...
                # Schedule suggestions
                    ...
                    word in partial_lower
                    for word in ["schedule", "time", "when", "reminder"]
                ):
                    suggestions = [
                        "Show my schedule",
                        "Schedule status",
